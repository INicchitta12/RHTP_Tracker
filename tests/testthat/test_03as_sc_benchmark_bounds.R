...
test_that("the as-typed figures are session 50's and no SC row moved", {
  d <- sc_rows()
  expect_equal(nrow(d), 228L)
  expect_equal(sum(d$hospital), 113L)
  expect_equal(round(sum(d$amount[d$hospital]), 2), 115840714.95)
  expect_equal(pct("session 50's typing"), 69.2)
  expect_equal(b$award_share_pct[b$variant == "session 50's typing"], 49.6)
})
...
test_that("1: rural-only on source-backed evidence is FAR below 60, and mostly untestable", {
  ...
  expect_equal(pct("what 60% would require"), 86.7)
})

test_that("2: psychiatric rows are found by CMS CCN, and their exclusion lands at 64.7", {
  d <- sc_rows()
  ...
  expect_equal(pct("all three psychiatric/behavioural rows"), 64.7)
  expect_equal(round(69.2 - pct("all three psychiatric/behavioural rows"), 1), 4.5)
})
...
test_that("no testable reading reaches 60; the combination leaves 3.1 points", {
  ...
  expect_equal(pct("psychiatric excluded AND the three site rows treated as non-hospital"), 63.1)
})
