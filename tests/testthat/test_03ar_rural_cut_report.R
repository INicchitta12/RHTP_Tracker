...
test_that("every NAMED_HOSPITAL row lands in exactly one class, and the partition is intact", {
  ...
  # 20 unpriced SMRP hospitals (+51 rows / +$57,215,819.43).
  expect_equal(nrow(rows), 1033L)
...
test_that("the rural figure is 179 rows / $176,578,205.56, from three classes of source only", {
...
test_that("the committed report tables match a fresh computation, and no state file moved", {
  ...
  expect_equal(sum(by$rural_rows), 179L)
  ...
  expect_equal(nrow(committed), 1033L)
