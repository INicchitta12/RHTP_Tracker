...
1. Get a clean, current checkout of main. If you are not already in a clone, run: git clone https://github.com/INicchitta12/RHTP_Tracker && cd RHTP_Tracker. Then: git checkout main && git pull --ff-only origin main. A 5xx from GitHub or its credential service ("503", "credential", "Service Unavailable", "Internal Server Error") is TRANSIENT, not a stop condition: retry up to 5 more times, waiting 30, 60, 120, 240 and 480 seconds, and only then report the last error verbatim and STOP (session 66; Washington's 2026-09-24 10:10Z firing was lost to one 503).
...
3. PUBLISH THE VERDICT. THIS STEP IS THE DELIVERABLE: a verdict that does not reach main did not happen (CLAUDE.md 0.5). New York's roster went unseen for 18 days because earlier runs of these Routines skipped it.
   ...
   c. git push origin HEAD:main. If it is rejected because main moved, run git pull --rebase origin main (the file is append-only and carries merge=union in .gitattributes) and push again. If rejected with a 5xx, wait and retry the same push. Retry up to 6 times in total, waiting 30, 60, 120, 240 and 480 seconds.
