...
R/
  ...
  03ak_ms_year1_probe.R        # Mississippi — SELECTION COMPLETE, announcement PROMISED (BUILT)
...
data/
  ...
  evidence/                    # <state>/<record_id>_<date>.pdf — COMMITTED
    ...
    CA/                        #   the CalRHT programme + funding pages, CMS's OWN
                               #   NOTICE OF AWARD -- TWO REVISIONS, -01-02 and
                               #   -01-04, and BOTH STAY: the later one moves no
                               #   money (it renames an officer) and its Federal
                               #   Award Date is +242 days past the anchor where
                               #   -01-02's is +92, which is what makes session
                               #   36's date pin measurable -- the budget
                               #   narrative and all
...
| RCJ 50-state survey | `R/03k_rcj_state_survey.R` | **Built and run (Session 16); `INVESTIGATED_NO_PROBE` added Session 43. 38 of 50 states hold Tier 3 candidates; 29 were RCJ_ONLY and never investigated. §0.1 signal, never a dollar figure. THE DISPOSITION IS NOW FOUR-WAY: `EXTRACTED` 22, `INVESTIGATED_NO_LIST` 8 (worked, archived, AND probed — a re-checkable negative), `INVESTIGATED_NO_PROBE` 6 (worked and written down, NO probe, NOT re-checkable), `QUEUED` 14. `SURVEY_EXTRACTED_STATES`, `SURVEY_INVESTIGATED_NO_LIST_STATES` and `SURVEY_INVESTIGATED_NO_PROBE_STATES` are the constants; both tables are REBUILT from them and never hand-edited** |
...
| **Mississippi Year 1** | `R/03ak_ms_year1_probe.R` | **Built and run (Session 44). A NEGATIVE ABOUT THE ROSTER ONLY, AND THE MOST IMMINENT STATE IN THIS REPOSITORY.** `mississippirhtp.com` — a DEDICATED RHTP DOMAIN, the third after Kentucky's and Arkansas's — says *"The selection process is complete"* for RCGC, RTG and TCE, that selected applicants are contacted **BY EMAIL** to begin sub-award execution, and that *"Governor Tate Reeves will formally announce details regarding all executed sub-awards in the coming weeks"*. South Carolina's shape WITH a public announcement attached. **ITS CMS FOOTER IS THE FIRST HERE THAT IS NOT 100% FEDERAL** — *"totaling $205,990,180.16, with 99.96% funded by CMS/HHS ($205,907,220.16) and 0.04% funded by non-government sources ($82,960)"* — measured against **222 committed footers, ALL of them 100 percent**. The headline is Tier 1 PLUS A MATCH and is **ACCEPTED as a `SOLICITATION` pool** by the tier check, which is the defect; **PARSE THE CMS SHARE AND DO NOT WIDEN THE MARGIN**. The footer is internally exact — headline − CMS share = $82,960.00, the printed match, to the cent. **BOTH CONTROLS ARE UNUSUALLY GOOD**: the Governor's newsroom is the channel the state itself named and carries priced awards; DOM's Completed Procurements page carries ten uniform intent-to-award notices AND the RHTP-titled **HORNE LLP** consultant award of 2025-08-13, **138 days BEFORE the NOA** — which session 20's sweep had ALREADY flagged `PROVENANCE_PREDATES_NOA`, so machine and hand agree from two directions. **TWO DIGEST MECHANISMS, ONE NEW**: Cloudflare email obfuscation (Louisiana's, recurring) and, on `medicaid.ms.gov`, the WordPress Simple Banner plugin serialising the render clock **TO THE MICROSECOND** into a script body. On Routine `trig_01RmgWna5oXyoMCXYrVqGVvE`, **TUESDAYS AND FRIDAYS 09:40 UTC**; ran live and reports UNCHANGED — `docs/session44_delaware_extraction_mississippi_footer.md` |
...
| Tests | `tests/testthat/test_00_cms_press_monitor.R`<br>`test_01_retrieve_rcj.R`<br>`test_02_normalize.R`<br>`test_03_state_registry.R`<br>`test_03b_budget_narratives.R`<br>`test_03c_cms_abstracts.R`<br>`test_03d_ga_great_health.R`<br>`test_03e_fl_year1_awardees.R`<br>`test_03f_pa_year1_awardees.R`<br>`test_03g_al_year1_awardees.R`<br>`test_03h_ak_year1_awardees.R`<br>`test_03i_sd_rht_contracts.R`<br>`test_03j_sd_year1_announcements.R`<br>`test_utils_recipient_classification.R`<br>`test_state_union.R`<br>`test_flow_table_parity.R` | `test_03m_or_year1_awardees.R`<br>`test_03n_tx_year1_probe.R`<br>`test_03o_ks_year1_awardees.R`<br>`test_03p_md_year1_awardees.R`<br>`test_03q_state_completeness_recheck.R`<br>`test_utils_pdf_text.R`<br>`test_03r_ne_year1_awardees.R`<br>`test_03s_in_year1_awardees.R`<br>`test_03t_ok_year1_awardees.R`<br>`test_03u_nv_year1_awardees.R`<br>`test_03v_mi_year1_awardees.R`<br>`test_03w_mo_year1_awardees.R`<br>`test_03x_nh_year1_awardees.R`<br>`test_03y_wi_year1_probe.R`<br>`test_03z_ia_year1_awardees.R`<br>`test_03aa_me_year1_awardees.R`<br>`test_03ab_ca_year1_probe.R`<br>`test_03ac_ct_year1_probe.R`<br>`test_03ad_nm_year1_probe.R`<br>`test_03ae_la_year1_probe.R`<br>`test_03af_ky_year1_probe.R`<br>`test_03ag_ny_year1_probe.R`<br>`test_03ah_nc_year1_sources.R`<br>`test_utils_footer_tier.R`<br>`test_03ai_ar_year1_awardees.R`<br>`test_03aj_wy_year1_awardees.R`<br>`test_02c_state_attribution_sweep.R`<br>`test_utils_footer_cms_share.R`<br>`test_03ak_ms_year1_probe.R`<br>`test_03al_de_year1_awardees.R`<br>`test_03am_id_year1_awardees.R`<br>`test_03an_oh_year1_awardees.R`<br>**Built — 52 files; all pass and 1 self-skips (the stage 00 first-run branch, now that the CSV exists). Zero quota. Run `Rscript tests/run_tests.R`.** **MEASURED 2026-09-19: 5,386 assertions across 52 files, 0 fail, 1 self-skip.** **Session 48 added 88 (5,298 -> 5,386), 87 of them in `test_03ab_ca_year1_probe.R` (221 -> 308), and the two that carry the weight are the ones about the COUNCIL rather than the menu**: the first requires all FOUR pinned hospital employers to be on the roster AND each one's position on the SRHRP page to be what `CA_RHPC_HOSPITAL_EMPLOYERS` says, and the second **promotes Marshall Medical Center into the awarded block offline and requires the build to STOP** — which is what makes "§0.3 here, §0.1 there" a checked distinction rather than a note. Beside them: the session-48 pair is required NOT to be in RCJ's eleven while Adventist Health Reedley's $1,325,000 IS required to be one of HCAI's five named awards (the half that widens the trap); `ca_srhrp_position()` is driven on an award, an eligibility row, an absent name and a gutted page; the count is pinned at 19 with all seventeen session-45 members still present and none typed; and the fourth global-menu move is reproduced across every `CA_PROBE_KEYS` page with the SRHRP control's 102 rows and awarded block asserted untouched. **MEASURED 2026-09-16: 5,298 assertions across 52 files, 0 fail, 1 self-skip.** **Session 47 added 57 (5,241 -> 5,298), all in `test_03ab_ca_year1_probe.R` (164 -> 221), and the ones that carry the weight read BOTH archived Notices of Award rather than the table that describes them**: the invariants are checked against both PDFs, the per-revision fields are required to differ, the later document is required to still say *"approves the key personnel change"* and *"All other terms and conditions remain in effect"*, and the drift is pinned at **exactly c(92L, 242L)** with a check that it exceeds Connecticut's 206. Two counterfactuals are DRIVEN — a revision with the amount moved to $199,000,000 and one with the budget period moved to 08/28/2026 both FAIL, which is what separates a paperwork revision from a finding about the award — and the label tripwire is driven three ways: the live label, a future *"(December 1, 2026)"* that must stop the build, and the label removed entirely, which must also fail rather than pass. Plus a test pinning that `cms_noa` is NOT in `CA_PROBE_KEYS`, which is why the label is what watches. **MEASURED 2026-09-12: 5,241 assertions across 52 files, 0 fail, 1 self-skip.** **Session 46 added 29 (5,212 -> 5,241), all in `test_03ab_ca_year1_probe.R` (135 -> 164), and the two that carry the weight are the ones that could have gone the other way**: the first walks every one of `CA_PROBE_KEYS` and requires the global navigation rename to move the CONTENT digest by EXACTLY +12 characters on each page — the mechanism reproduced offline, rather than asserted — and the second **plants `"CalRHT Awardees have been awarded"` IN THE MENU and requires the tripwire to fire on it**, which is what makes "the nav is deliberately not reduced away" a tested decision instead of a comment. The rest pin the manifest's three newly-DERIVED values (its character count, its refresh date, and `ADDED SESSION 45` where the generator had typed 36) and re-pin that session 34's retracted *"digests are STABLE"* claim has not come back. **Session 45 added 41 (5,171 -> 5,212)** for the RHPC: the governance roster, the narrow phrase set's exclusions, the tripwire firing per phrase per page, the two-hospital overlap against `ca_rcj_candidates()`, and the 7-row status table. **Session 44 added 431 assertions (4,740 -> 5,171) across FIVE new files.** `test_utils_footer_cms_share.R` (71) is the one to read first: it drives Mississippi's headline being **ACCEPTED** as a Tier 2 pool, pins that the footer's own three numbers close to the cent, requires the parser to be INERT on the 100% form (all eighteen phrasings measured in the corpus), drives the REFUSAL TO COMPUTE when a partial share prints no dollar figure, and **pins `RHTP_FOOTER_ALLOTMENT_MARGIN` at $10,000 with the reason**. `test_03al_de_year1_awardees.R` (45) prices Delaware's counterfactual — with the classifier unaided the state has NO hospital rows — and drives the $48,750 derivation, the fifth-award tripwire and the session-8 budget closure. `test_03ak_ms_year1_probe.R` (43) requires the tripwire NOT to fire on the page's own "selected applicants"/"sub-award execution" language. `test_03an_oh_year1_awardees.R` (31) pins the footer LIMIT — the tier check accepts a Tier 3 figure as a pool — and prices Ohio's own counterfactual at $10,000,000. `test_03am_id_year1_awardees.R` (28) is Delaware's control: same machine answer, opposite verdict. **The queue pins moved BECAUSE THE WORK WAS DONE**: the QUEUED bucket is now the TEN states left ($1,916,786,628 / 24 candidates) and the fifty split **25 / 9 / 6 / 10**, with a test requiring DE, ID and OH to have reached `EXTRACTED` and MS `INVESTIGATED_NO_LIST` rather than the weaker code. Session 43 added **23 assertions** (4,717 -> **4,740**) across `test_03k` and `test_00b` for the new `INVESTIGATED_NO_PROBE` code: they pin its membership, its presence in BOTH vocabulary columns, that the three worked statuses are DISJOINT, that its note still says it is WEAKER and that the next action is to WRITE THE PROBE, that the QUEUED bucket is EXACTLY the fourteen low-candidate states ($2,668,093,442 / 34 candidates), and that the fifty states split 22 / 8 / 6 / 14 with every state carrying a disposition. `test_state_union.R` now combines **twenty-six** state files, twenty-five states — and NEITHER Missouri's 27-row Hub Anchor roster, NOR Maine's 11-row invited cohort, NOR Arkansas's 50-row project file is among them. The first two are not awards at all; **Arkansas's is the same $149,177,618.45 twice, at a finer grain**, which is the sharper hazard.|
...
```
                          ROWS       DOLLARS  STATES
NAMED_HOSPITAL        :    613   482,781,258      16   GA 90.3M · **WY 72.7M** ·
                                                       AL 66.1M · OR 50.2M ·
...
- **THE 50-STATE DISPOSITION AFTER SESSION 44, which is the line to read
  before opening either of the two files below.** `EXTRACTED` **25** ·
  `INVESTIGATED_NO_LIST` **9** · `INVESTIGATED_NO_PROBE` **6** (HI MA MN NJ SC
  TN — worked, no probe, NOT re-checkable) · `QUEUED` **10**. **The QUEUED
...
- **`data/reference/ms_year1_status.csv` — 10 rows (Session 44), AND IT HAS NO
  `amount` COLUMN. `ms_year1_awardees.csv` does not exist and a test asserts
  its absence.** THREE programmes read `SELECTED_NOT_ANNOUNCED`: Mississippi
  says *"The selection process is complete"*, that recipients are contacted BY
  EMAIL, and that *"Governor Tate Reeves will formally announce details
  regarding all executed sub-awards in the coming weeks"*. One row is the
  **Governor's newsroom**, the POSITIVE control and the channel the state
  itself named; one is **MS Division of Medicaid's Completed Procurements**
  page, which is BOTH controls at once — ten uniform intent-to-award notices,
  and the RHTP-titled **HORNE LLP** consultant award of 2025-08-13, **138 days
  BEFORE the NOA**, which session 20's sweep had already flagged
...
- **`data/reference/nh_rcj_candidate_disposition.csv` — 5 rows (Session 29).**
  ...
  **documents**, not awards. **16 are administrators for whom no reachable
  ...
  run.
- **`data/reference/ca_year1_status.csv` — 7 rows (Sessions 34, 45), AND IT HAS
  NO `amount` COLUMN.** What each California RHTP channel publishes, with an
  ...
  **A SEVENTH ROW WAS ADDED IN SESSION 45, BY THE SCHEDULED WATCH**: the
  **Rural Health Policy Council (RHPC)**, `GOVERNANCE_ONLY`, `publishes_roster
  = No`. California's twice-weekly probe reported the CalRHT programme page
  **CHANGED for the first time** on 2026-09-09; the change was **ONE navigation
  link** ("Learn More") to a council **already linked** from the archived page,
  the reduced text went 11,162 -> 11,212 characters, and **every award tripwire
  still passed**. The council is seventeen named people with employers and
  biographies — a council member is not a recipient (Missouri's Hub Anchors,
  Connecticut's leadership team). **AND IT CARRIES A COMPOUNDING TRAP THIS
  PROJECT HAD NOT MET: TWO of the seventeen are hospital executives —
  Community Memorial Hospital-Ojai and Plumas District Hospital — and BOTH of
  those hospitals are among the eight distinct awardees in RCJ's eleven
  California Tier 3 candidates, i.e. the SRHRP seismic programme.** So a
  cross-reference of "hospitals named on the CalRHT estate" against the RCJ
  candidate list **matches on two hospitals, from two INDEPENDENT wrong
  reasons**, and neither is an RHTP award — the match would read as
  corroboration. Both names are pinned by `ca_assert_rhpc_is_governance()`,
  and both council pages are archived as `*_GOVERNANCE.html`.
...
```
Rscript tests/run_tests.R                        # 5,298 assertions, zero quota
...
Rscript R/03ak_ms_year1_probe.R --fetch        # MS: archive 5 sources + SHA-256
Rscript R/03ak_ms_year1_probe.R --validate     # MS assertions + BOTH controls, offline
Rscript R/03ak_ms_year1_probe.R --build        # writes the two MS status CSVs (NO award file)
Rscript R/03ak_ms_year1_probe.R --probe        # LIVE: has Mississippi announced yet?
Rscript R/03ak_ms_year1_probe.R --report       # the footer that is NOT 100% federal
