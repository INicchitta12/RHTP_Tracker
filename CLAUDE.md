...
5. **Keep working the RCJ_ONLY queue, richest first. LOUISIANA NOW LEADS** at
   6 candidates / 6 distinct awardees, then DE 5/5, AZ 4/2, CO 4/4.
   **Connecticut and New Mexico are `INVESTIGATED_NO_LIST` as of session 35**
   and are off the queue, as is California (session 34); Maine and Iowa are
   `EXTRACTED`. **CA, CT, NM, TX and WI** are `INVESTIGATED_NO_LIST`; IA, KS,
   MD, ME, MI, MO, NE, NH, IN, OK and NV are `EXTRACTED`.
...
```
Rscript tests/run_tests.R                        # 3,917 assertions, zero quota
