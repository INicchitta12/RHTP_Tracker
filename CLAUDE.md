...
docs/
  ...
  session62_stage2_0924_wrong_state_dispositions.md # Stage 2 on the
                                     # 09-24 pull; mode 6 reaches Tier 3;
                                     # 19 dispositions re-read; LA stale
...
| **NE** | **57** (+21 unpriced) | **$36,137,615** | **54** | **$12,606,594 named + $18,156,856 pooled-but-named** (VERIFIED session 49) |
...
| **NV** | **73** | **$87,400,000** (POOL level only) | **27 award actions** | **$0 — NEVADA PUBLISHES NO PER-RECIPIENT AMOUNT** |
| **MI** | **139** | **$69,883,392** (a TOTAL, not a floor) | **2** | **$259,121** — see below |
...
**SESSION 62 LEFT THESE.** (1) Indiana's GROW regional extraction + probe (session 61's item 1, untouched; `in_assert_regional_not_awarded()` passes only on the 08-31 archive). (2) Archive the pages session 62 read live (TX HHS0016568/16736, CA Distressed Hospital, DE DSHA, LA 09-09 deck, NE 5.3, MI roster) through each state's `--fetch`. (3) Rewrite `R/03ae` as a Louisiana extractor; extract Nebraska 5.3 and Michigan's six new rows. (4) Add CA Distressed Hospital and DE Downtown Development Districts to `non_rhtp_state_programs.csv`. (5) Read the new candidates in states with no disposition file: AK 248 (re-keyed), FL 80, KS 39, SD 28. (6) Stage 2 tiers SC Dept of Corrections and three MI roster rows UNASSIGNED.
