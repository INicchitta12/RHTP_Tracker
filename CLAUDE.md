...
data/
  ...
  evidence/                    # <state>/<record_id>_<date>.pdf — COMMITTED
    ...
    WY/                        #   the DRIVE-FOLDER documents the state links from
...
```
                          ROWS       DOLLARS  STATES
NAMED_HOSPITAL        :    578   410,119,935      14   GA 90.3M · AL 66.1M · OR 50.2M ·
...
across 14 states**. Arkansas is the first state in three sessions to move the
...
**WYOMING IS THE FIRST ITEM, AND IT IS AN EXTRACTION, NOT A HUNT (session 41).**
...
**INDIANA'S GROW REGIONAL GRANTS HAVE A CMS RELEASE NOW (2026-09-03, $120M).**
Run `Rscript R/03s_in_year1_awardees.R --fetch --force && --validate` against
the live page; `in_assert_regional_not_awarded()` is designed to fail the
day it awards, and the release is the signal that day may have come.
