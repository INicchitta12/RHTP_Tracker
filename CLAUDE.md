...
```
                          ROWS       DOLLARS  STATES
NAMED_HOSPITAL        :    692   542,946,270      17   GA 90.3M · WY 72.7M ·
                                                       AL 66.1M · **AK 62.4M** ·
                                                       OR 50.2M · FL 49.3M ·
                                                       **MS 47.5M (68 rows)** ·
                                                       KS 35.7M · PA 24.1M ·
...
- **THE 50-STATE DISPOSITION AFTER SESSION 46, which is the line to read
  before opening either of the two files below.** `EXTRACTED` **26** ·
  `INVESTIGATED_NO_LIST` **8** · `INVESTIGATED_NO_PROBE` **6** (HI MA MN NJ SC
  TN — worked, no probe, NOT re-checkable) · `QUEUED` **10**.
...
```
Rscript tests/run_tests.R                        # 5,336 assertions, zero quota
