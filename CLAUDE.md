...
**ARKANSAS IS THE FIRST THING TO DO, AND IT IS AN EXTRACTION, NOT A HUNT
(session 39).** The report is written and the source is located, archived
nowhere yet, and reconciles: **31 organisations, $149,177,618.45, 71.5% of the
allotment**, on `arkansasrhtp.com` under Arkansas's own words *"Download the
List of Organization and Award amounts"*, corroborated by the Governor's
2026-08-27 release (*"$149.3 million"*, *"the 50 project awards"*). **It was
deliberately NOT extracted this session because Task 2 said report first.**
Four things an extractor has to get right:

- **`rhtp_pdf_run_table()`, not `rhtp_pdf_lines()`.** The three amount columns
  are painted at one `y` and the line model merges them into
  `"$2,571,095.00$0.00$2,571,095.00"` — session 32's Adair County case exactly.
  The runs model separates them cleanly; this is the first state to need it.
- **The `Total:` row is a row.** It carries the full $149,177,618.45 in the
  same shape as an award row, so a naive parse reports 32 organisations and
  double the money. Use it as the reconciliation target, never as a recipient.
- **THE UNSTATED-FORM QUESTION IS THE LARGEST IN THE PROJECT IN DOLLARS: 16
  rows / $100,723,693, 67.5% of the round**, against a 9-row / $21,792,688
  named-hospital floor. It runs strongly UPWARD — Baxter Health $19.7M, Mercy
  Health Fort Smith $19.1M, Baptist Health $16.9M, St. Bernards $14.6M — and
  **Arkansas Rural Health Partnership ($18.8M) is a hospital CONSORTIUM**,
  which is §10.2's association row and must be read off the source, never the
  name. **PROMOTE NOTHING (§0.4)**; queue it as `AR_RECIPIENT_FORM_NOT_STATED`.
- **TWO OF FOUR INITIATIVES ARE STILL TO COME** — RISE and HEART, against *"the
  $209 million the state expects to award by this fall"* — so Arkansas's file is
  a FLOOR by construction and needs a probe, not a one-off.
...
5. **THE RCJ QUEUE IS NEARLY EXHAUSTED AS A DISCOVERY SIGNAL, AND THE NEXT
   ...
   **CA, CT, LA, NM, TX and WI** are `INVESTIGATED_NO_LIST`; IA, KS, MD, ME,
   MI, MO, NE, NH, IN, OK and NV are `EXTRACTED`.
...
```
Rscript tests/run_tests.R                        # 4,446 assertions, zero quota
