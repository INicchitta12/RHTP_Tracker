...
```
                          ROWS       DOLLARS  STATES
NAMED_HOSPITAL        :    609   482,781,258      15   GA 90.3M · **WY 72.7M** ·
                                                       AL 66.1M · OR 50.2M ·
...
- **THE 50-STATE DISPOSITION AFTER SESSION 43, which is the line to read
  before opening either of the two files below.** `EXTRACTED` **22** ·
  `INVESTIGATED_NO_LIST` **8** · `INVESTIGATED_NO_PROBE` **6** (HI MA MN NJ SC
  TN — worked, no probe, NOT re-checkable) · `QUEUED` **14**. **The QUEUED
  bucket is now exactly the fourteen low-candidate states, $2,668,093,442 and
  34 candidates between them**, and session 43 read all fourteen without
  extracting any (see §2 of `docs/session43_*`). So for the first time every
  state sits in a bucket that says what was done to it, and "queued" means
  nobody has built a file for it rather than nobody has looked.
...
```
Rscript tests/run_tests.R                        # 4,740 assertions, zero quota
