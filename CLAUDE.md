...
award tripwires against the LIVE bytes. **It is NOT yet on a Routine** — put it
on one, and note that NO ROTATING-TOKEN STUDY HAS BEEN DONE on
`health.wyo.gov`: the probe compares a CONTENT digest by inheritance from six
other hosts, not by measurement here (§0.4).
...
**THE SEVEN WORKED-BUT-UNCODED STATES (HI MA MN NJ SC TN WY) READ
`NOT_EXTRACTED` IN BOTH TABLES ON PURPOSE.** `R/03k`'s `INVESTIGATED_NO_LIST`
requires a committed archive AND a probe; none of the seven has a probe and
Wyoming is not a negative. Either write the probes (SC and MN first — SC
has awarded by email, MN is the largest §0.3 trap in the project) or leave
them as they are; do not hand-edit the tables.

**THE REMAINING QUEUE IS FOURTEEN LOW-CANDIDATE RCJ STATES HOLDING $2.67
BILLION AND 34 CANDIDATES BETWEEN THEM** — MT, MS, OH, CO, WV, ND, UT, VT,
VA, ID, WA, AZ, DE, RI, in allotment order. Three of them (OH, ND, WV) have a
CMS release and were locator-reported in session 11; Virginia was worked in
sessions 14-15. Work them by allotment, not by candidate count — Iowa is the
standing proof that the count says nothing.
...
```
Rscript tests/run_tests.R                        # 4,717 assertions, zero quota
