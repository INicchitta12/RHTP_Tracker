...
R/
  ...
  03ah_nc_year1_sources.R      # North Carolina — TWO ROSTERS, 44 named, NOT EXTRACTED (BUILT)
...
| **North Carolina Year 1** | `R/03ah_nc_year1_sources.R` | **Built and run (Session 37). NOT A NEGATIVE — TWO PUBLISHED ROSTERS, 44 NAMED RECIPIENTS, AND DELIBERATELY NOT EXTRACTED.** It is **THE SECOND FLORIDA**: invisible to BOTH discovery layers, the largest allotment in that group ($213,008,356), and it has published **39 named Mobile Integrated Health recipients ($10,000,000)** and **5 named NC ROOTS Hub Leads**. **NOT ONE PER-RECIPIENT DOLLAR EXISTS** — Nevada's and Iowa's shape — so an extraction must leave `amount` empty with the pool in `round_amount`. **38 of the 39 are county EMS agencies**; **Cape Fear Valley** is the one hospital-affiliated recipient and the only §10.2 judgement in the set. **THE HUB LEADS ARE NOT MISSOURI'S HUB ANCHORS AND ONE WORD DECIDES IT**: *"programmatic and **FIDUCIARY** leads"* against Missouri's *"will not act as the fiscal agent"*. **UNC appears under TWO SPELLINGS** across two documents from one agency (§2 forbids merging). **THE ROOTS PAGE'S ONLY CURRENCY FIGURE IS THE ALLOTMENT** (§0.2). Contracts were to be finalised **2026-06-01**, PASSED. **THE SECOND TIER — where the hospital money is — NAMES NOBODY.** `nc_year1_awardees.csv` is ASSERTED ABSENT — `docs/session37_footer_tier_ky_ny_nc.md` |
...
- **`data/reference/nc_year1_status.csv` — 6 rows (Session 37), AND IT HAS NO
  ...
  PER-RECIPIENT DOLLAR**. **`nc_year1_awardees.csv` DOES NOT EXIST AND AN
  ASSERTION KEEPS IT THAT WAY** — the extraction was deliberately deferred so
  it could be reported first, and `R/03ah`'s header records what it has to get
  right: the amounts are POOL figures; **Cape Fear Valley** is the one
  hospital-affiliated recipient among the 39; **UNC appears under two
  spellings** across two documents; and the ROOTS page's only currency figure
  is the **ALLOTMENT** (§0.2). North Carolina stays **`NOT_EXTRACTED`**, never
  `INVESTIGATED_NO_LIST`, which would be a false claim about a state with two
  rosters.
...
**NORTH CAROLINA IS THE EXTRACTION WAITING TO BE DONE, AND IT IS THE FIRST
ITEM (session 37).** It publishes **44 named recipients across two rosters**
and this session deliberately did not extract them — the evidence is archived
under `data/evidence/NC/`, `R/03ah_nc_year1_sources.R --report` prints the
finding, and `nc_year1_awardees.csv` is asserted absent. **Before building it,
read `R/03ah`'s header**: the $10,000,000 is a POOL and NOT a per-recipient
amount (so `amount` is empty on all 39 rows, Nevada's and Iowa's device);
**Cape Fear Valley Mobile Integrated Health** is the ONE hospital-affiliated
recipient among 39 county EMS agencies and the only §10.2 judgement in the set;
**UNC appears as "University of North Carolina Hospitals" and as "UNC Health"**
across two documents from one agency, which §2 forbids a machine merging; and
**the ROOTS page's only currency figure is the $213,008,356.47 ALLOTMENT**, so
§0.2's refusal is the only thing standing between that page and five hub awards
worth the whole state's money. **The Hub Leads ARE recipients** — NCDHHS calls
them *"programmatic and **fiduciary** leads"*, which is the word that separates
them from Missouri's Hub Anchors — but **no per-hub amount is published
anywhere**.

**AND THE SECOND TIER IS WHERE NORTH CAROLINA'S HOSPITAL MONEY WILL BE.** Each
Hub Lead runs its own regional funding opportunities; Trillium's Region 2 page
names nobody today, and `nc_assert_positive_control()` is designed to FAIL the
day one does.

**KENTUCKY AND NEW YORK ARE NEGATIVES WITH PASSED DATES AND NEITHER IS ON A
SCHEDULE YET (session 37).** Both `--probe` cleanly against live bytes and both
report UNCHANGED. **New York's is the more urgent**: its RCHI contracts were due
to BEGIN 2026-09-01 and all contracts must be EXECUTED by 2026-10-30, against
91 applications DOH says are still under review. Kentucky's two
award-notification dates (2026-07-10, 2026-08-26) have both passed. Putting
each on a twice-weekly Routine is the obvious next step, on Wisconsin's and
Maine's footing.
...
Rscript R/03ah_nc_year1_sources.R --validate # NC assertions incl. the NOT-EXTRACTED guard
Rscript R/03ah_nc_year1_sources.R --build    # writes nc_year1_status.csv (NO award file)
Rscript R/03ah_nc_year1_sources.R --report   # TWO ROSTERS, 44 named, NOT extracted
```
