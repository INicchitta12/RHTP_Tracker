...
**Last updated:** 2026-09-25 (Session 72 — **WHERE THE AWARDING STATE STATES A FORM, IT STANDS.** `ENROLLED_HOSPITAL_STATE_STATED_OTHER_FORM` RESOLVED at option (a) on the owner's instruction, and §10.2's AHC block gains the precedence rule (byte-identical in all three docs). The ten AK/OR rows keep their state-stated coding; each now RECORDS its CMS enrolment in `cms_enrolment_record` (read from the archived file) with `cms_enrolment_match` and `ccn` left empty. $0 moved. **AltaPointe ($3,602,968, CCN 014014) is owner-accepted**: the release states no form, so the enrolment decides; reasoning on each row and in `ALTAPOINTE_ENROLLED_OPERATOR` (RESOLVED). **UMMS matches NO enrolment on its own legal name**: the complete MD file (57) enrols its eight hospitals as separate legal bodies. It stays queued with UAB Montgomery, OHSU Casey Eye and MEDIC. `NAMED_HOSPITAL` unchanged at 1,185 / $1,022,314,842.52 / 30 — `docs/session72_state_form_precedence_altapointe_umms.md`) (Session 71 — **ACADEMIC HEALTH CENTRES ARE HOSPITALS WHERE CMS ENROLS THE LEGAL ENTITY.** `AHC_ENROLLED_HOSPITAL_OPERATOR` RESOLVED at option (b), on the owner's instruction. Spec §10.2 gains "Academic health centers and enrolled hospital operators" (byte-identical in all three docs, parity-tested). `R/03bj` re-types every row whose awardee's LEGAL ENTITY is an enrolled hospital: HOSPITAL_OR_SYSTEM, DIRECT/Yes, CCN on the row, `recipient_subtype = ACADEMIC_HEALTH_CENTER` where it is one. **33 rows / +$63,240,239.84**: 25 AHC rows / $53,539,221.31 (UAMS $17,060,066, UAB, South Alabama, OHSU, UW, U of Utah via WY, Regents of U Michigan, Emory, UNC and UIHC bridges) and 8 enrolled non-AHC rows / $9,701,018.53 (AltaPointe, Providence-AK, Ochsner, Choctaw Nation, Carson Valley). `NAMED_HOSPITAL` **1,152 / $959,074,602.68 / 28 -> 1,185 / $1,022,314,842.52 / 30** (WA, LA enter); subtract the AHC subtype and it is 1,160 / $968,775,621.21. AR floor 54.6% -> **63.0%**; GA ceiling 57.0% -> **60.2%** (Emory's pool; still strict). A PREFIX is never matched by machine (University of Alabama is not UAB). Ten rows whose STATE source states another form are HELD (`ENROLLED_HOSPITAL_STATE_STATED_OTHER_FORM`, $2,088,858), and seven name-only AHC strings are queued (`AHC_STRING_NAMES_NO_ENROLLED_ENTITY`). **Five of session 49's OTHER rows stated no form** (GA Behavioral Pediatric Resource Center, MI Rudyard, OK Empowerment Solutions, OR Regional Partners and SBHC Planning Grants) and are back on §8's fallback, $0. `other_assert_forms()` now requires a determined form on all 113 OTHER rows. **21 CMS hospital enrolment files archived** (the owner's seven + 14). Run `R/03bj --apply` after any rebuild of a touched state; AR, NV, LA and WA builds chain it — `docs/session71_ahc_enrolled_hospitals_other_forms.md`) (Session 70 — **ARKANSAS IS THE THIRD COMPLETE STATE AND THE ONLY ONE ON INTENTS.** The completed-state tables now carry `intent_rows` / `pct_priced_on_intent` (all 31 states) and `award_action_stage` (COMPLETE states). All three are read off each row's `validation_source_type`. AR is 100% intent (80/80 rows); FL and GA are 0%. AR: $203,862,687.29, 97.6%, floor 54.6% on 30 rows / $111,276,895.52. **NARHC is settled to §8's fallback in BOTH rounds** (`AR_NARHC_TWO_TYPES`): session 49's OTHER had a basis that stated no form, and CMS, NPPES and the IRS BMF carry no record. $0 moved. **UAMS IS AN ENROLLED HOSPITAL'S EXACT LEGAL NAME** (CMS AR Hospital Enrollments, dba UAMS Medical Center, CCN 040016). That puts Winston County's rule (session 50) against OHSU's and UNC's. It is queued as `AHC_ENROLLED_HOSPITAL_OPERATOR`, worth +$17,060,066 in AR (floor to 63.0%). NOT re-coded. Federal records under `data/evidence/federal_records/2026-09-25/` — `docs/session70_complete_states_intent_narhc_uams.md`) (Session 69 — **ARKANSAS ROUND 2 EXTRACTED, ARHP'S FLOW SETTLED, ARKANSAS IS THE THIRD COMPLETE STATE.** `R/03ai` writes `ar_year1_round2_awardees.csv`: RISE AR/HEART, 38 orgs / 43 actions / **$54,685,068.84**, closing to its own `Total:` row. It is a separate file because session 49's overlay is keyed on round 1's row index; the two files ARE additive. **The $203,520 "gap" is a misprint in the Governor's release, not a disagreement**: Conway Regional's GME figure is printed "$203, 520.00". Read with the space, the 54 projects close to the cent; the strict reading misses exactly that figure. Both readings are pinned, and the list stays the figure of record. **+12 `NAMED_HOSPITAL` rows / +$18,870,981.65** (St. Bernards under §10.2's foundation row; Baptist Health carried from session 49 by exact string). **UAMS, CARTI and North Arkansas Rural Health Consortium are QUEUED** (`AR_R2_QUEUED_FORM`, $7,068,862, all `No`). NARHC's round-1 row keeps session 49's OTHER, so the two files differ on it. `AR_R2_RECIPIENT_FORM_NOT_STATED`: 21 orgs / $19,749,288.19. **`AR_ARHP_CONSORTIUM_FLOW` RESOLVED at option (c)**: in all eight described projects ARHP buys or delivers, and no source says money reaches a hospital. ARHP's site lists hospitals among FQHCs, clinics and medical schools. Result: `IN_KIND_BENEFIT` ×3 / `NON_HOSPITAL` ×1, all `No`, **$0 moved**, applied after session 49's overlay (`--build` now re-applies it). **AR `COMPLETE`** on the Governor's "completes the distribution of the $208 million". The $4,916,708.71 remainder is $4,680,988 of **PLANNED** administration (budget narrative) plus **$235,720.73 UNEXPLAINED** (`ar_year1_allotment_gap.csv`). AR hospital share: **54.6% floor, 30 rows / $111,276,895.52**. `NAMED_HOSPITAL` goes **1,140 / $940,203,621.03 → 1,152 / $959,074,602.68**, 28 states. Round 1's figure is $92,405,913.87, not ".96". — `docs/session69_arkansas_round2_arhp_flow_complete.md`) (Session 68 — **ARKANSAS HAS PUBLISHED ITS SECOND ROUND AND IT IS READ, NOT EXTRACTED.** The AR tripwire was real: DF&A's RISE AR / HEART list (posted 2026-09-24) names **38 organisations / 43 organisation×initiative actions / $54,685,068.84** (RISE AR $27,213,468.74, HEART $27,471,600.10), closing to its own `Total:` row to the cent. The Governor's 2026-09-24 release ($54.6M; 30 HEART + 24 RISE projects) says the round *"completes the distribution of the $208 million"*. HEART's 30 priced projects equal the list exactly; **RISE's prose prices only 23 amounts, $27,009,948.74, $203,520 short — UNRESOLVED.** Both rounds together are $203,862,687.29, **97.6%** of the allotment. Preliminary reading, nothing coded: ~12 actions / ~$18.9M go to name-evident hospitals; ARHP ($2,048,665) is the consortium flow question again; UAMS, CARTI and a rural health consortium stay open. Archived under `data/evidence/recheck/2026-09-25/AR/`. **The AR probe will trip every run until `R/03ai` is extended.** **KENTUCKY IS ROUTINE:** `funding` differs by one zero-width space; `rch` recasts FHKY as a *"convening partner"*, and the Hub Leads are still unnamed. **PR #70 was already merged** (`2ad0fc3`). **ALASKA'S NEW RUNNER HAS NOT FIRED YET**: its first run is Mon 2026-09-28 14:07Z. — `docs/session68_arkansas_round2_kentucky_alaska.md`) (Session 67 — **THE NEW RUNNERS PUBLISH, AND THE 31 ORIGINALS ARE DELETED.** KY `trig_015jnDJLCsWAX2CCCamBb1tu` pushed `a6c76d2` (19:35Z) and AR `trig_01UjocQhFRgFwNZioQ6JHyvj` pushed `7cfb554` (20:22Z), both carrying their own trigger id. The 31 disabled originals are deleted; **Alaska's old Routine is now DISABLED** (the permission refusal did not recur) and kept, not deleted. **AR's first new-runner firing is a TRIPWIRE**: the home page names "RISE AR and HEART Project Impact", which the archive does not — RISE AR / HEART may have published, UNREAD. KY reported CHANGED on `funding` and `rch`. **FIVE MORE TESTS PINNED LIVE DATA EXACTLY** and are now keyed by release date, superset or CMS-conditional: GA and VA pins in `test_00`, IL/WY/FL in `test_00b`, IL in `test_03k`, and `test_01b`'s exposed set. Each old form was driven to fail on injected growth and each new form passes. Suite green.) (Session 66 — **THE CMS ROUTINE'S 13:10Z DECLINE WAS THE SUITE, NOT CMS.** Three releases published at 12:00Z (DE $23M, a second AR, a second SD) changed the runner-rewritten state list, and four tests pinned it exactly. So the monitor could never record a new state. Tests now assert floors and recency, and the miss is explained, not filled. **DELAWARE IS NOW `BOTH`** in the trigger list. **32 RUNNER ROUTINES RECREATED WITH THE 5xx RETRY** on their same runners; old ids are kept in `routines.csv`, and coverage is checked across the cut-over. The originals are disabled, not yet deleted, pending two proven publishes. **Alaska's old Routine could not be disabled (permission refusal) and needs the owner.** §2.2a records that a manual re-fire cannot recover a missed run (Kansas, Washington twice).) (Session 65 — **NE `trig_019qmwzbTMo9k83etPqjJV7z` (Wed 10:30Z) AND NV `trig_012WYj2vrKgD7oxuURp3ygCb` (Tue/Fri 15:10Z) ON ROUTINES**, both with a GitHub-5xx retry. **WASHINGTON'S RUNNER IS NOT STUCK**: its 10:10Z fetch hit a transient credential-service 503 and stopped per its prompt; both re-fires used `fire_trigger`, which mints a sourceless session that cannot push. The miss is entered in the new `config/probe_gaps_explained.csv` (never back-filled). The CMS Routine's 13:10Z run declined to commit on 2 suite failures and will show as a miss at 19:00Z. It is NOT explained. The suite passes at main's 13:10Z head, so the failures came from the data CMS served that run. **California goes RCJ_ONLY -> NEITHER** (its only clean candidates were state money); the exposed set is 20 -> 18. **LOUISIANA'S "20 hospital-setting awards, $6,285,515" ENTERS NO BUCKET** (§7 rule; queue row RESOLVED). **STAGE 2 RE-RUN**: 87 records QUARANTINED under the seven new registry rows (GA 42, TX 25, CA 8, DE 7, MI 5; SUBAWARD 73 -> 118); `change_status` reset by the same-pull re-run, delta preserved in `first_seen` and `b612701` — `docs/session65_ne_nv_routines_wa_runner_la_aggregate_stage2.md`) (Session 64 — **THE SPEC'S TIER 3 EXAMPLE WAS STATE MONEY.** §0.2's 'GA Dual Track Remote Critical Care, $900K to 4 rural hospitals' and §10.2's pass-through example were DCH's State Office of Rural Health grant series ('subject to the availability of appropriated funds', zero RHTP mentions, first solicited 2023-11-15); re-pointed by patch to Georgia's signed Workforce Retention Technology NOA and Nebraska's NHVN award. **Georgia's award file rests on no Dual Track figure: $0.** **SEVEN STATE PROGRAMMES REGISTERED** (CA Distressed Hospital Small Grant $25M, TX TNFP/WVAN/HOPES, DE Downtown Development, MI CVI, GA Dual Track): the §6.2 sweep goes 66 -> **111 rows in 8 states**; the overlap check gains a hand-read, amount-verified same-name exemption. **FIVE STATES RE-EXTRACTED**: NV 72 -> 155 named actions (still no amounts; hospital actions 27 -> 42 at $0), NE +13 Initiative 5.3 INTENTS ($5,549,692.25; +6 hospital rows / $1,825,002.28 on CMS records), MI 139 -> 145 ($101,318,437; six state agencies, hospital unchanged), LA leaves the negatives (RCCB 53 awards, 5 named, $0 to any bucket), SD +8 Rural Strong contracts inside the $31.5M round (+5 hospital rows / $716,800). `NAMED_HOSPITAL` 1,114 / $937,661,818.75 / 27 -> **1,140 / $940,203,621.03 / 28**; rural 207 / $177,456,356.11; survey 36/7/3/4. **INDIANA GROW PROBE ON A ROUTINE** (`trig_01NZAAixvNoKcZMBKAiCUCYD`, Fri 09:50Z); NE and NV probes written and UNCHANGED live, on no Routine; WA's 10:10Z firing hit a GitHub 503 and was re-fired -- `docs/session64_dual_track_registry_five_reextractions.md`) (Session 63 — **EVERY DISPOSITION RE-READ AGAINST THE 09-24 PULL, AND A RULE THAT A DISPOSITION'S PROSE MAY NOT CONTRADICT ITS COUNTS** (`rhtp_assert_disposition_prose()`, called by every builder, tested over every committed file). **23 NEW DISPOSITION FILES** (`R/03bh`, 1,269 candidates). **INDIANA GROW EXTRACTED** (`R/03bi`): 186 recipients, NO amounts, 44 named-hospital rows at $0, `--probe` on no Routine yet. **AWARD FILES NOW KNOWN INCOMPLETE**: NV (155 vs 72), NE (5.3 intent, 13), MI (145 vs 139), LA (Credit Bank: 53 awards, 20 hospital, only 5 named), SD (8 Rural Strong contracts on OpenSD), plus single rows in VT VA FL AK NC. **STATE MONEY IN RCJ'S RHTP SET**: CA Distressed Hospital Small Grants ($25M, 4 hospitals), TX Nurse-Family Partnership / Workplace Violence / HOPES, GA Dual Track (the spec §0.2 example), DE Downtown Development rebates, MI CVI -- `docs/session63_dispositions_prose_rule_indiana_grow.md`) (Session 61 — **NEW JERSEY EXTRACTED: 103 priced awards, $83,060,837, 35 named-hospital rows / $35,275,076** (30 on EXACT CMS NJ Hospital Enrollment records at MEDIUM; Virtua Health Inc. the one LOW bridge, $3,150,255, queued `NJ_VIRTUA_PARENT_BRIDGE`). **AtlantiCare Health Services, Inc. is the FQHC, not the hospital** (three CMS FQHC CCNs; Gifford's shape), which is why it is 35 and not session 60's ~37. Bracketed SITES (AHS [Hackettstown], Inspira [Elmer] ...) go in `site`, never the awardee (§0.3a corollary). 35 rows / $11,964,249 on §8's fallback, one-directional, queued `NJ_RECIPIENT_FORM_NOT_STATED`. `NAMED_HOSPITAL` 1,035 / $902,386,742.75 / 25 -> **1,070 / $937,661,818.75 / 26**; split 34/8/4/4 -> **35/8/3/4**. **INDIANA'S GROW REGIONAL GRANTS HAVE ALL AWARDED**: eight Governor releases (2026-09-03), region figures summing to $112.4M plus a $16,249,593.22 surplus, and the GROW page names 186 recipient rows (178 names, ~30-40 hospitals) with NO per-organisation amount -- archived under `data/evidence/recheck/2026-09-24/IN/`, NOT extracted. **SEVEN ROUTINES** (DE ID OH SD TX NJ NEWSROOM), each on its own pushing runner; NJ and NEWSROOM gate on this branch reaching main. **`R/03bg_newsroom_sweep.R`**: 21 newsroom indexes in 14 of the 18 no-release states, tripping on a NEW headline pairing RHTP and award language (New Jersey's own ICYMI headline is the positive control); MA MD NH IL recorded UNREADABLE and re-tested every run. **Stage 2 on the 09-24 pull was NOT run** -- `R/02c` reads the stage-2 table, so the order is Stage 2 then 02c then each failing disposition -- `docs/session61_nj_extraction_indiana_regions_routines_newsroom_sweep.md`) (Session 60 — **NEW JERSEY PUBLISHED A 103-ROW PRICED ROSTER ON 2026-07-31 ($83,060,837, six sections, ~37 hospital rows) AND NOTHING HERE SAW IT FOR 55 DAYS** — no CMS release, no probe, not linked from the programme page, and RCJ holds 11 of the 103 even after a refresh. Archived under `data/evidence/recheck/2026-09-24/NJ/`, NOT extracted (the parse there is DERIVED). MN is a NEGATIVE whose 94 eligible hospitals ARE published (a uniform $1.4M cap, §0.3; RCJ's 10 Tribal rows are a Tier 2 cap table); MA is UNREADABLE (mass.gov 403 on four agents). **THE EXPOSED SET**: `R/01b_rcj_pull_diff.R` counts **27 states / $5.26B on no Routine**, 18 of them with no CMS release either (Tennessee's shape); DE ID OH SD TX have probes nothing schedules. **RCJ RE-PULLED 2026-09-24, 86 calls, quota 2,000 → 1,914**: awards 1,429 → 2,534. Leads (not findings): INDIANA's Regional Grants have begun awarding ($15.2M East-Central, $13M Southeast, Governor releases); NEBRASKA Initiative 5.3 intent ($5.46M); LOUISIANA 'RHTP Updates September 2026'; HAWAII HPCA $17M; and the FIRST TIER 3 §0.1 mode-6 misfile — Oregon's Wallowa Memorial filed under NM. **Stage 2 was NOT re-run** (disposition builders re-derive counts from the record table and must fail when they move; run R/02c first). **VIRGINIA** (11 named first-tier partners, no amounts; VHHA Foundation Unclear) and **WASHINGTON** (8 first-tier lines, $67,020,000 named; WSHA's $42M `Unclear`, queued `WA_WSHA_FLOW`) EXTRACTED; split 34/8/4/4, QUEUED = AZ MT RI UT. No bucket moved — `docs/session60_exposed_set_rcj_refresh_va_wa.md`) (Session 59 — TENNESSEE EXTRACTED, WASHINGTON'S FOOTER PARSED, FIVE STATES ON ROUTINES, NUVITA RE-TYPED. `R/03ay`: TDH's 2026-09-03 HART roster, 53 named awards in 44 counties and NO AMOUNT on any row (Nevada's shape). Coding the RECIPIENT and not TDH's "31 governments / 22 nonprofits" label (§0.3a) gives TWO hospital rows at $0: Macon Hospital, Inc (a HAND-READ BRIDGE to CMS's Macon County General Hospital Inc, CCN 441305, a CAH; LOW) and Cookeville Regional Medical Center Foundation (§10.2 foundation row, parent CCN 440059; LOW). The other 51 are typed from TDH's own 31/22 split, which the file reproduces exactly. `NAMED_HOSPITAL` 1,033 / $902,386,742.75 / 24 -> **1,035 / $902,386,742.75 / 25**. RAMP ($104.4M TennCare Shared Savings, STATE money, with an "RHTP Competitive Grant Extension" goal) is the §0.1 control. Tennessee escaped both discovery layers because RCJ's only pull (08-27) predates the roster, CMS issued no Tennessee release, `INVESTIGATED_NO_PROBE` meant nobody watched, and the programme page never linked the workbook. `rhtp_footer_parse()` learns the `SUBAWARD_OF` form (inert on all 224 existing footers; the subaward's figures go to `subaward_*`, never `cms_*`). CO `trig_01P9oihsxYj8Ksv5hSmKy6a4`, VA `trig_01QU6u3VFCJY1xWCrjmFeLcp`, ND `trig_01DUE8JsjXhHGDndT2N26Phf`, WA `trig_01XkZWESmQbqfq58cKuQ84i9`, TN `trig_01Ad384uvudquMnQXVHokXTX`, each on its own pushing runner and GATED on its probe reaching main; Virginia's RHT host serves an incomplete chain, repaired with the DigiCert intermediate in `config/certs/` (verification never off). CO and ND -> `INVESTIGATED_NO_LIST`; VA and WA stay QUEUED (unextracted first-tier work). Nuvita Health VENDOR_OR_CONTRACTOR -> OTHER on the owner's instruction, $0 moved — `docs/session59_tennessee_wa_footer_five_routines_nuvita.md`) (Session 58 — FLORIDA'S FIVE UNCLEAR ROWS ARE SETTLED AND ITS CEILING IS ITS FLOOR, 26.2%. On the owner's instruction `R/03ax --apply` wrote session 57's three codings onto `fl_year1_awardees.csv` rows 9/16/18/33/71, all `distributed_to_hospital = No`: North Florida Rural Health Corp `NONPROFIT_CBO` (IRS 85-2728333, ORG_WEBSITE/MEDIUM); Empowerq Health Care `NONPROFIT_CBO` on the hand-read bridge to Empower Healthcare, Inc. (IRS 85-2591676, GENERAL_KNOWLEDGE/LOW); Nuvita Health `VENDOR_OR_CONTRACTOR`, OWNER-CONFIRMED as Nuvita Cellular Health, GENERAL_KNOWLEDGE/LOW. $6,331,219.97 left the ceiling and no hospital dollar moved. The overlay runs AFTER session 49's (rebuild order: 03e --ingest, 03ap overlay, 03ax --apply). `ga_mixed_pool_split_search.csv` now records that GEORGIA'S TRUE SHARE IS STRICTLY BELOW 57.0%: each mixed pool holds a positive, unpublished non-hospital award — `docs/session58_fl_codings_ga_strict_ceiling.md`) (Session 57 — A REPORT, NOTHING RE-CODED. **FLORIDA'S CEILING**: the five Unclear rows ($6,331,219.97) are three organisations and none is a hospital in any CMS Florida enrolment file. North Florida Rural Health Corp resolves on an EXACT IRS record (EIN 85-2728333, 501(c)(3) clinic); Empowerq Health Care resolves on a hand-read BRIDGE to Empower Healthcare, Inc., Pahokee (IRS 85-2591676), LOW; Nuvita Health has NO federal record at all and resolves only on absence. Floor stays 26.2%; ceiling 29.6% -> 29.2% / 27.5% / 26.2% by reading, proposals in `fl_ceiling_resolution.csv` awaiting the owner. **GEORGIA'S POOLS**: no per-hospital split exists for the $6.5M Rural Stabilization or $15.635M AHEAD pool; each holds one non-hospital member (DBHDD; the unnamed provider of the 87 readiness assessments) whose amount is unpublished, so the 57.0% ceiling cannot be lowered by a sourced figure. **GAPS**: FL $21,736,938.39 = $20,993,819.50 of planned admin (AHCA's own narrative, internally inconsistent by $501,930) + $743,118.89 UNEXPLAINED; GA $21,713,842.63 = $2,605,473.80 state costs on CMS's image-only revised NOA + $19,108,368.83 of contractual budget in no published pool, UNEXPLAINED as to composition — `docs/session57_fl_ceiling_ga_pools_allotment_gaps.md`) (Session 56 — WHICH STATES HAVE FINISHED YEAR 1 AWARDING. `R/03aw` gives every one of the 31 extracted states a `year1_status` read off a SOURCE, never a percentage: **COMPLETE 2 (FL, GA), PARTIAL 28, UNKNOWN 1 (SD)**. Florida: *"AHCA has awarded the full scope of Florida's RHTP funding"*; Georgia: Phase 4 *"complete[s] the initial Year 1 award cycle"*, and CMS says the same. MICHIGAN'S "all RHTP Subrecipients" is a ROSTER claim, not a round claim; the same page advertises additional GFOs. Hospital share in the COMPLETE states: **FL 26.2% (ceiling 29.6%), GA 45.8% (ceiling 57.0%)**. Georgia is bounded, not exact, because two pools name hospitals with no split ($22,135,000) — `docs/session56_year1_completion_status.md`) (Session 55 — EVERY SCHEDULED ROUTINE NOW PUSHES. MO (`3d740ff`) and CA (`d37ef9c`) v2 firings published by schedule; the 16 v1 Routines are DELETED. The CMS trigger list moved to runner `trig_0174R4VLbhacyCKCKds8qYxM` (Mon/Thu 13:00Z) and its `--run` now LOGS a verdict (`rhtp_cms_press_verdict()`); WV got its first `--probe` (news index trips on a new "award" release; RHTP + grants pages name-diffed) and WV (`trig_016jUYvhEgJEvMpEoZChp8AK`, Fri 11:30Z) and VT (`trig_011gjpEvcXAi5XnXgzo6STJy`, Tue 11:30Z) are on Routines — nineteen in `config/routines.csv`. The eight QUEUED states (AZ CO MT ND RI UT VA WA) were read and NONE has a named roster; CO and VA date awards to 2026-09-30. TENNESSEE HAS ONE: 53 named HART awards, no amounts (2026-09-03), archived NOT extracted — its `INVESTIGATED_NO_PROBE` status is stale. WASHINGTON's 80%-federal subaward footer returns ZERO rows from `rhtp_footer_parse()` — recorded, not patched — `docs/session55_routines_cms_wv_vt_eight_state_check.md`) (Session 54 — ROUTINES RE-BOUND TO SESSIONS THAT PUSH; `config/routines.csv` carries the 16 v2 ids, `logging_since` 2026-09-23T14:18Z. VERMONT (111 executed agreements, $84,540,011.33 + the $2,635,000 GMCB MOU kept out; 27 hospital rows / $22,641,819.43, incl. Mary Hitchcock NH $8,820,815.60 and UVM Health Network $2,474,684.82 at LOW; CMS enrolment files caught "1248 Hospital Drive Opco LLC" (a NURSING HOME) and Gifford Health Care (the FQHC, not the hospital)), CONNECTICUT (4 agreements, $49.98M rounded; Day Kimball + Sharon $33.35M NAMED; Hartford HealthCare's Windham + Charlotte Hungerford $12.65M ONE UNSPLIT row in `POOL_NAMED_HOSPITALS`), WEST VIRGINIA (7 awards, $6,444,803; CAMC + Cabell Huntington Foundation $1,224,000) and MISSOURI's 20 SMRP hospitals ($0, no amounts published) EXTRACTED. `NAMED_HOSPITAL` 982 / $845,170,923.32 / 20 -> **1,033 / $902,386,742.75 / 24**; `POOL_NAMED_HOSPITALS` +$12,650,000 (CT). LA probe scoped past LDH's mega-menu — `docs/session54_routine_cutover_vt_ct_wv_mo_extraction.md`) (Session 53 — SELF REGIONAL'S GREENWOOD PEDIATRICS ROW RE-TYPED UNDER §0.3a (+1 row, +$145,000; `NAMED_HOSPITAL` 982 / $845,170,923.32; SC 114 / $115,985,714.95, 50.0% of awards / 69.3% of dollars); MISSOURI (20 named, no amounts) AND CONNECTICUT ($46M to 4 named systems, one pair unsplit) ROSTERS FOUND; VERMONT has 112 executed agreements / $87.2M, WEST VIRGINIA 7 named awards / $6.4M, RHODE ISLAND unreadable — all archived under `data/evidence/recheck/2026-09-23/`, NONE extracted; LA probe tripwire is menu furniture; coverage check flags WY 9/22 and will flag NY/LA 9/23 — `docs/session53_self_regional_mo_ct_rosters_vt_wv_ri.md`) (Session 52 — ROUTINES CANNOT PUBLISH, AND NOW A CHECK SAYS SO; SEVEN PROBES RETUNED; NEW YORK AND KANSAS EXTRACTED; SOUTH CAROLINA BOUNDED. **TASK 1**: all 58 lines committed to `logs/probe_results.csv` before this session came from INTERACTIVE sessions, and not one Routine firing reached it. The fresh-session triggers carry `sources: []` and `outcomes: []`, so they have no repository to push to, and a test firing of New York's rewritten prompt produced no commit anywhere. The log gains an `origin` column, because an interactive run was MASKING a lost Routine firing (Alaska 9/21). `config/routines.csv` registers all 16 Routines, and `R/probe_coverage.R` fails by name on any firing with no line from its own Routine. **Re-binding the Routines to a session with a repo and an outcome branch is the owner's decision, and it is open.** **TASK 2**: `furniture` (EXACT-match, unlike `known`), `rhtp_name_scope()` (anchored content region; refuses a missing anchor) and a `CLOSED` break token. All seven probes pass live, and a test proves each still fires on a real recipient. **TASK 3**: NEW YORK, 56 lead rows / $76,190,022.00; 35 hospital-lead rows / $47,358,790.79 (of which $13,961,712 are LOW bridges); 21 non-hospital leads Unclear (§7's third class). **TASK 4**: the PDF reader did not implement `'` or skip `%` comments, so KDHE's roster read as EMPTY. Fixed, and proved inert on 77 of 78 committed PDFs (a correction on the 78th). KANSAS Emerging Technology, 14 / $16,006,648; 7 hospitals / $10,176,973; Attica held out. Kansas probe + Routine `trig_01JkvfXqqNaLXuf8xFja5nNR`. **TASK 5**: no testable reading brings SC to ~60% (psych out 64.7%; plus non-hospital sites 63.1%; grain moves counts, not dollars); only rural-only counting could, and it is untestable here. `NAMED_HOSPITAL` 981 rows / $845,025,923.32 / 20 states — `docs/session52_probe_log_retune_ny_ks_sc_bounds.md`) (Session 51 — A BUCKET MUST NOT MIX TIERS, TWO OF FOUR REFUSALS SETTLED, THE RURAL CUT REPORTED, AND EVERY PROBE RUN LIVE. **TASK 1**: Iowa's $50,000,000 Centers of Excellence pool row is REMOVED from `POOL_NAMED_HOSPITALS`. It was a Tier 2 footer sitting beside Nebraska's Tier 3 award, and a bucket's figure is the sum of its rows. The bucket goes 2 / $68,156,856.12 -> 1 / $18,156,856.12. The ten CoE award actions are counted once, in `NAMED_HOSPITAL` at $0. The figure is context in `ia_notice_footers.csv`. The partition now refuses any priced `AMOUNT_IS_POOL_NOT_AWARD` row, and spec §0.2 gains a third worked example. **TASK 2**: the four refusals were two MS and two SC, not four SC. The two SC ones are settled from ARCHIVED federal records: Community Initiatives Inc. is a 501(c)(3) (IRS EIN 31-1741660, NPPES, both Greenwood) -> `NONPROFIT_CBO`; Graceful Health Solutions, LLC is an NPPES community-health clinic in no CMS enrolment file -> `OTHER`. $0 moved, and South Carolina has no fallback row left. Delta Health Transformation Council ($3,000,000) and CAMHP Foundation stay refused, now as measured negatives across six CMS files, NPPES and the BMF. **TASK 3**: `NAMED_HOSPITAL` is 939 rows / **$787,490,159.53** (session 50 wrote .80). 151 rows / $157,997,116.60 carry a source-backed rural designation; 692 rows / $562,739,209.13 in 18 states carry none. **TASK 4**: CMS 21 -> 23 states (NM, MO). Eight name-tripwire halts, of which only NEW YORK is a roster: 56 lead applicants, $76,190,022, public since 2026-09-04 and missed by its Routine. KANSAS published 14 Emerging Technology awards, $16,006,648, in a PDF the reader cannot read. MO and CT announced hospital awards by count only — `docs/session51_tier_bucket_refusals_rural_cut_live_probes.md`) (Session 50 — THE UNSTATED-FORM QUESTION IS ANSWERED FOR THE TWO STATES THE WORKBOOK NEVER REACHED, AND IOWA'S CENTERS OF EXCELLENCE POOL IS RECORDED WITH ITS TIER ON IT. **TASK 1 — `VQ_INKIND_AFTER_RETYPE` RESOLVES TO OPTION (a), AND THE DISTINCTION SESSION 49 CALLED "REAL AND ALSO THIN" TURNS OUT TO BE IN THE GRAMMAR OF THE SOURCE.** All four KDHE and MDH award paragraphs were read. On the three that moved, the AWARDEE IS THE SUBJECT OF THE SENTENCE and the money funds its own work — *"**Stormont Vail HealthCare proposes** PrairieLINK"*, *"**Greeley County Health Services will develop** the Western Kansas Shared Services Group"*, and MDH heads Meritus Health Center's column **"Lead Organization"** against a precision medicine clinic, five primary-care recruitments and its own All-In-Health model. **SALINA REGIONAL'S PARAGRAPH IS THE ONLY ONE OF THE FOUR IN WHICH THE AWARDEE IS NOT THE SUBJECT**: *"**Five founding providers will form AstraHealth Kansas**, a shared services organization providing ... infrastructure **to** rural hospitals across Kansas."* The money capitalises a NEW, SEPARATE organisation of which Salina is one of five founders, and what reaches rural hospitals is a SERVICE — session 31's money-movement test, now resting on the source's own grammar rather than on session 31's authority. $10,591,281 stays in, $932,310 stays out, **net $0**. **TASK 2 — WINSTON COUNTY MEDICAL FOUNDATION IS A HOSPITAL, AND THE RULE IT NEEDED WAS NOT §10.2's.** CMS Hospital Enrollments carries `ORGANIZATION NAME` = **WINSTON COUNTY MEDICAL FOUNDATION**, Louisville MS, against **CCN 250027**; the IRS Business Master File gives the same body as **"Winston County Medical Foundation Dba Winston Medical Center"** with its Form 990 checking *"operated one or more hospital facilities"* (Schedule H); CMS's provider dataset lists Winston Medical Center at 17550 East Main Street against the foundation's 17570, the same campus. **SO THE FOUNDATION IS NOT THE ARM OF A HOSPITAL — IT IS THE HOSPITAL'S OWN LEGAL NAME**, §10.2's NAMED-parent test is never reached, and the answer is ordinary §8 typing. FOUR ROWS, **+$3,785,455.49**, including the row that carried `IN_KIND_BENEFIT`, whose flow re-ran to `DIRECT`. **TASK 3 — IOWA'S CENTERS OF EXCELLENCE POOL IS IN `POOL_NAMED_HOSPITALS`, AND THE TENSION IS STATED RATHER THAN BURIED.** PHTHORC26008 holds ten award actions, all ten recipients are hospitals (session 49 verified the two on the fallback), and Iowa publishes NO per-hospital split — that code's condition word for word. **BUT THE $50,000,000 IS TIER 2**: it is the notice's own CMS footer, recorded as `SOLICITATION` in `ia_notice_footers.csv`, the pool the RFP was advertised at in the notice's own word *"approximately"*, and not an award total. Nebraska's $18,156,856 in the same bucket is a Tier 3 award amount, so **THE BUCKET NOW HOLDS TWO FIGURES OF DIFFERENT TIERS** — and the ten award actions are STILL IN `NAMED_HOSPITAL` AT $0, so a reader adding the ROW COUNTS counts the Centers of Excellence twice. The buckets are never added (`rhtp_hospital_total()` refuses), the awardee string says **POOL ROW** in capitals, and it is queued as `IA_COE_POOL_IS_TIER_2` with all three options priced. **`AMOUNT_IS_POOL_NOT_AWARD` was added to §8** on sessions 12/17/19/26/39/49's footing, and `ia_assert_no_amount_column_effect()` was NARROWED rather than removed: it now asks whether `amount` is populated on any row that is NOT the pool row, so "Iowa prices nobody" is still asserted on all 264 award actions. **TASK 4 — 246 ROWS TYPED DIRECTLY, AND THE EVIDENCE IS MOSTLY A FEDERAL RECORD.** Mississippi (96 rows / $56,022,017.62) and South Carolina (150 / $92,759,791.23) were the two largest open form questions left here and the two states `verification_queue_2.xlsx` never reached. **169 of the 173 ORGANISATIONS ARE TYPED AND FOUR ARE REFUSED**, with `basis_type` on every row. **+74 ROWS AND +$80,696,969.67 INTO `NAMED_HOSPITAL`, AND NOTHING OUT** — one-directional by construction, because every open row was already `No`, so both states' floors were genuine floors. **WHERE SESSION 49's QUEUE PASS WAS 306 GENERAL-KNOWLEDGE ANSWERS OF 399, THIS ONE IS THE OTHER WAY ROUND**: CMS's Hospital, FQHC and RHC Enrollment files carry the state's own awardee string as an `ORGANIZATION NAME` or DBA on **70 of the 74 rows that move, worth $71,354,639.67** at `ORG_WEBSITE`/`MEDIUM`. **ONLY $9,342,330 RESTS ON A HAND-READ BRIDGE**, and all four are named so a reader can subtract them — Ochsner MS ($3,000,000, a shorthand no federal record carries), Webster Healthcare Services ($500,000, ONE WORD from CMS's *Webster Health Services Inc*, decided on the award's county), Southwest Mississippi Regional Center ($277,077, which has **a real rival** in the Department of Mental Health's regional centre and is decided on PIKE vs LINCOLN county) and Acadia. **SOUTH CAROLINA BECOMES THE LARGEST NAMED-HOSPITAL STATE IN THIS REPOSITORY**, ahead of Arkansas: 55 rows / $56,587,138 -> **113 / $115,840,715**. Mississippi goes 68 / $47,454,812 -> **84 / $68,898,205**, fifth. **NO STATE'S PUBLISHED TOTAL MOVED** — MS still sums to $104,115,146.80 and SC to $167,299,900.69; this pass re-TYPES and re-prices nothing. **SOUTH CAROLINA'S NINETEEN SCDBHDD ROWS ARE TYPED `STATE_AGENCY`** — session 47 left them alone because §8's name rule does not reach an acronym, and typing them moves **$0**. **MISSISSIPPI'S FIVE REGIONAL COMMUNITY MENTAL HEALTH CENTRES ARE `OTHER`** with the form stated, because a CMHC is neither a hospital nor the awarding agency. **TWO STEM TRAPS ARE REFUSED BY NAME**: *Singing River Services* is the Region 14 CMHC and not **Singing River Health System** (a different county), and *George Regional Health and Rehab* is a nursing facility, not **George Regional Hospital**, which is not an awardee here at all. **ACADIA HEALTHCARE CO. IS TYPED AND QUEUED IN THE SAME BREATH** ($5,565,253, `SC_ACADIA_PARENT_SCOPE`): CMS carries `HHC SOUTH CAROLINA INC` dba **Lighthouse Behavioral Health Hospital**, Conway, CCN 424002, so Acadia is a hospital operator — but the awardee is the CORPORATE PARENT whose South Carolina estate also holds two outpatient opioid treatment centres, both separately awarded here and both typed `OTHER`, and no source publishes a split. **FOUR ORGANISATIONS ARE REFUSED AND KEEP THE FALLBACK AND ITS FLAG**, the largest being **Delta Health Transformation Council, Inc. ($3,000,000)**, which no federal or IRS record carries and whose stem matches BOTH the Greenville hospital AND the FQHC — a stem match could type it either way and neither would be a determination (§2). Queued as `UF_FORM_NOT_DETERMINABLE`, 5 rows / $3,999,320. **AND THE AGENCY BENCHMARK'S TWO HALVES NOW DISAGREE, WHICH SESSION 47 COULD NOT SEE.** Against *~50% of awards, ~60% of dollars*: **% OF AWARDS LANDS ALMOST EXACTLY — 49.6%** where session 47 could only bracket 24.1%–89.9%; **% OF DOLLARS OVERSHOOTS BY NINE POINTS — 69.2%**, 65.9% with Acadia subtracted. Three readings are available (the benchmark may count only RURAL hospitals; it may exclude the psychiatric hospitals and Acadia, 4.5 points between them; it may treat Prisma's and Self Regional's non-hospital sites separately) and **NONE IS ADOPTED, because adopting one would be adjusting a classification toward a figure**. NO ROW WAS TYPED TO REACH THE BENCHMARK AND NONE ADJUSTED AWAY FROM IT, and a test fails if a later session quietly does. **THE TYPING IS AN OVERLAY AND ITS PLAN IS DERIVED FROM THE BUILDERS, NOT FROM THE COMMITTED CSVs** — once `--apply` has run the committed file no longer carries the fallback flag, so a plan read from it would produce an EMPTY plan that reads exactly like "there was never anything to do". **TWO ROUND-TRIP DEFECTS WERE CAUGHT IN THE DIFF AND NOT IN THE CONTENT**: both files spell "empty" as the literal string `NA`, so a plain read-then-write rewrote 167 and 228 rows in six untouched columns; and `as.character()` on a double renders 3000000 as `3e+06`, which rewrote the amount column on 28 more. Sessions 23–25's lesson a sixth time — read the DIFF, not the file — `docs/session50_unstated_form_typing.md`) (Session 49 — THE RETURNED VERIFICATION QUEUE IS INGESTED, AND THREE POLICY CHANGES COME WITH IT. **ALL 399 ROWS OF `verification_queue_2.xlsx` CAME BACK ANSWERED** — 500 committed award rows, 382 organisations, 31 questions, 21 states — and the workbook is READ-ONLY, pinned by SHA-256. **`NAMED_HOSPITAL` GOES 747 ROWS / $599,533,409 / 18 STATES -> 865 / $706,793,190 / 19**: +118 rows and **+$107,259,781.21 NET** (120 in at $110,294,573.21, TWO OUT at $3,034,792). **NEITHER POOL BUCKET MOVED AT ALL**, because nothing here touched a pass-through award — every `_FLOW` question is still open. **ARKANSAS IS NOW THE LARGEST NAMED-HOSPITAL STATE IN THIS REPOSITORY**, ahead of Georgia for the first time (9 rows / $21.8M -> 18 / $92.4M: Baxter Health $19.7M, Mercy Fort Smith $19.1M, Baptist Health $16.9M, St. Bernards $14.6M), and its queue row had stated a $122,516,381 CEILING — so **about $30M of that spread resolved DOWNWARD**, which is the ceiling doing its job. **NORTH CAROLINA ENTERS THE PARTITION AT ALL** for ONE row and $0 (Cape Fear Valley Mobile Integrated Health, option (c) of the three its queue row offered), which is what takes the state count 18 -> 19. **AND SEVENTY-TWO OF THE 120 ROWS THAT MOVED IN CARRY NO AMOUNT** — Iowa 64, Nevada 7, North Carolina 1 — so **IOWA ALONE GAINS 64 NAMED-HOSPITAL AWARD ACTIONS AND NOT ONE DOLLAR** and a reader working down the dollar column sees none of it (Nevada's rule, session 26). **TASK 1 — §8 GAINS `OTHER`**, on session 39's `MANAGED_CARE_ORGANIZATION` footing and for that same condition: THE SOURCE STATES A FORM §8 DOES NOT CARRY. Thirty-five answers are in it — a retail pharmacy, a PACE organisation, a non-emergency medical transport company, a workforce investment board, a nursing home, a midwifery centre, a hospice's foundation, a mobile-imaging company. **It is NOT the standing fallback** (which says the form is UNDETERMINED, and using it for a determined form asserts an ignorance the record no longer has — the one thing `RECIPIENT_TYPE_INFERRED`'s own note forbids), **not `VENDOR_OR_CONTRACTOR`**, and **NEVER A HOSPITAL TYPE**, so like the MCO code it can only keep dollars OUT. **A row carrying it must state the determined form in `determination_basis`**, asserted on every row: `OTHER` with nothing behind it is the fallback wearing a different name. **TASK 2 — §10.2 GAINS THE HOSPITAL-FOUNDATION ROW, APPLIED ACROSS EVERY COMMITTED STATE FILE AND NOT ONLY THE QUEUE.** A foundation or affiliated arm of a **NAMED** hospital or health system is `HOSPITAL_OR_SYSTEM` and takes §10.2's ordinary DIRECT row: seven organisations, TEN award rows, **$17,853,500** — St. Bernards Development Foundation (AR, $14,551,090), Citizen's Foundation (Citizens Health) (KS, $2,832,423), Sky Lakes Foundation (OR, $469,987), MercyOne Genesis and MercyOne North Iowa Foundation (IA, 5 rows, UNPRICED) and Incline Village Community Hospital Foundation (NV, UNPRICED). **THE LOAD-BEARING WORD IS *NAMED*, AND IT IS MEASURED: `\bfoundation\b` MATCHES 57 ROWS ACROSS THE COMMITTED FILES AND ONLY TEN ARE THE ARM OF A NAMED HOSPITAL.** It deliberately does NOT reach the foundation of an ASSOCIATION of hospitals — **New Hampshire's Foundation for Healthy Communities, whose eligible class is hospitals AMONG OTHERS (§0.3) and whose flow is settled; reading it as a hospital's foundation would move $66,547,394 on nothing at all**, and a test drives that counterfactual — nor Nevada Rural Hospital Partners Foundation, nor a foundation whose PARENT is not a hospital (Talbot Hospice, Cahaba Medical Care (an FQHC), Superior Health (a conversion grantmaker), Gottsche, MPhA). **TWO REFUSALS ARE WORTH READING BECAUSE THE RULE COULD HAVE REACHED THEM. KANSAS PUBLISHES TWO CITIZENS SPELLINGS AND THEY NOW CLASSIFY DIFFERENTLY**: the one that NAMES its parent is a hospital, the one that does not stays `NONPROFIT_CBO` — the verifier said so (*"MOST LIKELY ... NOT CONFIRMED"*) — **$146,476 apart**, which is North Carolina's two spellings of UNC in a state that PRICES its rows. And **MISSISSIPPI'S WINSTON COUNTY MEDICAL FOUNDATION ($3,785,455, 4 rows) WAS NOT PROMOTED**: Mississippi is not in the workbook, nobody verified it, and its name states a COUNTY. Both queued. **TASK 3 — GENERAL KNOWLEDGE BECOMES AN ADMISSIBLE BASIS, AND `basis_type` IS WHAT MAKES IT HONEST.** 306 of the 399 answers are `GENERAL_KNOWLEDGE`, 80 `ORG_WEBSITE`, **13 `STATE_SOURCE`** — and **NOT ONE OF THE 111 HOSPITAL ANSWERS RESTS ON A STATE SOURCE**, which is not a weakness in the answers but the shape of the question: a state's award document names a recipient and an amount and does not say what KIND of organisation it is, which is exactly why these rows sat on §8's fallback for thirty sessions. Read against an unamended §0.4, **306 of 399 answers are violations** and the eight-state form question stays open forever. **THE AWARD'S OWN URL DOES NOT BUY `STATE_SOURCE`, AND THAT IS THE DESIGN**: 238 of the 399 bases carry a URL whose host IS the row's state source, and on nearly every one that URL locates the AWARD and says nothing about the FORM — the workbook says so itself on 143 rows (*"[Org type from general knowledge; URL is the award source.]"*). **THE CLASS IS CARRIED IN THE CONFIDENCE, NOT HIDDEN BY IT**: `GENERAL_KNOWLEDGE` -> LOW, the other two -> MEDIUM, **HIGH still requires a CCN match so NOTHING in this pass reaches it**, and a reader can therefore SUBTRACT — of the $110,294,573 that moved in, **$83,692,269 rests on `ORG_WEBSITE` and $26,602,304 on `GENERAL_KNOWLEDGE`**. **THE TWO HUMAN VERIFIERS ANSWERED 62 ROWS AND THOSE 62 CARRY $80.4M OF THE $110.3M** — they took the large priced names and 30 of MPB's 31 answers are `ORG_WEBSITE`; the 337 machine-assisted rows are mostly the unpriced long tail. **THE GUARDRAILS, AND WHAT EACH COST. MISSOURI'S HUB ANCHORS AND MAINE'S INVITED COHORT DID NOT ENTER ANY BUCKET AND STRUCTURALLY COULD NOT** — both files were re-typed (Missouri's roster goes **14 -> 19 hospitals of 27**, Maine's stays 11 of 11) and both carry **NO `amount`, NO `flow_type` and NO `distributed_to_hospital`**, are not in `STATE_FILES`, and cannot reach the partition at all; a test drives all three absences. **ANSWERING A `_FLOW` ROW DID NOT SETTLE ITS FLOW**: all four came back with a TYPE and all four flows are unchanged — MHA's $8,625,000 still `PASS_THROUGH_UNRESOLVED`/Unclear, ARHP's $18,833,521 still `FLOW_UNRESOLVED_HOSPITAL_AFFILIATED` in NEITHER bucket. **THE FLOW TEST WAS RE-RUN ON EVERY RE-TYPED ROW AND ONLY WHERE THE RE-TYPE CAN CHANGE ITS ANSWER** — §10.2's DIRECT row is the one branch keyed on recipient IDENTITY, so flow is re-derived exactly where a row crosses the `HOSPITAL_OR_SYSTEM` boundary (122 rows) and left alone elsewhere. **AND FOUR RE-TYPED ROWS CARRIED `IN_KIND_BENEFIT` WITH THE SAME GENERATED BASIS SENTENCE — *"the recipient is not a hospital and keeps the funds"* — WHOSE FIRST HALF IS EXACTLY WHAT THE VERIFICATION OVERTURNS.** Three were re-run to DIRECT (Stormont Vail $5,465,969, Greeley County $1,541,906, Meritus $3,583,406) and **SALINA REGIONAL HEALTH CENTER ($932,310) WAS HELD**, because session 31 read it INDIVIDUALLY — tightening the pass-through marker into a money-movement test moved exactly TWO committed rows in the whole repository and Salina was one of them. **THE DISTINCTION IS REAL AND IT IS ALSO THIN, AND SAYING SO IS THE POINT**: four rows share one sentence and now have two answers, so it is queued as `VQ_INKIND_AFTER_RETYPE` ($10,591,281 against $932,310 held) with all three options priced. **IOWA: SIOUX CENTER HEALTH IS A HOSPITAL, AND THE $50,000,000 CENTERS OF EXCELLENCE POOL IS NOW HOSPITAL-ONLY.** PHTHORC26008 holds TEN award actions; eight were already hospitals and the two on the fallback — **Sioux Center Health** and **Manning Regional Health Care Center** — both verify as critical access hospitals. **WHAT THAT IS AND IS NOT**: it makes the RECIPIENT SET hospital-only, a statement about WHO, and attaches NO dollar to anybody. Iowa prices nobody, the **$50,000,000 is a TIER 2 figure** from that notice's CMS footer in `ia_notice_footers.csv`, $50m/10 = $5m is nobody's published figure (§6.2) and adding it to a Tier 3 total is §0.2 — asserted, with `amount` still empty on all 264 Iowa rows. **THE ONE ROW NOBODY COULD ANSWER**: NVHA's `93 X 95 NV` came back `UNKNOWN`, which is not a §8 value, and **NOTHING WAS WRITTEN TO IT** — it keeps the fallback and its flag, UNKNOWN is recorded in the answers CSV and not in a state file, and it is queued. **THE REVIEW QUEUE GOES 20 ROWS -> 24, WITH THIRTEEN RESOLVED** — including `GHA_RECIPIENT_TYPE`, open since session 19 and answered `NONPROFIT_CBO`, so Georgia stops being the outlier of three at $0; **three resolved to the EXACT figure they had predicted** (Maryland's $3,034,792 downward exposure, Wyoming's $752,302 and $1,700,000). **`MS_RECIPIENT_FORM_NOT_STATED` IS NOW THE LARGEST OPEN FORM QUESTION IN THE REPOSITORY** at 96 rows / $56,022,017.62, Mississippi being absent from the workbook. **ONE DEFECT CAUGHT BY A TEST AND FIXED BEFORE IT WAS COMMITTED**: a first draft wrote this session's prose into `recipient_type_source`, which on FLORIDA holds the OWNER'S ORIGINAL §8 CODE — session 10's back-fit preserved *"UNCLASSIFIED"* there precisely so the change stays auditable — so the column is left alone entirely and the verification's provenance goes in `basis_type` / `verified_by` / `verified_basis`, which are this session's columns and nobody else's — `docs/session49_verification_ingest_policy_changes.md`) (Session 48 —  THE TRIPWIRES LEARN TO READ NAMES, AND SOUTH CAROLINA GETS THE PROBE IT WAS MISSING. **TASK 1 — A MARKER LIST CANNOT CONTAIN THE PHRASING A STATE HAS NOT USED YET, AND SESSION 46 PAID FOR IT.** HCA's page said *"HCA has **selected six Regional Hub Organizations** to lead Healthy Horizons"* and named Cibola General Hospital, the Regents of the University of New Mexico, Eastern Plains Council of Governments, **Gila Regional Medical Center** and **Nor-Lea Hospital District** — and every one of `NM_AWARD_POSTED`'s ten phrases was **driven against that sentence in a test and NOT ONE MATCHES**. `rhtp_assert_no_new_organisations()` (`R/utils_config.R`) asks the phrasing-independent question instead: **does the page NAME an organisation the committed ARCHIVE does not?** Run against the two REAL committed NM archives — session 35's and session 46's — **it names all six hubs**, which is the counterfactual as data rather than as a session note. **IT IS A SECOND SIGNAL AND THE PHRASE LISTS ALL STAY**, because they are what catch a state announcing awards while naming NOBODY (South Dakota's shape, South Carolina's email notices), which no name diff can see. **THE BASELINE IS THE ARCHIVE, NOT A CONSTANT, AND THAT IS WHAT MAKES CHROME CANCEL** — navigation, staff lists and the agency's own name are in both copies — so the suffix list can be BROAD where South Dakota's had to be NARROW: South Dakota counts names against a THRESHOLD, so a false positive spends part of its budget, while a diff has no budget to spend. **THE EMPTY-BASELINE GUARD CAUGHT A REAL ONE ON ITS FIRST RUN**: Missouri's `hub_roster` is a **PDF**, and handed to an HTML reader it yields 450 characters of PDF header — a diff against nothing **passes silently forever while wearing a green verdict**, which is a fact about our reading reported as one about the state (§0.4). Pointed at Missouri's own PDF reader the roster yields 30 names, so a **TWENTY-EIGHTH Hub Anchor** now fires. **ITS OWN TESTS FOUND TWO MORE DEFECTS, BOTH FALSE-POSITIVE GENERATORS**: a **trailing full stop** kept for comparison (to preserve *"Inc."*) meant the LAST name on a page read as new the moment a page gained a sentence after it; and a **ZERO-WIDTH CHARACTER** inside a name breaks the run, so *"Gila​Regional Medical Center"* comes out as *"Regional Medical Center"* — session 34's character, and normalising afterwards cannot repair it because the first word is already gone, so they are stripped BEFORE extraction. **A DATE IS NOT A NAME, AND LOUISIANA IS THE MEASUREMENT RATHER THAN THE GUESS**: LDH re-dated all seven windows between 2026-09-02 and 2026-09-21 **without naming anybody**, and against the committed prior snapshot that produced three "new organisations" whose only new words were the month — the reduction flattens a table cell boundary to a space. Breaking a run on a calendar token drops all three, and **the LA pair is a test**: the digest MOVED and the name tripwire is SILENT IN BOTH DIRECTIONS, which is the discrimination the whole thing is for. **WIRED INTO EIGHTEEN PROBES, SUBJECT PAGES ONLY** — a control or a press index moves for reasons that are not the state awarding, and halting on those is session 46's stale dated anchor in a new costume. **ALASKA AND SOUTH DAKOTA ARE RECORDED EXEMPTIONS** (Alaska's rolling xlsx is diffed by the extraction itself; South Dakota's portal already carries its own org-name tripwire), and **a test reads the source of every file in `R/` and requires any file offering `--probe` to run it**, so a twenty-first state file cannot quietly ship with only a phrase list. **ALL 39 WIRED PAGES ARE SILENT AGAINST TODAY'S ARCHIVES with baselines of 7 to 257 names.** **TASK 2 — SOUTH CAROLINA IS ON A ROUTINE, AND IT WAS THE ONLY EXTRACTED STATE WITH A PROBE AND NO SCHEDULE.** `trig_01UdVg36W1aWGTDKs3jfHtfV`, **THURSDAYS 08:30 UTC**, first firing 2026-09-24. **WEEKLY, AND THE REASON IS THE PROJECT'S OWN RULE**: twice-weekly is earned by a state that has PUBLISHED an award date (Wisconsin, Connecticut, Mississippi), and South Carolina publishes none — SCDHHS says only that the **Tech Catalyst Fund**'s opportunities *"will be announced through a future stage"*. That is North Carolina's and New Mexico's footing. **08:30 because the fifteen Routines already running occupy a continuous band from 09:40 to 21:10 UTC.** It watches the fifth of five initiatives, **$32,730,351** administered through the **South Carolina Research Authority** (§7's designated pass-through route), which has named nobody. The probe **ran live first and reports UNCHANGED on all four pages** — with the new name tripwire inside it, which is the wiring validated end to end against a live host. **TASK 3 — THE TOTALS, RE-DERIVED FROM THE COMMITTED FILES RATHER THAN CARRIED FORWARD.** `NAMED_HOSPITAL` is **747 rows / $599,533,408.65 / 18 states**; `POOL_NAMED_HOSPITALS` 1 row / $18,156,856.12 (NE); `POOL_UNNAMED_HOSPITALS` 1 row / $50,008,264.00 (IL). **ALASKA after the 244-action refresh is 43 named-hospital rows / $62,396,425.47**, across **16 distinct awardee strings** — fourth largest state contribution, behind Georgia, Wyoming and Alabama. **NO FIGURE MOVED AND NONE WAS RE-CODED**: this is the same arithmetic session 46 and 47 recorded, reproduced from the 28 committed files, and the two-decimal form is stated because `sum(amount)` carries cents that the rounded headline does not — `docs/session48_name_tripwire_sc_routine.md`) (Session 47 — SOUTH CAROLINA IS EXTRACTED: 228 AWARDS, $167,299,900.69, AND THE FIRST STATE TO LEAVE `INVESTIGATED_NO_PROBE`. **IT LEFT THE WAY THAT CODE'S OWN NOTE SAID IT WOULD — BY THE STATE PUBLISHING, NOT BY ANYONE WRITING THE MISSING PROBE.** Session 43 put South Carolina there deliberately, because `INVESTIGATED_NO_LIST` would have promised a re-checkable negative this repository did not have AND would have been false about the stage, since SCDHHS had already issued Notices of Award Determination on 2026-07-31 (MB# 26-026) and told the recipients **BY EMAIL**. So when SCDHHS posted *"SC RHTP Year 1 Award List"* on 2026-09-15, **NOTHING HAD TO BE RETRACTED**: South Carolina went `INVESTIGATED_NO_PROBE` -> `EXTRACTED` without ever passing through `INVESTIGATED_NO_LIST`. The disposition goes **26/8/6/10 -> 27/8/5/10**, both tables REBUILT from `R/03k`'s constants. **THE DOCUMENT PUBLISHES NO TOTAL OF ANY KIND** — no grand total, no subtotal, no `Total:` row — which is new here: Arkansas reconciled to its own `Total:` row and Mississippi to the Governor's stated figure, so **$167,299,900.69 IS THE SUM OF THE ROWS and reconciling to it proves nothing on its own.** What makes it a reconciliation is three facts from OUTSIDE the arithmetic: **CMS independently states *"The RHTP funding will support 228 GRANTS"***, its headline is **$167 Million**, and **every project numbers 1..n with no gap** — which is a completeness check a total cannot give. **BOTH MISSISSIPPI DEFECTS WERE LOOKED FOR AND BOTH HAVE AN ANALOGUE. (1) THE MISSING-CHARACTER DEFECT IS IN THE ROW NUMBER**: Healthcare Workforce row 2 is printed **`I2`**, a capital I where a digit belongs, so a parser keying on `^[0-9]+$` DROPS Rebound Behavioral Health and **$120,000**. The total would catch it — **which is exactly why this file does not depend on the total**: rows are anchored on the AMOUNT column and the numbers are the completeness check instead. Recorded, not corrected (§8). **(2) THE HYPHEN DEFECT IS THE INVERSE OF MISSISSIPPI'S AND COSTS A NAME RATHER THAN A DOLLAR.** South Carolina's hyphens are painted as separate runs with NO SPACES — `Prisma Health` | `-` | `Upstate (Oconee, Lee, and Laurens)` — so no delimiter split breaks, **no dollar moves and NO TOTAL NOTICES**. What breaks is the NAME, in either direction: pasting with a separator invents *"Prisma Health - Upstate"*, and pasting another row's runs without one LOSES A REAL SPACE (page 7's `Self Regional` | ` ` | `Healthcare (Lakelands Region)`). **SO NEITHER CONSTANT SEPARATOR IS CORRECT AND THE ANSWER WAS MEASURED FROM THE PDF'S OWN `/Widths`**: Aptos space 203/1000 em, hyphen 340/1000, observed hyphen advance 4.08pt so 12pt type. **45 within-line boundaries — 43 unspaced (worst residual 37 em-units) against 2 spaced at 205.6 and 209.8 versus a space of 203**, a separation of more than five times the noise, and anything near the midpoint is **REFUSED** because a boundary this test cannot read is a name this project cannot spell. **THE RESIDUAL IS KERNING, NOT ERROR** — a `/Widths` array carries no pair adjustments — which is why the assertion pins the SEPARATION rather than a tolerance. **AND THE READER WAS ALREADY RIGHT**: `rhtp_pdf_run_table()` returns the producer's space runs as runs of their own, so `collapse = ""` is exact and session 32's open question needed **no fix on this producer** — the widths PROVE it rather than repair it. **WHAT THE MEASUREMENT FOUND THAT NOTHING ELSE WOULD HAVE: SOUTH CAROLINA SPELLS ONE ORGANISATION THREE WAYS** — 17 rows `Prisma Health-Upstate`, ONE `Prisma Health- Upstate (Anderson)`, ONE `Prisma Health (Oconee)` — and the first two differ by a SINGLE SPACE RUN, so either naive paste collapses 130 awardee strings to 129 for a reason nobody could see. Nothing merged (§2). **(3) A THIRD SHAPE, AND IT IS THE ONE THAT WOULD HAVE BROKEN A LINE-MODEL READ: 33 ROWS WRAP their name over two lines with the number and amount painted at the MIDPOINT and the line ids INTERLEAVED** (page 8: name line 22, number 21, name 23, amount 24), so `rhtp_pdf_lines()` cannot assemble them AT ALL. Arkansas's line model MERGES, Wyoming's SPLITS, **SOUTH CAROLINA'S INTERLEAVES**; rows are built by Y-BAND and the failure is driven on all 33 while requiring the line model to still read 20 ordinary rows. **§6.2 WITH NO CMS FOOTER ANYWHERE, ON EITHER DOCUMENT** — the award list contains *"Centers for Medicare"*, *"financial assistance"*, *"funded by CMS"* and even the bare token *"CMS"* **ZERO TIMES** (Arkansas's shape) — **AND SESSION 39'S FOOTER HAS GONE FROM THE PROGRAMME PAGE TOO**, `scdhhs.gov/RHTP` now 301-ing to `/resources/grants`. Recorded as an observation about the SOURCE, never a finding about the programme (§0.4). **THE PROVENANCE IS STRUCTURAL AND UNUSUALLY GOOD: the award list's FOUR Initiative headings are EXACTLY the four SCDHHS names as its RHTP initiatives, and the FIFTH it names — the Tech Catalyst Fund — is ABSENT, which is what the page says to expect.** A document whose section headings are the state's own initiative set, complete and with the one exception the state itself flags, is tied to the programme by its STRUCTURE rather than by a sentence. Plus CMS's 228-grant count, and a date test passing with room (applications due 2026-06-01, Anticipated NOA 2026-07-31, list posted 2026-09-15, against the 2025-12-29 NOA). **BOTH §0.1 NEGATIVE CONTROLS SIT ON THE RHTP PAGE ITSELF AND THE SECOND IS THE HOSPITAL-MONEY ONE**: the $48.2M Rural & Medically Underserved Area roster (2024-02-02, *"rural"* in its title) and *"SCDHHS Awards Behavioral Health Crisis Stabilization Grants to **13 SOUTH CAROLINA HOSPITALS**"* (~$35,000,000, 2023-06-23) — California's SRHRP trap in duplicate, **and §6.2's date test disposes of both on its own**, 23 and 30 months before the NOA. **THE YEAR IS PARTIAL AND SOUTH CAROLINA SAYS SO**: the Tech Catalyst Fund, **$32,730,351**, runs through the South Carolina Research Authority (§7's designated pass-through route) and has named nobody. **THE HOSPITAL FIGURE IS A FLOOR AND THE UNCERTAINTY IS THE SECOND LARGEST IN THIS REPOSITORY: 55 rows / $56,587,137.77 (24.1% of rows, 33.8% of dollars) against 150 rows / $92,759,791.23 on §8's standing fallback**, the unstated-form question a THIRTEENTH time. One-directional, so the ceiling is **205 rows / $149,346,929.00 (89.9% / 89.3%)**. **NOTHING WAS PROMOTED (§0.4) AND THE REFUSAL RUNS BOTH WAYS**: Self Regional Healthcare ($17,663,869), McLeod Health, AnMed, Prisma Health and Tidelands Health are not promoted UP, and the nineteen **SCDBHDD** rows — the state's own behavioral health department, whose acronym §8's name rule does not reach — are not demoted to `STATE_AGENCY`. **THE TWO MUSC SPELLINGS CLASSIFY OPPOSITE WAYS AND MOVE $3,345,533**: *"Medical University Hospital Authority"* is `HOSPITAL_OR_SYSTEM`/`Yes` and *"Medical University of South Carolina"* is `UNIVERSITY_OR_AHC`/`No` — North Carolina's UNC finding in a state that PRICES its rows, and **BOTH machine answers are KEPT**, because these are arguably two different legal bodies and no source in hand says otherwise. **THE PROJECT NAMES MOVE NOTHING, MEASURED**: not one of the eight carries a hospital token, so feeding them to the flow classifier changes not even a flow code — Arkansas moved eleven codes and $0; South Carolina moves neither, so the exclusion is §0.3a as a RULE rather than a repair. **`SC_RECIPIENT_FORM_NOT_STATED` IS FLAGGED IN SOUTH CAROLINA'S OWN FILES ONLY AND DELIBERATELY NOT IN THE SHARED QUEUE**, because a verification pass is in progress in a separate workbook; it lives on the rows and in `sc_year1_status.csv`, with an assertion that fails if either goes. **ZERO RCJ TIER 3 CANDIDATES against 33 RCJ records** — Florida's shape a FIFTH time, third largest of the five in dollars — **though not quite their shape**: those four were invisible to BOTH layers, while South Carolina reached the trigger list through CMS's newsroom on 2026-09-17, **TWO DAYS AFTER SCDHHS HAD ALREADY POSTED THE ROSTER**. `NAMED_HOSPITAL` goes **692 rows / $542,946,270 / 17 states -> 747 / $599,533,409 / 18** — `docs/session47_south_carolina_extraction.md`) (Session 46 — THE PROBES STOP WRITING TO THE EVIDENCE ARCHIVE, MISSISSIPPI IS EXTRACTED, AND ALASKA IS REFRESHED. **TASK 1 — A PROBE READS.** `tx_probe()` opened with `tx_fetch_sources(force = TRUE)`, so from session 19 until today **every firing of the Texas Routine re-downloaded eleven sources INTO `data/evidence/TX/`** and rewrote the manifest. **THE DAMAGE IS NOT UNTIDINESS**: `tx_validate()` reads the archive, so after the re-fetch the probe was comparing the live site against bytes it had JUST DOWNLOADED — it could only ever report that Texas had not moved, and **a roster appearing on an RFA page would have been ARCHIVED FIRST AND ASSERTED AGAINST SECOND**, so the tripwire designed to fail the build would have passed on the very thing it was watching for. Rewritten read-only (fetch to memory, content digest, tripwires against the LIVE bytes) and it probes **both state-funded CONTROL pages too**, failing in both directions — a subject page GAINING the award section means Texas awarded, a control LOSING it means HHSC renamed the thing we look for. Ran live: six pages content-identical, no roster, both controls firing. **`rhtp_probe_run()` MAKES IT A RULE**: every one of the NINETEEN `--probe` branches goes through it, it snapshots `data/evidence/` around the probe on **path, size AND MTIME** (a `force = TRUE` re-fetch rewrites files whose BYTES have not changed, and a digest-only snapshot calls that clean), and refuses if anything moved. **TWO TESTS READ THE SOURCE OF EVERY FILE IN `R/`** — one requires any file offering `--probe` to route it through the guard, the other that no `*_probe` body calls a `_fetch` writer. Nothing caught Texas for twenty-seven sessions because **nothing looked**. **AND `logs/probe_results.csv` IS COMMITTED**: state, timestamp, `UNCHANGED`/`CHANGED`/**`TRIPWIRE`**/`ERROR`, and which page. A fired tripwire is **logged AND re-raised** — the throw is the signal, and without this the run that MATTERED would be the only one leaving no trace; `ERROR` is kept apart from `TRIPWIRE` because one is a statement about the state and the other about our access (§0.4). One defect caught in testing: **`isTRUE()` IS NOT VECTORISED**, so a first draft reported every page of a multi-page probe as UNCHANGED — indistinguishable from a quiet week. **TASK 1b — THE THREE HALTED CONTROLS, AND TWO OF THE THREE STATES HAD MOVED WHILE NOBODY COULD SEE.** All three failed on a DATED ANCHOR THAT HAD GONE STALE, which is the apparatus working. **LOUISIANA: LDH RE-DATED ALL SEVEN WINDOWS.** The assertion pinned *"Late July to mid August"* x3 and *"Mid to late August"* x4 and found **0 and 0**; the block now reads **"End of September" x6 and "Mid-September" x1**. Still seven against seven, still every application Closed, still nobody named — **so the negative is unchanged and only its CLOCK moved: Louisiana did not award, it SLIPPED, by 30 to 46 days**, measured from two committed archives (the 2026-09-02 snapshot is KEPT as `programme_prior`, because it is the only evidence LDH ever published the July/August dates). Windows are now PARSED and each one's latest date DERIVED, so a further slip re-dates the finding instead of halting the Routine, while a window disappearing or a solicitation re-opening still fails hard. **WHICH WINDOWS HAVE PASSED IS DERIVED, NEVER ASSERTED**: on 2026-09-21 it is ONE of seven, and the old version claiming all seven was a claim about a state read off a constant (§0.4). `la_year1_status.csv` reads `stage` and `announcement_window` off the page on every build for the same reason. **NEW MEXICO: SIX REGIONAL HUBS ARE SELECTED AND NAMED, THREE OF THEM HOSPITALS — AND THE TRIPWIRE DID NOT FIRE.** The halt was a passed submissions deadline; underneath it HCA now says *"HCA has selected six Regional Hub Organizations to lead Healthy Horizons ... with more than $74 million in regional funding"* and names Cibola General Hospital, UNM (x2), Eastern Plains Council of Governments, **Gila Regional Medical Center** and **Nor-Lea Hospital District**. **EVERY ONE of `NM_AWARD_POSTED`'s ten phrases was checked against that sentence and NOT ONE MATCHED** — "selected organizations" is not "selected six Regional Hub Organizations" — so the tripwire whose whole job was to fire the day New Mexico named a recipient **sat quiet while New Mexico named six**. A marker list built from the phrasings a state has ALREADY used cannot catch the one it uses NEXT; the verb is now matched with its object left open and the six hubs are pinned **BY NAME**. **SELECTED IS NOT AWARDED** (§0.3, Missouri's Hub Anchors) and New Mexico's is a step STRONGER than Missouri's because HCA attaches a pool where DSS attached nothing — but there is still NO per-hub amount, no executed contract and no subrecipient, and HCA's own next sentence is *"Opportunities to engage in with Regional Hubs are forthcoming"*. **NOT EXTRACTED — reported first, Arkansas's and Wyoming's footing**; the $74M is a POOL and $74m/6 = $12.3m is nobody's figure (§6.2). **SIX REGIONS ARE FIVE ORGANISATIONS** — HCA's 2026-09-18 release says *"has selected five organizations"* and UNM holds Regions 2 AND 3, North Carolina's ROOTS arithmetic exactly; both counts pinned. **TASK 2 — MISSISSIPPI: 167 AWARDS, $104,115,146.80, AND IT IS THE FIRST STATE TO LEAVE `INVESTIGATED_NO_LIST`**, which is what that code is FOR. The Governor's 2026-09-14 release names and prices all 167 across the three programmes the funding page had said were complete: **RTG 97 / $47,406,374.18, RCGC 43 / $43,334,338.62, TCE 27 / $13,374,434**. **THE $0.80 IS TRUNCATION**: nine rows carry cents summing to $3.80, and the headline drops the remainder rather than rounding (which would have given $104,115,147). Neither figure corrected (§8). **THE TWO ODD-SHAPE ROWS, AND ONLY ONE COSTS A DOLLAR.** RCGC 23 is *"North Mississippi Medical Center, Inc. - Lee County- $2,500,000"* — **the space before the hyphen is MISSING**, so a `" - "` split finds no amount and DROPS the row: 166 awards, short by $2,500,000 and by the largest award in the section. **THE TOTAL CATCHES THAT ONE.** RCGC 30 is *"Northeast Mental Health - Mental Retardation Commission d.b.a. LIFECORE Health Group - Monroe County - $27,600"* — **the ORGANISATION'S OWN LEGAL NAME contains `" - "`**, so a left-to-right split reads the awardee as "Northeast Mental Health" and the COUNTY as "Mental Retardation Commission d.b.a. LIFECORE Health Group". **THE MONEY IS UNTOUCHED**: 167 rows, every total reconciling to the cent, and a file quietly containing an organisation that does not exist in a county that does not exist. **SO THE ROW COUNT AND THE TOTAL AGREE ON A DEFECTIVE PARSE AND THE SECOND DEFECT IS INVISIBLE TO BOTH.** A THIRD shape is not a defect but is a silent drop: TCE 9 prints **"Lafayette Co."** where the other 166 print "County" (-1 row, -$300,000). **LIFECORE HOLDS TWO AWARDS UNDER TWO SPELLINGS** ($27,600 and $30,610) and they are **NOT MERGED** (§2). **68 NAMED-HOSPITAL AWARD ACTIONS, $47,454,812.18** — third largest single-state contribution after Georgia and Wyoming. **167 ACTIONS ARE 103 DISTINCT AWARDEE STRINGS** and Memorial Health System alone holds TWELVE, so it is one row per AWARD ACTION (Michigan's lesson as the default). **THE UNSTATED-FORM QUESTION A TWELFTH TIME AND ONE-DIRECTIONAL: 96 of 167 rows / $56,022,017.62, 53.8% of the round**, every one already `No`, so $47,454,812.18 is a genuine FLOOR and **$103,476,829.80 a genuine CEILING**; NOTHING PROMOTED (§0.4), queued as `MS_RECIPIENT_FORM_NOT_STATED`. **THE DESCRIPTIONS ARE NOT USED FOR TYPING** — a description describes an ACTIVITY and §0.3a judges the RECIPIENT (Arkansas's precedent), asserted by requiring every `recipient_type` to be reproducible from the NAME alone. §6.2 passes on the release's own RHTP sentence plus the **CMS SHARE** of session 44's non-100%-federal footer, never the headline. **THE YEAR IS PARTIAL AND MISSISSIPPI DATES THE REST ITSELF**: workforce and psychiatric emergency services *"will be announced in the next 30 to 45 days"* (**2026-10-14 .. 2026-10-29**) and two more open in October, so **$101,792,073 is in no public roster**. **TASK 3 — ALASKA: 185 -> 244 AWARD ACTIONS, $239,186,195** (+59, +$57,314,828, **0 revised, 0 withdrawn**); hospital rows **32 -> 43**, hospital dollars **$49,686,225 -> $62,396,425**. **ALASKA'S OWN WEEKLY TABLE ACCOUNTS FOR IT EXACTLY**: Week 5 (32) + Week 6 (27) = **59**, the diff of two archived workbooks stated independently by the state, against a cumulative **$239M / 244 Projects**. **THE SINGLE-WEEK FORM OF THAT CONTROL FAILED, CORRECTLY** — it held only because every previous refresh had landed exactly one week later, and this one spans three; it now sums the trailing weeks, the COUNT exactly and the MONEY within a tolerance **DERIVED from Alaska's own document** (Alaska's six weeks sum to $238.7M against its own stated $239M, a $0.3M gap in the publisher's arithmetic that no reading of ours can close). **THREE CONSTANTS HAD BEEN DOING TWO JOBS EACH**: `AK_PRIOR_FILE` answered both "what did Alaska publish LAST time?" and "which snapshot did CMS describe?", so rolling it would have moved the 142-project reconciliation onto a file CMS never described — split out as `AK_CMS_ANCHOR_FILE`; the Planning count was asserted UNCHANGED, a true observation turned into a rule, and **ALASKA HAS RESUMED PLANNING AWARDS, 19 -> 26**, so it is re-based onto the anchor; and the revision note diffed against the rolling prior with a hard-coded date pair, so this refresh would have **SILENTLY DROPPED Southcentral Foundation's +$4,306,887 note**, session 22's finding. **TWO NEW ORGANISATION-TYPE TOKENS AND THE CLASSIFIER REFUSED RATHER THAN GUESS**: *"Health Care Providers"* and *"Health care provider"* are SERVICE tokens (reading either as `HOSPITAL_OR_SYSTEM` would add hospital dollars where Alaska stated only a class), and *"Education Organization (Not Public University)"* is Alaska **RE-WORDING ITS OWN TOKEN** — both exact strings listed, never a fuzzy match, because the earlier snapshots are still parsed. **PROVABLY INERT ON EVERY OTHER STATE**: all twenty-six other extractors re-run, every reference CSV byte-identical except Arkansas's date-derived `days_since_close`; nineteen xlsx renders differed only in `dcterms:created` and were reverted. **AND THE COMPLETENESS RE-CHECK LEARNED IT CAN BE THE STALE ONE** — it alleged **ALASKA HAD WITHDRAWN 59 AWARDS** when its own 2026-08-29 download simply predated the extraction; staleness is now detected and reported, and the withdrawal check still fires when the snapshot is current (§0.4). **WHAT MOVED**: `NAMED_HOSPITAL` **613 rows / $482,781,258 / 16 states -> 692 / $542,946,270 / 17**; the disposition **25/9/6/10 -> 26/8/6/10** — `docs/session46_probe_hygiene_mississippi_alaska.md`) (Session 44 — DELAWARE, IDAHO AND OHIO ARE EXTRACTED, MISSISSIPPI IS ON A WATCH, AND §0.3a GAINS A SECOND HALF THAT ITS OWN WORKED EXAMPLE NOW PROVES IS NEEDED. **TASK 1 — §0.3a.** Session 43's re-count holds: the Delaware defect is **SEVEN of eleven rows**, not four — rows 1-4, row 8 (**Beebe Medical Center**), row 9 (**TidalHealth**) and row 11 (a §6.2 multi-recipient field naming **Beebe Healthcare**), every one `rhtp_award_yn = yes` and `hospital_yn = no`. **BUT ONE THING SESSION 43 SAID ABOUT ROWS 8 AND 9 IS WRONG AND CORRECTING IT MAKES THE FINDING SHARPER (§2.1)**: it called them *"bare hospital names with NO ACTIVITY ATTACHED AT ALL"*, and the workbook says otherwise — both carry `activity_type = "Rural Delaware Diabetes Wellness Pilot Program"` and a full description, and the reviewer instructions have listed row 8 as *"Beebe Medical Center | Diabetes wellness pilot"* since session 6, so the two documents already disagreed. **THE REAL DISTINCTION IS NOT WHETHER AN ACTIVITY IS ATTACHED BUT WHERE IT SITS**: on rows 1-4 the setting is WELDED INTO THE `awardee` CELL (*"Beebe Healthcare – Georgetown Middle School"*), so the recipient column ITSELF reads as a school; on rows 8-9 the `awardee` cell is NOTHING BUT A HOSPITAL NAME and the activity beside it is a DIABETES PILOT — clinical care, nothing non-hospital in it. **NEITHER COLUMN OFFERED A WRONG ANSWER AND BOTH ROWS CAME BACK `no` ANYWAY.** So the rule gains a **SECOND HALF — READ THE RECIPIENT AT ALL**: refusing to be led by the activity is what rows 1-4 needed and is not enough on its own, because the failure on 8-9 is not misreading the recipient field but NOT READING IT. And a **COROLLARY**: the `awardee` field is not always only the awardee — where a publisher packs a recipient and a site into one string, split it and code the half that received the money. Patched into all three documents; CLAUDE.md had **NO §0.3a SECTION AT ALL** and gains one, insert-only. **NOTHING WAS RE-CODED** (blocker 2 and Stage 5 own the workbook). **TASK 2 — MISSISSIPPI, AND ITS FOOTER BREAKS THE TIER CHECK IN A WAY 222 OTHER FOOTERS DO NOT.** Measured before anything was written: **222 CMS footers parse out of the committed corpus** (186 HTML/text + 36 PDF) and **ALL 222 ARE 100 PERCENT FEDERAL**, `tier_amount == headline` on every one. Mississippi's says *"totaling **$205,990,180.16**, with **99.96% funded by CMS/HHS ($205,907,220.16)** and **0.04% funded by non-government sources ($82,960)**"*. Driven, not reasoned about: the headline declared `STATE_ALLOTMENT` is REFUSED (correct) and declared `SOLICITATION` is **ACCEPTED, WHICH IS WRONG** — it is Tier 1 plus a match and there is no Tier 2 pool in that sentence at all. **AND THE FOOTER IS INTERNALLY EXACT, WHICH SESSION 43 DID NOT SAY**: headline − CMS share = **$82,960.00**, the printed match, TO THE CENT, so the publisher's three numbers close on each other and the CMS share is a STATED figure. (Session 43's $82,960.16 is the headline against the §7.1 ANCHOR, which carries the anchor's own 16 cents — a different comparison, also true.) **THE FIX IS A PARSER, NOT A WIDER MARGIN**: `rhtp_footer_parse()` / `rhtp_footer_cms_share()` / `rhtp_assert_footer_text_tier()` take TEXT, so the distinction is made once where the evidence is; they are **INERT ON ALL 222 COMMITTED FOOTERS**; and where a footer states a partial share and prints NO dollar figure they return **NA, never a product** (§0.4 — headline × 99.96% is a number nobody published). `RHTP_FOOTER_ALLOTMENT_MARGIN` **STAYS AT $10,000** and a test pins it there. **BOTH MISSISSIPPI CONTROLS ARE UNUSUALLY GOOD**: the positive one is the channel the STATE ITSELF NAMED (the Governor's newsroom, carrying *"deploying $3,375,709"* for youth mental health), and the §6.2 negative control **IS RCJ'S THIRD CANDIDATE** — DOM's Completed Procurements page shows *"Quote – Rural Health Transformation Program"*, *"Public Notice of Award – 8/13/2025"*, *"the selection of **HORNE LLP**"*, a NAMED AWARDEE ON A STATE HOST UNDER AN RHTP TITLE that is the consultant hired to WRITE THE APPLICATION, **138 days BEFORE the NOA** — and **session 20's sweep had ALREADY flagged that row `PROVENANCE_PREDATES_NOA`** off the title's own date, so a filter written four sessions before anyone looked at Mississippi and a live read agree from two directions. **TWO DIGEST MECHANISMS AND ONLY ONE IS NEW**: `mississippirhtp.com` is Cloudflare email obfuscation (Louisiana's seventh, recurring — 3 fetches, 217,198 bytes every time, 3 digests); **`medicaid.ms.gov` IS NEW AND IS THE MOST LITERAL YET** — the WordPress Simple Banner plugin serialises the render wall-clock time **TO THE MICROSECOND** into a script body, three times per render, fixed-width so a byte check passes it. Routine `trig_01RmgWna5oXyoMCXYrVqGVvE`, **TUESDAYS AND FRIDAYS 09:40 UTC** (below the 10:50-21:10 band the other fourteen occupy); ran live and reports UNCHANGED. **TASK 3 — DELAWARE IS EXTRACTED AND IT IS §0.3a's WORKED EXAMPLE ARRIVING AS DATA: 4 AWARD ACTIONS, 3 ORGANISATIONS, `amount` EMPTY ON EVERY ROW.** *"DHSS today announced **awards** to establish four new school-based health centers"* — Nemours Children's Health, TidalHealth, **Beebe Healthcare TWICE**. **AND §8's NAME RULE MISSES ALL THREE, WHICH IS MEASURED**: `rhtp_classify_recipient_type()` returns `NONPROFIT_CBO`/`LOW` for each, so **LEFT TO THE MACHINE DELAWARE HAS NO HOSPITAL ROWS AT ALL** — the spec's own defect reproduced IN CODE thirty-seven sessions after it was found by hand — while *"Beebe **Medical Center**"*, the same organisation under `DE Verify.xlsx`'s spelling, classifies `HOSPITAL_OR_SYSTEM`/`HIGH`. **THE OVERRIDE IS THE SPEC'S, NOT A SESSION'S (§0.4)**: §0.3a names all three as *"hospitals and health systems"* and states the coding, and the classifier's answer is preserved on every row. **IDAHO IS THE CONTROL THAT PROVES THIS IS ABOUT EVIDENCE**: Comagine Health gets the SAME machine answer and KEEPS it, because Idaho states no form. **TWO DELAWARE FIGURES ARE DELIBERATELY OUT OF THE AWARD FILE** — the $195,000 School-Based Health Centers *"Year 1 Budget"* is a PLAN not a round (§0.3; $195,000/4 = $48,750 is nobody's figure, and a guard refuses it) and the release's only currency figure is the **$157,394,963.86 ALLOTMENT** (§0.2). **ONE CLOSURE ARRANGED BY NOBODY**: the fifteen live Year 1 budgets match the fifteen parsed from Delaware's budget-narrative PDF in **session 8** TO THE CENT, $141,655,467.48 both ways. **TASK 4 — IDAHO AND OHIO, ONE NAMED AWARDEE EACH.** Idaho names **Comagine Health** with NO amount (an INTENT — *"Anticipated award start date"*) and contributes to **NO bucket at all**; its footer is the **WEAKEST SUBJECT MET SO FAR**, *"**This website** is supported by"*; RCJ carries it as **"Co-Imagine Health" at $1** and the match is THIS PROJECT'S READING, not a source's (§0.4), which does not matter because the row is built from the state page; **ELEVEN opportunities are open**, the most active of the fourteen. Ohio names **Ohio University at $10,000,000** — a UNIVERSITY, so `NON_HOSPITAL` and **$0 of hospital money** (Maine's UNE row) — and **ITS CMS FOOTER PRINTS $10,000,000, THE SUBAWARD ITSELF**, a THIRD POSITION on §0.2's axis after the allotment and the pool. **THE TIER CHECK CANNOT SEE IT**: $10,000,000 collides with no allotment, so declared `SOLICITATION` it is **ACCEPTED**, and it is Tier 3 — session 37 said the tier is not in the footer's GRAMMAR, and Ohio adds it is not always in its ARITHMETIC either. **Asserted, not patched.** Ohio's year is PARTIAL (*"Additional contracts will be awarded in the coming months"*) and ODH's page is STALE, still saying the state *"will submit an application. If awarded..."*. **RCJ IS RIGHT ABOUT OHIO** — name, figure and document — which is rare enough to pin. **WHAT MOVED**: `NAMED_HOSPITAL` goes **609 rows / $482,781,258 / 15 states -> 613 / $482,781,258 / 16** — Delaware adds four rows and **NOT ONE DOLLAR**, the third state after Iowa and North Carolina to move the row count alone. The disposition goes `EXTRACTED` 22 -> **25**, `INVESTIGATED_NO_LIST` 8 -> **9**, `INVESTIGATED_NO_PROBE` 6, `QUEUED` 14 -> **10** (AZ CO MT ND RI UT VA VT WA WV, $1,916,786,628 and 24 candidates), both tables REBUILT from `R/03k`'s constants. **SESSION 43 REFUSED TO MOVE THESE FOUR ON A REPORTING PASS AND SESSION 44 MOVED THEM THROUGH THE WORK, WHICH IS THE ONLY ROUTE OUT OF THAT BUCKET** — `docs/session44_delaware_extraction_mississippi_footer.md`) (Session 43 — WYOMING IS ON A TWICE-WEEKLY WATCH, THE FOURTEEN-STATE LOW-CANDIDATE QUEUE IS READ END TO END, AND A STATUS CODE NOW SEPARATES A RE-CHECKABLE NEGATIVE FROM ONE NOBODY CAN RE-CHECK. **TASK 1 — WYOMING**: `R/03aj --probe` ran LIVE first and reports UNCHANGED on all four pages with no award language and no open call; Routine `trig_016y7C12cyxy7GmGNJDxoAbW`, **TUESDAYS AND FRIDAYS 21:10 UTC**, first firing 2026-09-04. Twice-weekly because **THE OBLIGATION DEADLINE FALLS BEFORE THE NEXT COMMITTEE MEETING** — end of October 2026 (the minutes) or 2026-10-01 (WDH's timeline) against a committee that next meets 4–5 NOVEMBER with the CMS site visit — so the window in which 75 approvals become executed agreements opens and closes with no public meeting in it. 21:10 because the thirteen Routines already running occupy a CONTINUOUS BAND from 10:50 to 20:20 UTC. **AND THIS FILE'S OWN "NEXT SESSION" NOTE WAS WRONG AND IS CORRECTED (§2.1)**: it said NO ROTATING-TOKEN STUDY HAD BEEN DONE on `health.wyo.gov` and that the content digest was inherited, while session 42's own block above calls it THE ELEVENTH ROTATING-DIGEST MECHANISM on two fetches three seconds apart and `wy_probe()`'s docstring says it is MEASURED — session 35's `R/03ab` header defect, this time in the instructions a future session reads first. **TASK 2 — THE FOURTEEN-STATE RCJ LOW-CANDIDATE QUEUE, $2,668,093,442 AND 34 CANDIDATES BETWEEN THEM, READ IN ALLOTMENT ORDER AND DELIBERATELY NOT EXTRACTED.** `/api/v1/activity` found the route a NINTH time (609 state URLs). **THREE OF THE FOURTEEN ARE NOT NEGATIVES.** **MISSISSIPPI IS THE MOST IMMINENT STATE IN THIS REPOSITORY**: `mississippirhtp.com` is a DEDICATED RHTP DOMAIN — the THIRD after Kentucky's and Arkansas's, and Arkansas's was the one that had awarded $149M — and its funding page says *"The selection process is complete"* for three programmes and that *"Governor Tate Reeves will formally announce details regarding all executed sub-awards in the coming weeks"*. **SOUTH CAROLINA'S EMAIL SHAPE WITH A PUBLIC ANNOUNCEMENT ATTACHED**, against **700+ applications, ~$82M available, $676M sought** — oversubscribed EIGHT TO ONE (§0.3 in the state's own numbers) — and *"The first awards are expected by the end of the month"*, A DATE THAT HAS PASSED. **AND MISSISSIPPI'S CMS FOOTER IS THE FIRST IN THIS REPOSITORY THAT IS NOT 100% FEDERAL, WHICH BREAKS THE TIER CHECK IN A NEW WAY**: all **215** footer occurrences across **76** committed files say *"100 percent funded by CMS"* (154) or *"100% funded by CMS"* (61), with NO other percentage anywhere; Mississippi's says *"totaling **$205,990,180.16**, with **99.96% funded by CMS/HHS ($205,907,220.16)** and **0.04% funded by non-government sources ($82,960)**"*. The CMS SHARE matches the §7.1 anchor TO THE DOLLAR; the HEADLINE exceeds it by $82,960.16, **EIGHT TIMES the $10,000 margin** — so, DRIVEN rather than reasoned about, `rhtp_assert_footer_not_allotment()` REFUSES the headline as `STATE_ALLOTMENT` and **ACCEPTS IT AS A `SOLICITATION` POOL, WHICH IS WRONG**. Parse the CMS SHARE, never the headline, and **DO NOT WIDEN THE MARGIN** (§0.2: a figure that fails is a document to re-read). **DELAWARE HAS AWARDED, NAMES TWO HOSPITALS, AND PRICES NOBODY**: DHSS's 2026-07-29 release gives FOUR award actions to THREE organisations — **Beebe Healthcare ×2, Nemours Children's Health, TidalHealth** — with NO per-recipient amount (Nevada's, Iowa's and North Carolina's shape at the smallest scale yet). **IT IS THE SPEC'S OWN §0.3a WORKED EXAMPLE ARRIVING AS A REAL AWARD**: a school-based health centre is an ACTIVITY and these recipients are HOSPITALS, so `DIRECT`/`Yes`, not `NON_HOSPITAL` — the row `9fdc156` fixed and `219d803` reverted now has live data behind it. RCJ carries all four CORRECTLY NAMED AT $1 EACH (Missouri's placeholder), and Delaware's own footer figure IS the allotment ($157,394,963.86 against $157,394,964), so it is Tier 1 wearing the weak *"This project"* grammar. **AND THE OWNER'S OWN `DE Verify.xlsx` — BLOCKER 2's EVIDENCE — CARRIES THE SAME FOUR ROWS WITH `notes` READING "Cannot determine award amount"**, a hand verification and a live read agreeing that Delaware names recipients and prices none of them, nothing arranged. **RE-COUNTING IT CORRECTS §0.3a: THE DEFECT IS SEVEN OF ELEVEN ROWS, NOT FOUR** — the four school-based centres PLUS **Beebe Medical Center**, **TidalHealth**, and a multi-recipient row naming **Beebe Healthcare**, every one `rhtp_award_yn = yes` and `hospital_yn = no`. **AND ROWS 8 AND 9 ARE THE SHARPER HALF, BECAUSE §0.3a's OWN EXPLANATION DOES NOT REACH THEM**: the four school rows were coded by ACTIVITY, which is the error that section names, while rows 8 and 9 are BARE HOSPITAL NAMES WITH NO ACTIVITY ATTACHED — nothing to be misled by. So the rule gains a second half: judging the recipient is not only refusing to read an activity, it is READING THE RECIPIENT AT ALL. Patched into the spec INSERT-ONLY; NOTHING WAS RE-CODED (blocker 2 and Stage 5 own it). **IDAHO NAMES ONE AWARDEE** — Comagine Health, no amount — which RCJ carries as **"Co-Imagine Health" at $1** — no such organisation exists and Comagine Health is Idaho's only named RHTP awardee, so it is evidently the same body under a corrupted name, though NO DOCUMENT SAYS SO and the match is this session's reading, not the source's (§0.4). **OHIO NAMES ONE** (Ohio University, $10M) and RCJ is RIGHT about it. **MONTANA AND COLORADO ARE NEGATIVES WITH DATES ON THEM**: Montana's $4M EMS pool closed 8/15/26 with *"funding decisions will be shared in **September**"*, and Colorado *"anticipates making our award announcements by the **end of September 2026**"*. **FOUR CANDIDATE SETS NEED READING BEFORE ANYONE TRUSTS THEM**: WEST VIRGINIA's four carry FILE NAMES as document titles and one is the **Breast and Cervical Cancer Screening Program** (Indiana's appended-label shape, NOT established here); ARIZONA's four are budget lines from a webinar held BEFORE the state was awarded anything; WASHINGTON's two are **`[Contractor Name]`**, a literal template placeholder, and a CLASS at $30.2M; UTAH's two are ONE award under TWO document revisions (Connecticut's double-count). **AND MONTANA'S *Directory* IS A §0.3 TRAP WORTH NAMING**: 100+ organisations on a state page that DPHHS itself says *"does not guarantee a partnership, subcontracting opportunity, or contract award"*. **`hcpf.colorado.gov` NEEDS THE RFC CRAWLER AGENT AND THAT IS SESSION 10'S ANSWER, NOT SESSION 27'S** — honest agent 403, bare `Mozilla/5.0` 403, the RFC convention CARRYING OUR NAME AND CONTACT URL 200, `robots.txt` itself 403 — so identifying honestly is the fix and **NO michigan.gov-STYLE EXCEPTION WAS TAKEN**. **`taggs.hhs.gov` IS AN INDEPENDENT FEDERAL RECORD OF EVERY STATE'S RHTP AWARD, THIS PROJECT HAS NEVER USED IT, AND IT WAS OBSERVED RATHER THAN FETCHED HERE** (RHTCMS332047 VT, RHTCMS332053 DE, RHTCMS332087 OH) — a §7.1 corroboration source that is NEITHER RCJ NOR THE STATE, worth a session, not used here. TWO hosts read UNREADABLE/UNKNOWN (§0.4): Ohio's JS solicitation table and Idaho's Laserfiche repository. **THE §0.1 SWEEP WAS RE-RUN FIRST, AS INSTRUCTED, AND REBUILDS BYTE-IDENTICAL — AND THAT IS A REPRODUCTION, NOT A RE-MEASUREMENT**: there is exactly ONE pull on disk (2026-08-27), so it re-read the same 5,056 records and could only return the same answer. **The genuine re-measurement is due at the next national pull and must run before the next state is extracted.** **TASK 3 — `extraction_status` AND `queue_status` GAIN `INVESTIGATED_NO_PROBE`.** Six states (HI MA MN NJ SC TN) sat at `NOT_EXTRACTED`, WHICH SAYS NOBODY HAS LOOKED. `INVESTIGATED_NO_LIST` would have been FALSE FOR ALL SIX: it promises a re-checkable negative — a committed archive AND a probe with a tripwire — and **ONLY HAWAII HAS AN ARCHIVE AT ALL** while for MA, MN, NJ, SC and TN the finding lives in PROSE ALONE, and **NOT ONE OF THE SIX HAS A PROBE**. **AND TWO ARE NOT NEGATIVES**: SOUTH CAROLINA HAS AWARDED (712 applications, notices by EMAIL), and MASSACHUSETTS IS UNREADABLE RATHER THAN SILENT (every mass.gov path Akamai-403 on four agents), so the code would state a fact about the STATE from a fact about OUR ACCESS (§0.4). The new code says WHAT THIS REPOSITORY DID, is documented as DELIBERATELY WEAKER, and names the next action: WRITE THE PROBE. Both tables REBUILT from `R/03k`'s constants, never hand-edited; six rows move and no other column changes. **NO HOSPITAL DOLLAR MOVED ANYWHERE and nothing was promoted (§0.4)** — `docs/session43_wyoming_watch_low_candidate_queue.md`) (Session 42 — WYOMING IS EXTRACTED AND IS THE SECOND LARGEST NAMED-HOSPITAL STATE IN THIS REPOSITORY; INDIANA HAS STILL NOT AWARDED; AND §0.1 GAINS A SIXTH FAILURE MODE THAT IS DIFFERENT IN KIND FROM THE FIVE BEFORE IT. **TASK 1 — WYOMING: 77 AWARD ACTIONS — 75 ACROSS SIX RECIPIENT-LEVEL TABLES IN ONE DOCUMENT PLUS TWO FROM ITS MINUTES — $173,859,752 TO A NAMED RECIPIENT, AND 31 NAMED-HOSPITAL ROWS WORTH $72,661,324** — behind Georgia's $90.3M and ahead of Alabama's $66.1M. The FOURTH zero-signal state with a published, named, priced roster (FL, NC, AR, WY): `trigger_source = NEITHER` on BOTH layers, and the roster is in a GOOGLE DRIVE FOLDER `health.wyo.gov` links behind five words. **THE RUN MODEL IS MANDATORY AND THE ASSERTION PRICES WHAT THE LINE MODEL COSTS**: six rows carry the recipient name in a SEPARATE PAINTED RUN, so `rhtp_pdf_lines()` reads Initiative 1.1 as **SIXTEEN hospitals and $43,044,174**, orphaning **$5,156,000 IN SILENCE** — the two rows still exist with no name on them, and nothing about the output looks wrong. `wy_assert_line_model_splits_names()` is Arkansas's assertion in the mirror (there the line model WELDS three columns, here it SPLITS one row). **AND A SECOND RUN-MODEL FINDING COST A NAME AND AN EIN BEFORE IT WAS CAUGHT**: four tables let a long name OVERFLOW INTO THE EIN COLUMN — *"North Lincoln County Hospital District dba Star Valley Heal83-0327251Y"* is one painted row — so a fixed boundary truncated the name AND returned **NA for the EIN**, which is the exact key that carries Wyoming's stated hospital form from 1.1 into 1.2; name and identifier now come out of ONE WIDE BAND split by regex, with the name kept AS PAINTED. **THE RECONCILIATION CLOSES AT THREE LEVELS**: each table to its own `Total:` row, each table to the budget summary's Approved award, and the summary's fifteen lines to **$205,004,742** — the §7.1 anchor ($205,004,743) to a dollar and CMS's own NOA ($205,004,742.95) to a rounding, three figures pinned and none corrected (§8). **1.1's RECIPIENT FORM IS STATED BY WYOMING AND THAT MOVES $7,525,331**: §8's name rule misses *Powell Valley Health Care Inc*, *Cody Regional Health* and *Crook County Medical Services District*, while WDH heads the column **"Hospital"**, the initiative is *"Critical Access Hospital - Basic"* and the budget narrative gives ONE Facility Type — session 38's UNC and session 39's MCO precedents, and **ILLINOIS'S ELIGIBLE CLASS, NOT NEW HAMPSHIRE'S** (and it does not even need §10.2's pass-through row, because the recipient IS the hospital in all 21 cases). **THE TWO 18s ARE NOT THE SAME 18** — the narrative names eighteen ELIGIBLE CAHs and the committee approved eighteen, and THREE NAMES DIFFER EACH WAY, so the coincidence of count must not read as corroboration (§0.3). **1.2's FIVE HOSPITAL LEAD AGENCIES ($11,000,000) ARE JOINED ON THE EIN AND NEVER ON THE NAME** — Georgia's application-number precedent, an exact key from one publisher in one document — and §0.3a judges the RECIPIENT not the activity (Delaware's Beebe). **THE FIFTH DOES NOT CLOSE ON THAT KEY: "Sheridan Memorial Hospital" CARRIES TWO EINs IN ONE DOCUMENT** (83-6000241 in 1.1, 92-0606087 in 1.2), recorded and not resolved (§8). **EVERY ROW IS A COMMITTEE APPROVAL, NOT AN EXECUTED AGREEMENT**, and two publishers disagree about the deadline — the minutes say *"End of October 2026 (executed contracts/agreements)"* and WDH's timeline says *"October 1, 2026"* — so all 75 are `NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No` + **`AMOUNT_PRELIMINARY`**, §8's existing code; **NO NEW VOCABULARY CODE WAS INVENTED (§2)** and the per-row conditions live in `approval_condition`, not a flag. **THE MINUTES ARE A SECOND READING AND DISAGREE ONCE**: they give University of Utah **$9,225,398** where the table gives **$9,255,398.00**, and it is the TABLE'S figure that sums to the $17,712,410 both documents state ($17,682,410 against $17,712,410) — nothing corrected, both pinned, and the ARITHMETIC is what says which. **THE APPLICANT TRAP IS THE LARGEST IN THIS REPOSITORY**: the same document names **FIFTY-SIX applicants it did not award**, **FORTY under Initiative 3.1 alone requesting $38,794,342 and MOSTLY NAMED WYOMING HOSPITALS** — Sheridan Memorial four times, Converse County three, Laramie County three, Powell Valley three, Ivinson Memorial, North Big Horn, Niobrara, Sweetwater, South Big Horn. The guard is on the **(table, name) PAIR** and not the name, because Memorial Hospital of Converse County is AWARDED under 1.1 and 1.2 while being turned down under 2.2 and TWICE under 3.1; the three 1.1 late submissions share one EIN, **94-2545356 — Banner Health**. **§6.2 PASSES ON CMS'S OWN NOTICE OF AWARD, THE FIFTH STATE** after NV/CA/CT/KY, and **SESSION 36'S DATE PIN HOLDS A FIFTH TIME**: *"Revision (Budget)"*, Federal Award Date 05/14/2026, **+136 days**, budget period still 12/29/2025 — with the minutes corroborating the later date from the other side (*"Wyoming executed its formal agreement with CMS on May 14, 2026"*). The CMS footer is session 27's STRONG form AND its figure IS the allotment (§0.2, session 37's Iowa rule), declared `STATE_ALLOTMENT` and checked by `rhtp_assert_footer_not_allotment()`. **TWO ROWS NAME NOBODY AND THE CLASSIFIER WOULD HAVE SAID OTHERWISE**: 4.2's tribal allocations read *"No bidders, recommend sole source w Tribal provider"*, a SENTENCE, and handed it the shared classifier returns **`TRIBAL_ORG` at HIGH confidence** — §6.1's programme-name-as-awardee hazard — so both are `NOT_YET_NAMED` with `amount` EMPTY and $267,000 in `round_amount`. **THE ELEVENTH ROTATING-DIGEST MECHANISM, AND THE FIRST THAT SURVIVES THE REDUCTION.** `health.wyo.gov` rotates a token on EVERY request (two fetches three seconds apart: 199,946 bytes each, different hashes), which the tag-stripping reduction absorbs — **and the programme page carries a SECOND one that it cannot reach**: a Gravity Forms ANTI-SPAM HONEYPOT whose FIELD LABEL is drawn at random per render, IN THE RENDERED WORDS (*"**Email** This field is for validation purposes…"* one fetch, *"**Name** This field is for validation purposes…"* the next). **THE FIRST LIVE PROBE REPORTED THE PAGE CHANGED ON THAT ONE WORD OUT OF 951.** Every earlier mechanism — a Boomerang nonce, an Incapsula cache-buster, a `?v=` stamp, a re-rolled mailto, a cache variant, a Dynatrace `rpid`, Cloudflare's XOR'd email, in.gov's whitespace — lived in an ATTRIBUTE or a SCRIPT BODY; this one is CONTENT. `wy_html_text()` normalises the label by name and a test drives it; after the fix all four watched pages report UNCHANGED. **WYOMING'S LARGEST SINGLE RECIPIENT IS `Unclear` AND IS IN THE AWARD FILE FOR MISSOURI'S REASON INVERTED**: the sole-source MASTER FISCAL AGENT contract with the **Wyoming Innovation Partnership, $38,618,260**, named by the minutes AND by the budget summary's Notes column — Missouri's Hub Anchors are OUT of its file because DSS said they *"will not act as the fiscal agent"* and **WIP is IN because Wyoming's motion says it IS one**; its class is INDIVIDUALS AND INSTITUTIONS, so it is New Hampshire's FHC (`Unclear`, NEITHER bucket) and NOT Illinois's ICAHN (`Yes`), a $38.6M difference. **$30,877,990 — 15.1% OF YEAR 1 — NAMES NOBODY AT ALL** and lives in `wy_year1_status.csv`, which has no `amount` column. **NOTHING WAS PROMOTED (§0.4)**: 24 rows / $5,716,842 on §8's fallback, one-directional, with Initiative 3.1's *"Powell Valley Health Care"* ($752,302) refused because that table publishes NO EIN and NO organisation type, and *"Campbell County Health (CCH) Emergency Medical Servic"* ($1,700,000) refused because the producer TRUNCATES its name and its EIN is not in 1.1. **TASK 2 — INDIANA HAS NOT AWARDED ITS GROW REGIONAL GRANTS.** `in_assert_regional_not_awarded()` was run AGAINST THE LIVE PAGE: HTTP 200, all three pre-award predicates hold, and the reduced content is **byte-identical to the 2026-08-31 archive** even though the FILE digest moved — **a TENTH digest mechanism and the mildest yet, `www.in.gov` varying LEADING WHITESPACE ONLY** (48 bytes), where every earlier one changed something inside a tag. **The CMS release that put Indiana on the trigger list NAMES NOBODY**: it says the $120M *"will support Indiana's GROW regional grants"* across *"eight regions"* — a Tier 2 announcement of the pool. **BUT THE CLOCK HAS STARTED**: Indiana's own timeline reads *"July — September 2026 Application review, scoring, and award determinations"* and **"Sept. 1, 2026 Grant agreement period begins, and funding distributed to individual entities"** — two days before this ran. **TASK 3 — §0.1 FAILURE MODE 6: THE RECORD IS FILED UNDER THE WRONG STATE.** Modes 1-5 are defects IN a record (programme, tier, action, grain, section); **mode 6 is a defect in WHICH STATE THE RECORD IS**, so every state-scoped check this project runs — the §6.2 registry, the date test, the allotment ceiling, the §8 name rules — is applied to the wrong state's data AND PASSES. Numbered into the spec §0.1 and mirrored into this file §6.1, both INSERT-ONLY (41 and 36 lines, nothing deleted). **`R/02c_state_attribution_sweep.R` MEASURES IT ACROSS ALL 5,056 COMMITTED RECORDS, AND BOTH HALVES OF THE ANSWER MATTER. TEN RECORDS ARE ANOTHER STATE'S, IN FIVE STATES** — WY←UT (5, incl. **Utah's own $195.7M allotment carried as an `UNASSIGNED` WYOMING row** against Wyoming's $205.0M), ND←AR (2, Governor Sanders's $209M Arkansas release), MO←MI, **UT←OK (Wyoming's mirror)**, WA←FL — **so Wyoming is the largest and NOT the only one. AND NOT ONE OF THE TEN IS TIER 3**, the only tier an extractor reads, so the defect has **NOT reached a single award file here**, Missouri's and Indiana's included. That is a measurement of the corpus as pulled 2026-08-27, never a property of the aggregator. **THE SWEEP FLAGS AND A HUMAN READS**: 23 flagged, every verdict hand-read and visible in `SWEEP_VERDICTS`, and **all EIGHT Tier 3 flags are FALSE POSITIVES with legible causes** — a state name inside the recipient's own legal name (*Providence Health & Services–Washington*, a REAL ALASKA AWARDEE in `ak_year1_awardees.csv`), a bare county in a county list (Alabama's *"(Clarke, Washington)"*), a street address (*905 Washington Street*). Widening the exclusion list until the output is empty would suppress the ten real ones with the eight false, so `sweep_build()` REFUSES both an unread flag and a stale verdict. **`NAMED_HOSPITAL` GOES 578 ROWS / $410,119,935 / 14 STATES -> 609 / $482,781,258 / 15** — `docs/session42_wyoming_extraction_wrong_state_defect.md`) (Session 41 — EVERY ROUTINE ID VERIFIED LIVE, THE ZERO-SIGNAL SET IS EXHAUSTED, AND WYOMING HAS A ROSTER IN A DRIVE FOLDER. **TASK 1: all THIRTEEN Routine ids in this file resolve on the live schedule, every cron matches the day and time stated here, and the Arkansas id `trig_01Sw1CDPqYQKXFq4WEVRHH2a` EXISTS** — created 2026-09-03T15:28Z, enabled, first firing 20:20 UTC tonight; the premise that it was written from a draft and never created is not borne out. **STAGE 00 RAN LIVE: 9 STATES -> 12** — AR (2026-08-31, $149.3M), HI (2026-09-01, $58M), IN (2026-09-03, $120M — the GROW Regional pool, so `R/03s`'s tripwire must be run against the live page next). medicaid.gov lags on IN only; Virginia now reads BOTH, so the lag session 15 could not size closes within six days. Five test pins re-based. **WYOMING IS THE FOURTH ZERO-SIGNAL STATE WITH A PUBLISHED, NAMED, PRICED ROSTER (FL, NC, AR, WY), AND IT IS IN A GOOGLE DRIVE FOLDER** linked from health.wyo.gov: the Rural Health Transformation Advisory Committee's *"Award Approvals - 8.11.26"* and its minutes. Budget summary reconciles to $205,004,742 to the dollar; **1.1 Critical Access Hospital – Basic: 18 named hospitals approved, $48,200,174 (sums to the summary exactly), 3 denied as late (all Banner), eligible class HOSPITALS ONLY** (Illinois's class); 1.2 EMS 11 lead agencies $23,260,000, several hospital-led; GME 3 ($17,712,410, incl. Cheyenne Regional $2,193,968.90; the minutes print U of Utah as $9,225,398 where the table's $9,255,398 is what sums); tech 10 ($12,652,243); primary care 8 FQHC/tribal ($30,465,504.74); ~$59M of fiscal-agent and RFP money names nobody. **EVERY ROW IS A COMMITTEE APPROVAL** — obligation deadline end of October 2026, several conditional — so it is `NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No`. **IT NEEDS THE RUN MODEL**: two 1.1 rows and four 1.2 rows carry the name in a separate run, so a line-model extractor publishes 16 hospitals and orphans $5,156,000 in silence. **THE FIFTH STATE TO PUBLISH CMS'S OWN NOA** (Revision (Budget), Federal Award Date 05/14/2026, +136 days, budget period still 12/29/2025 — session 36's pin holds a fifth time). **AND A NEW §0.1 DEFECT: RCJ FILES UTAH'S DOCUMENTS UNDER WYOMING** — five of 29, including Utah's $195.7M allotment as an `UNASSIGNED` Wyoming row. Wrong STATE, after wrong programme, tier, action and grain. NOT EXTRACTED — reported first, Arkansas's footing; evidence archived under `data/evidence/WY/`. **HAWAII HAS AWARDED AT LEAD-AGENCY LEVEL AND ITS AWARD SYSTEM IS UNREADABLE**: CMS's and the Governor's 2026-09-01 releases allocate $45M to JABSOM (HOME RUN) and $13M to DOH (ambulances) — lead-agency allocations, Virginia's §0.2 shape, no hospital; SHPDA's own page names ONE award to a non-state entity, *"Notice of Award on 8/28 to Hawaii Primary Care Association"* (FQHC class stated, `NON_HOSPITAL`), whose amount lives only in HANDS — 403 on three agents, UNREADABLE/UNKNOWN (§0.4). Two hospital-facing RVBI items are at RFI stage (8/19, 8/20). 17 of RCJ's 25 Hawaii documents are AHEAD/global-budget material, not RHTP. `data/evidence/HI/` archived. **MASSACHUSETTS IS NEW HAMPSHIRE'S SHAPE**: every mass.gov path is Akamai-403 on four agents, robots.txt included; the Wayback API holds a 2026-08-06 snapshot and web.archive.org still resets the handshake (blocker 7, now with a cost). What can be read — two RCJ proxies (discovery only, 2 API calls), the state's own Key Dates snippet, the 2026-08-03 Athol Daily News — says NOTHING is awarded and contracts are due *"Fall 2026"* via COMMBUYS, which is UNREADABLE. **THE DISPOSITION (TASK 3)**: EXTRACTED 21 / $4,311,042,814; INVESTIGATED_NO_LIST 8 / $1,717,700,768; zero-signal worked but table still `NOT_EXTRACTED` 7 / $1,303,162,979 (HI MA MN NJ SC TN WY — NOT reclassified, because `R/03k`'s definition of `INVESTIGATED_NO_LIST` requires a probe and WY is not a negative at all); RCJ low-candidate queue 14 / $2,668,093,442 holding 34 candidates. Survey and queue REBUILT after stage 00 (never hand-edited): HI `NEITHER` -> `CMS_ONLY`/`QUEUED`, AR -> `CMS_ONLY`, IN -> `BOTH`, so `NEITHER` is now ten states. **THE ZERO-SIGNAL SET IS EXHAUSTED. NO HOSPITAL DOLLAR MOVED ANYWHERE** — `docs/session41_routine_verification_state_audit.md`) (Session 40 — ARKANSAS IS EXTRACTED, AND IT IS THE FIRST STATE IN THIS REPOSITORY WHOSE AWARDS ARE PUBLISHED AT TWO GRAINS BY TWO PUBLISHERS, RECONCILING TO THE CENT. **31 ORGANISATIONS, 37 PRICED AWARD ACTIONS, $149,177,618.45 — 71.5% OF THE $208,779,396 ALLOTMENT.** `arkansasrhtp.com` is a DEDICATED RHTP DOMAIN, the second after Kentucky's and the first that has AWARDED, and Arkansas was invisible to BOTH discovery layers — **ZERO RCJ Tier 3 candidates and no CMS release, `trigger_source = NEITHER`**, which is FLORIDA'S SHAPE a third time after North Carolina and the largest of the three in dollars. **THREE COUNTS, AND NONE IS THE OTHERS: 31 organisations** (DF&A's award-list rows), **37 award actions** (organisation x initiative, the grain Arkansas PRICES and this file's row count — six organisations hold an award under BOTH initiatives), **50 project awards** (the Governor's own count, the grain Arkansas DESCRIBES). Arkansas publishes NO organisation-to-project map, so the three are three different claims and `--report` prints all three. It is 37 rows and not 31 because of **MICHIGAN'S LESSON**: RCJ carried one row per ORGANISATION where MDHHS published one per AWARD and understated the state by $7,833,333. **THE RECONCILIATION IS THE STRONGEST IN THIS REPOSITORY OUTSIDE FLORIDA'S, AND NOTHING WAS ARRANGED** — DF&A published an organisation table and the Governor published a project list, and the two close at THREE LEVELS AT ONCE: the whole round, each initiative's total, and **each of the 37 organisation-initiative pairs**, all to the cent. The Governor rounds the two column totals independently in prose (*"$55.7 million through the ... THRIVE initiative and $93.6 million through the ... PACT priority"*, headline *"$149.3 million"*). **SESSION 32'S RUN MODEL IS WHAT MAKES THE AWARD LIST READABLE AT ALL, AND THE ASSERTION KEEPS IT THAT WAY.** The three amount columns are painted at ONE y, so `rhtp_pdf_lines()` returns **`"$2,571,095.00$0.00$2,571,095.00"`** — Iowa's "Adair County Memorial Hospital Greenfield" one column over and WORSE, because a welded amount string is not a mangled name but an UNPARSEABLE one: a session reading this PDF with the line model extracts NOTHING and reports an empty state. `ar_assert_line_model_merges()` asserts the line model STILL merges (session 35's lesson as code). Nothing thresholds a gap: a test pins that no x boundary could work, because the body amount runs sit between x=297 and x=359 while the HEADER runs sit at 292, 372 and 451. **THE `Total:` ROW IS PAINTED IN THE SAME SHAPE AS AN AWARD ROW** — read as a recipient it gives 32 organisations and **DOUBLE the money**, and the figure still looks like a total because it is one; a test drives the mistake. **WHICH COLUMN IS THRIVE AND WHICH IS PACT IS ESTABLISHED FOUR WAYS, BECAUSE THE ROW IDENTITY SURVIVES AN INVERSION** (addition commutes): the header's own x order, every body row's painted order, **the column totals** (swapping them fails the reconciliation — the only check that can tell $55.7M from $93.6M), and the Governor naming each separately. **AND ARKANSAS'S OWN HEADER CARRIES THE TRAP**: the total column is labelled **"YR1 PACT/THRIVE Total"**, naming PACT FIRST, while the columns beside it run THRIVE then PACT. **§6.2 — THE FOOTER IS DEMOTED ON BOTH GROUNDS THIS PROJECT KNOWS, AND SESSION 26'S NEVADA LESSON IS MEASURED RATHER THAN CITED.** The CMS footer is session 27's WEAK form (*"This **project** is supported by"*) AND prints **$208,779,396.02, WHICH IS THE ALLOTMENT** — Tier 1 wearing a project's grammar, declared `STATE_ALLOTMENT` and checked by `rhtp_assert_footer_not_allotment()`, with a test driving the refusal of a `SOLICITATION` declaration. The same footer sits unchanged on the **RISE and HEART** pages, which have awarded **NOTHING AT ALL**, so a check keyed on "does this page carry the CMS footer" answers YES for both. **AND THE AWARD LIST ITSELF CARRIES NO FOOTER AT ALL** — "Centers for Medicare", "RHTP" and "Rural Health Transformation" occur ZERO times in it (Maine's shape) — so the footer could not have carried this state's provenance even in its strong form. What does: **each NOFO's own header** (*"Funding Opportunity Number: THRIVE YR 1-2026"*, *"Issuing Agency: Arkansas Department of Finance and Administration (DF&A)"*, *"Program Authority: CMS Rural Health Transformation (RHT) Program"*), which ties the exact two initiatives that ARE the award list's column headers to RHTP; the Governor's release (*"recipients of the first round of Rural Health Transformation Program (RHTP) grants"*, quoting the CMS Administrator); and the **Year 1 Revised Budget Narrative**, which places all four initiatives inside the plan (Kansas's second source) and carries no footer because the document IS the plan. The date test passes with room — earliest NOFO opened 2026-05-11, awards announced 2026-08-27, against the 2025-12-29 NOA. **NINE ORGANISATIONS ARE SPELLED DIFFERENTLY BY THE TWO PUBLISHERS, PLUS A TENTH DIFFERENCE THAT IS SYSTEMATIC AND INVISIBLE: THE AWARD LIST PRINTS U+0027 AND THE RELEASE PRINTS U+2019**, so the join fails on every apostrophe-bearing name for a reason neither document shows (session 34's curly apostrophe, load-bearing this time). `AR_RELEASE_SPELLINGS` is a FIXED, HAND-READ map with every entry visible (§2 forbids a fuzzy hospital match), and the assertion requires that after applying it the two documents name the SAME 31 organisations — so a new divergence fails the build instead of dropping a row from the reconciliation. **AND ONE PAIR CLASSIFIES DIFFERENTLY, WORTH $301,400**: *"Arkansas Children's Hospital"* (the award list) is `HOSPITAL_OR_SYSTEM`/HIGH/`DIRECT`/**`Yes`** while *"Arkansas Children's"* (the release) falls to §8's fallback and is `No`. North Carolina's two spellings of UNC moved a CODING at $0; **ARKANSAS'S MOVE A DOLLAR**. The award rows carry the AWARD LIST's spelling (the primary source, §8) and `ar_year1_projects.csv` records BOTH machine answers side by side. **ONE OF THE FIFTY PROJECT AMOUNTS IS PAINTED AS TWO NODES** — `$` and `1,455,689.00` — so a pattern requiring the digits immediately after the dollar sign finds **49 of 50** and drops Arkansas Rural Health Partnership's $1,455,689.00 **IN SILENCE**: the sums simply miss by that much and nothing points at the row. Both patterns are asserted. **THE HOSPITAL FLOOR IS 9 ROWS / $21,792,687.96 AND THE UNSTATED-FORM QUESTION IS THE NINTH INSTANCE AND BY FAR THE LARGEST IN DOLLARS: 16 organisations / 21 award rows / $100,723,693.49, 67.5% OF THE ROUND.** DF&A publishes a recipient and an amount and NOTHING about the form, and no project description either. It is ONE-DIRECTIONAL (every one already `No`), so $21,792,687.96 is a genuine FLOOR and **$122,516,381.45** a genuine CEILING — the largest single classification uncertainty in this repository — and **FOUR OF THE SIX LARGEST AWARDS IN THE STATE ARE INSIDE IT**: Baxter Health $19,738,729.91, Mercy Health Fort Smith $19,056,249, Baptist Health $16,926,432, St. Bernards $14,551,090. **NOTHING WAS PROMOTED (§0.4)**; a test drives the counterfactual and prices it at **$89,106,021.91**. **ARKANSAS RURAL HEALTH PARTNERSHIP ($18,833,521) IS A SECOND AND DIFFERENT QUESTION — §10.2's, NOT §8's**: a hospital CONSORTIUM on this pipeline's knowledge and not on the document's, where §10.2 turns on what the source says the money DOES and the award list says NEITHER. Both rows carry `FLOW_UNRESOLVED_HOSPITAL_AFFILIATED` in NEITHER bucket, queued separately on Michigan's `MI_MHA_FLOW` footing. **THE DESCRIPTIONS WERE READ AND THEY MOVE NO DOLLAR — MEASURED**: feeding the Governor's project descriptions to the classifier moves 11 of the 50 rows `NON_HOSPITAL` -> `IN_KIND_BENEFIT` and NOT ONE DOLLAR into or out of the hospital total, so the reading lives in `ar_year1_projects.csv` (one row = one project = one description) and is deliberately NOT folded into the 37-row file, where a row may span three projects with three descriptions. **A description describes an ACTIVITY and §0.3a judges the RECIPIENT**, which is why typing the 16 from the blurbs is REFUSED rather than left open. **NO NEW VOCABULARY CODE WAS INVENTED (§2)**: the flag for a figure the state says is not final is §8's existing **`AMOUNT_PRELIMINARY`** — Alaska's condition word for word — and a first draft's invented `AMOUNT_NOT_FINAL` was caught by the vocabulary test. **TWO OF FOUR INITIATIVES ARE STILL TO COME, SO THIS IS A PARTIAL YEAR BY CONSTRUCTION**: RISE AR closed 2026-07-24 and HEART 2026-08-07, **NEITHER PUBLISHES AN AWARD DATE** (Missouri's and North Carolina's footing), ~$59.6M is unawarded, and what dates the wait is CMS's own deadline which both NOFOs print themselves — *"all funds must be obligated by October 30, 2026"*. **THE INITIATIVE PAGES CANNOT TELL AWARDED FROM UNAWARDED**: all four carry the IDENTICAL *"What is next? Upcoming announcements ... via Notices of Funding Opportunities (NOFOs)"* sentence, INCLUDING the two that have awarded $149M, and the assertion pins that equality rather than glossing it. **The signal is the HOME PAGE's award-list link** in Arkansas's own words — *"Download the List of Organization and Award amounts"* — which exists for THRIVE/PACT and neither of the other two; `ar_assert_award_index()` fails in BOTH directions and both branches are driven. The eligible class is recorded BEFORE RISE and HEART award: *"rural hospitals, clinics, EMS providers, behavioral health providers, CINs"* — hospitals AMONG OTHERS, New Hampshire's FHC class and NOT Illinois's. **THE EIGHTH ROTATING-DIGEST MECHANISM, AND THE FIRST TIME THE FAILING PAIR WAS MEASURED RATHER THAN REASONED ABOUT.** WordPress stamps a `wp_block_styles_on_demand_placeholder:<13 hex>` token into an inline `<style>` BODY, derived from the RENDER TIMESTAMP: two fetches ten seconds apart are **BYTE-IDENTICAL** and a copy thirty minutes earlier DIFFERS — **at exactly the same byte length**, because the token is fixed-length. So a back-to-back pair is GUARANTEED to report the digest stable and a byte-count check passes it too (session 34's California lesson, and this time the earlier copy was in hand). `ar_reduce_html()` discards `<style>` bodies, the CONTENT digest is identical across all three at 6,572 chars, and the LIVE probe reported all four watched pages UNCHANGED on content **while the file digest MOVED on every one**. **ONLY MAINE'S DIGESTS HAVE EVER HELD.** Arkansas reads `EXTRACTED` in both survey tables, both **REBUILT** from `R/03k`'s constants; `NAMED_HOSPITAL` goes **569 rows / $388,327,247 / 13 states -> 578 / $410,119,935 / 14**, and Arkansas is the first state in three sessions to move the DOLLAR figure rather than only the row count — `docs/session40_arkansas_extraction.md`) (Session 39 — §8 GAINS `MANAGED_CARE_ORGANIZATION`, AND THE ZERO-SIGNAL STATES PRODUCE ARKANSAS: A $149M PRICED ROSTER NOBODY HAD LOOKED FOR. **TASK 1 — THE FIRST §8 CODE ADDED BECAUSE THE SOURCE STATES A FORM §8 DOES NOT CARRY.** Eight states publish a recipient and say NOTHING about its form, and §8's standing answer is `NONPROFIT_CBO` + `LOW` + `RECIPIENT_TYPE_INFERRED`. North Carolina STATES it, and leaving those rows on the fallback asserts the form is UNDETERMINED where the state has stated it outright — the one thing that flag's own note forbids. Added on session 10's `PHYSICIAN_PRACTICE` footing, written into `vocabularies.csv` with full notes and mirrored into the spec, this file and the reviewer instructions by PATCH, INSERT-ONLY (§2.1). **THE TASK SAID THREE ROWS AND IT IS TWO, WHICH IS THE POINT.** NCDHHS calls Trillium *"an NC Medicaid Tailored Plan and Managed Care Organization (MCO)"* and Vaya *"a public NC Medicaid Managed Care Organization (MCO)"* — both now typed from the source at MEDIUM. **ACCESS EAST IS NOT AN MCO**: its stated form is *"a comprehensive care management provider"*, a different thing in North Carolina's own Medicaid vocabulary, and widening a code ON THE DAY IT IS ADDED to a form its source does not state is §0.4's failure in miniature — so it keeps the fallback and **the queue row went from THREE recipients to ONE, not to zero**. A test reads the archive and requires that NCDHHS still does NOT call it an MCO, and that *"Managed Care Organization (MCO)"* occurs there exactly TWICE. **NO DOLLAR AND NO ROW COUNT MOVED**: an MCO is not a hospital type, all five Hub Leads stay `PASS_THROUGH_UNRESOLVED` + `Unclear` in NEITHER bucket, `nc_year1_awardees.csv` changed on exactly TWO ROWS, and the partition still returns NO BUCKET AT ALL for North Carolina. **TASK 2 — THE FIVE LARGEST ZERO-RCJ-SIGNAL STATES, AND THE GROUP IS NOW 3 OF 13 WITH A PUBLISHED ROSTER.** All five hold RCJ records and ZERO Tier 3 candidates, and **`/api/v1/activity` FOUND THE ROUTE A SEVENTH TIME** — `state_source_url` is NA on every one of their records and `stage2_state_sources.rds` held 49 real state URLs. **ARKANSAS IS THE LARGEST FIND SINCE OREGON AND IS DELIBERATELY NOT EXTRACTED (the task said report first): 31 ORGANISATIONS, $149,177,618.45, 71.5% OF THE ALLOTMENT**, on `arkansasrhtp.com` — a DEDICATED RHTP DOMAIN, the second after Kentucky's and the first that has AWARDED — under Arkansas's own words *"Download the List of Organization and Award amounts"*. **IT RECONCILES TO THE CENT** against the PDF's own `Total:` row on all three columns, and **A SECOND PUBLISHER CARRIES THE SAME ROUND**: the Governor's 2026-08-27 release says *"$149.3 million"* and *"the 50 project awards"*, so 31 organisations hold 50 projects. TWO of four initiatives remain (RISE, HEART) against *"the $209 million the state expects to award BY THIS FALL"*. **SESSION 32'S RUNS MODEL READS IT DIRECTLY** — the line model alone merges the three amount columns into `"$2,571,095.00$0.00$2,571,095.00"`. Sized but not written: 9 named hospitals / $21,792,688 against **16 rows / $100,723,693 — 67.5% of the round — on §8's standing fallback**, the ninth instance of Kansas's shape and BY FAR THE LARGEST IN DOLLARS, running strongly UPWARD (Baxter Health $19.7M, Mercy Health Fort Smith $19.1M, Baptist Health $16.9M, St. Bernards $14.6M, and Arkansas Rural Health Partnership $18.8M is a hospital CONSORTIUM, §10.2's association row). NOTHING PROMOTED (§0.4). **SOUTH CAROLINA HAS AWARDED AND TOLD THE RECIPIENTS BY EMAIL — A SHAPE THIS PROJECT HAD NOT MET.** Its RHTP page published *"Anticipated Notice of Award — July 31, 2026"* and **it hit that date exactly**: bulletin MB# 26-026 says *"Today, SCDHHS issued Notices of Award Determination for the **712 applications** received ... **Applicants should check their email**"*. **NO ROSTER, NO AMOUNTS, NOT EVEN A COUNT OF AWARDS** — South Dakota published a count and a total with no names; South Carolina publishes only a count of APPLICATIONS. Its CMS footer is the STRONG form and its figure is the ALLOTMENT ($200,030,252.32, §0.2). **THE POSITIVE CONTROL AND THE §0.1 TRAP ARE ON THE SAME PAGE TWICE OVER**: the Rural & Medically Underserved Area Grant ($48.2M, roster LINKED, awarded 2024-02-02) and the Behavioral Health Crisis Stabilization Grant (~$35M **TO HOSPITALS**, roster linked, awarded 2023-06-23) — California's SRHRP trap in duplicate, and **§6.2's date test disposes of both on its own**, 18–30 months before the NOA. SCRA is a §7 designated pass-through (*"SCDHHS has contracted with SCRA to manage and administer the Tech Catalyst Fund"*) and has not awarded. **MINNESOTA IS A NEGATIVE WITH THE MOST HOSPITAL-WEIGHTED ALLOCATION IN THE PROJECT**: an Entity Type table giving **Hospitals 70% of the award, 94 ELIGIBLE ENTITIES**, FQHCs 5%/5, CCBHCs 2%/16, Tribal Nations 2%/10, as ***"formula-based, non-competitive grants"*** — ~$135.2M, and **§0.3 AT ITS MOST TEMPTING**, because a formula allocation to a pre-identified class reads as though the eligible list IS the award list. MDH has published NEITHER the 94 NOR any award; the deadline was extended to 2026-05-26 and has passed. **TENNESSEE IS A NEGATIVE AT SOLICITATION STAGE WHOSE AWARD CHANNEL IS UNREADABLE** — the Caspio RHTP Partner Portal returns *"You need to enable JavaScript to run this app"* (Maine/Connecticut/Louisiana a fourth time, UNKNOWN not absent, §0.4) — with a clean POSITIVE CONTROL: TDH publishes named awards in a recognisable form (2026-06-15, a chronic-disease programme) while all three of its RHTP items are OPPORTUNITIES. **NEW JERSEY IS THE THINNEST RHTP PRESENCE IN THE PROJECT**: its SORH page, last reviewed 2026-08-06, still says the state *"applied for"* the funding and never mentions the $147,250,806 it was awarded, and its full FY2026-27 Directory of Grant Programs contains *"rural"* **ZERO TIMES**. **NOTHING WAS EXTRACTED AND NO HOSPITAL DOLLAR MOVED ANYWHERE** — `docs/session39_managed_care_code_five_zero_signal_states.md`) (Session 38 — NORTH CAROLINA IS EXTRACTED, THREE STATES ARE ON SCHEDULES, AND §10.2 GAINS A THIRD ELIGIBLE CLASS. **THE THREE ROUTINES, EVERY SLOT OFFSET FROM THE NINE ALREADY RUNNING** (which occupied 12:50 through 18:40; these are 10:50, 11:10 and 19:30, outside that range at both ends): **NEW YORK** `trig_01G2ZijwemE2YgbAuiBirKs2`, **SUNDAYS AND WEDNESDAYS 11:10 UTC** — the tightest clock in the project, because RCHI's contracts were to BEGIN 2026-09-01 and every RHTP contract must be EXECUTED by 2026-10-30, and Sunday carried no Routine at all; **KENTUCKY** `trig_01FqfAJvnMzz811goNzNZYqi`, **MONDAYS AND THURSDAYS 19:30 UTC** — twice-weekly on Wisconsin's and Connecticut's footing, because Kentucky published TWO award-notification dates itself and both have passed; **NORTH CAROLINA** `trig_01SepuwJWXSkqpNQ9WgJkxMb`, **SATURDAYS 10:50 UTC** — weekly, and THE REASON IS STATED: it is the only one of the three that has published NO date for what is being watched, which is Missouri's and New Mexico's footing. **`nc_probe()` HAD TO BE WRITTEN FIRST** and follows Missouri's and Wisconsin's pattern — the tripwires run against the LIVE bytes, not the archive (session 25's Indiana lesson), and it ran live reporting UNCHANGED on all five pages. **NORTH CAROLINA IS EXTRACTED: 44 NAMED RECIPIENTS ACROSS TWO ROSTERS AND `amount` EMPTY ON ALL 44** — 39 Mobile Integrated Health recipients ($10,000,000, a POOL figure) and 5 NC ROOTS Hub Leads (NO figure at all). **AND IT INVERTS THE PAIRING NEVADA AND IOWA ESTABLISHED.** Those two publish named HOSPITALS with no amounts, so their danger is quoting the $0 without the row count; North Carolina's 44 are EMS agencies and regional pass-through leads, so **it contributes 0 rows and $0 to EVERY bucket** — Maine's and Missouri's shape, and **A THIRD DISTINCT WAY A STATE REACHES $0** (Nevada/Iowa: named hospitals, no amounts; Maine/Missouri: named hospitals OUTSIDE the award file; **North Carolina: a complete award file containing no hospital at all**). `nc_assert_row_count_is_the_finding()` asserts all three halves at once and its tests drive the mistake in the order it is made — first the $10,000,000 divided over 39 rows, then Cape Fear Valley promoted. **SESSION 37'S "38 OF 39" IS REALLY 37, AND THE EXTRACTION IS WHAT FOUND IT**: TWO names carry no EMS token, not one — **Cape Fear Valley Mobile Integrated Health** and **Clay County**, which the release prints WITHOUT the "EMS" its thirty-eight siblings carry. **NEITHER WAS PROMOTED (§0.4)**: session 37 called Cape Fear Valley "the one hospital-affiliated recipient" and that is THIS PIPELINE'S OWN KNOWLEDGE, not the document's — the archive says nothing about it beyond the name, while NCDHHS's own sentence calls all 39 *"local EMS agencies"* and its release describes *"EMS-led Mobile Integrated Health programs"*. Both take §8's standing fallback and are QUEUED, worth **$0 either way**, moving only the state's named-hospital ROW COUNT — which for an unpriced state is the only hospital quantity there is. **THE TWO SPELLINGS OF ONE HUB LEAD CLASSIFY DIFFERENTLY, WHICH IS SHARPER THAN A COUNTING PROBLEM**: *"University of North Carolina Hospitals"* (the release) hits §8's hospital name rule — `HOSPITAL_OR_SYSTEM`, HIGH, `DIRECT`, **`Yes`**, A NAMED-HOSPITAL ROW — while *"UNC Health"* (the standing page) hits nothing and falls to the fallback, `No`. **NEITHER MACHINE ANSWER IS USED**: NCDHHS's own page states the form, *"a public academic medical center ... With more than 1,000 beds"*, and §8's code for an academic health centre is `UNIVERSITY_OR_AHC` (Oregon's OHSU precedent), which can only keep dollars OUT. `nc_assert_unc_two_spellings()` ASSERTS THE DIVERGENCE RATHER THAN REPAIRING IT. **AND THREE HUB LEADS RAISE A CONDITION THIS PROJECT HAD NOT RECORDED: THE SOURCE STATES A FORM §8 DOES NOT CARRY** — *"Managed Care Organization"* (Trillium, Vaya) and *"a comprehensive care management provider"* (Access East). That is NOT the unstated form eight other states raise; the state HAS stated it and §8 has no code for the answer. **NO CODE WAS INVENTED (§2)**: the three keep the fallback with the state's own sentence in `recipient_type_source`, queued at $0. The two whose stated form §8 DOES carry were typed from the source. **THE HUB LEADS ARE NOT MISSOURI'S HUB ANCHORS** — *"programmatic and **fiduciary** leads"* against *"will not act as the fiscal agent"* — so they ARE recipients, and all five are `PASS_THROUGH_UNRESOLVED` + `Unclear` in NEITHER bucket, because the only published description of a Hub Lead's network is hospitals AMONG OTHERS (New Hampshire's FHC class). **§0.2 IS WRITTEN INTO THE DATA**: the ROOTS rows' `round_amount` is **`NA`**, because the only currency on either ROOTS document is the **$213,008,356.47 ALLOTMENT** and filling the empty column with the only number available publishes the whole state award as five hub awards; a test fills it and requires the throw. The MIH pool carries **GEORGIA'S TRAP** — $10,000,000 repeated 39 times, so the column sums to **$390,000,000**. **§10.2 GAINS A THIRD ELIGIBLE CLASS AND IT IS NEW YORK'S.** ICAHN is `Yes` (hospitals only); FHC is `Unclear` (hospitals among others). RCHI says *"A **hospital must be included** as either the lead applicant or the partner Organization"* — MANDATORY in every award and NOT NECESSARILY THE RECIPIENT. **It is not ICAHN's `Yes`**, because the dollar may go to a non-hospital lead. **And it is not `Unclear` for FHC's reason**, which is the half most easily lost: FHC is Unclear because a hospital MIGHT be among the recipients, while New York's rule is stronger than "might" — a hospital is present in every awarded partnership, by rule, knowable in advance, so FHC's sentence *"we cannot say a hospital is involved"* is simply FALSE here. What remains unknown is narrower: **whether any dollar reaches the hospital that had to be in the room.** **A REQUIRED PARTNER IS NOT A RECIPIENT — PARTICIPATION IS NOT RECEIPT, exactly as eligibility is not receipt (§0.3) — and the coding is read off the AWARD, one award at a time, never off the rule.** Patched INSERT-ONLY into all three documents (47 lines each, nothing deleted) and `test_flow_table_parity.R` now asserts BYTE PARITY on it too. **NO HOSPITAL DOLLAR MOVED ANYWHERE** — `docs/session38_nc_extraction_ny_class.md`) (Session 37 — THE CMS FOOTER'S TIER IS NOT RECOVERABLE FROM ITS GRAMMAR, AND THE TWELVE NO-SIGNAL STATES HAVE BEGUN. **§0.2 GAINS A SECOND WORKED EXAMPLE AND IT GOVERNS MACHINE READING.** Virginia is two tiers on one page in prose a reader can weigh; **IOWA IS TWO TIERS IN THE SAME SENTENCE OF THE SAME BOILERPLATE**, from one agency. Its three footers read *"This **Best and Brightest- Medical Equipment Procurement** is supported by … totaling approximately **$66,002,161.80**"*, *"This **Centers of Excellence** … **$50,000,000.00**"* and *"This **Combat Cancer Health Hub Program** … **$209,040,063.71**"* — the first two Tier 2 pools, the third Kentucky-sized Tier 1, and the sentences are **WORD FOR WORD IDENTICAL apart from the programme name and the number**. All three are session 27's **STRONG, programme-scoped form**; all three name a real Iowa RHTP programme. **SESSION 27'S AXIS ANSWERS PROVENANCE AND NOT TIER, AND THE TWO QUESTIONS ARE INDEPENDENT.** Tier 1 is knowable **ONLY BY COLLISION WITH THE §7.1 ANCHOR** — a coincidence of VALUE, not a statement by the publisher — so **THE DEFAULT IS REFUSAL**: `rhtp_assert_footer_not_allotment()` / `rhtp_assert_footer_tiers()` (`R/utils_config.R`) refuse any footer declared `SOLICITATION` within **$10,000** of that state's allotment **AND** any declared `STATE_ALLOTMENT` that stops colliding. The margin is measured rounding (Iowa to the dollar, NH to the cent, Kansas's $8,000.18 transposition) and is **600× below** the smallest genuine Tier 2 footer here, $6,000,000. A missing anchor returns **NA, never TRUE** (§0.4). **IT CAUGHT LIVE EXPOSURES IN THREE OF THE FOUR STATES TOUCHED THIS SESSION**: Kentucky attaches CMS's OWN Notice of Award to **ALL NINE** of its RFAs, so each ships a document whose only figure is the allotment — against a stated **$800,000 maximum award**, a **266× overstatement**; North Carolina's ROOTS page names **FIVE AWARDEES** and contains **exactly one currency figure**, the allotment; New York's programme page carries one figure against a **$76,190,022** pool. Iowa's eleven are wired and pass; **New Hampshire's two hand-coded footers were checked against the machine rule and AGREE**. **TASK 1 — IOWA'S TIER 2 NOTE SAID "four" AND THE ANSWER IS THREE**: session 32 corrected the split to 8/3 and moved every assertion, missing the one place the number was PROSE in a `paste()`. Now DERIVED from `IA_NOTICES`; sum and allotment render byte-identical, so the only CSV change is the word. **AND THE REBUILD EXPOSED A SECOND STALENESS NOBODY LOOKED FOR**: `ia_rcj_candidate_disposition.csv` said *"all four June ones"* while its own source line already said three — a committed reference table out of step with the code that generates it. **THE TWELVE NO-SIGNAL STATES BEGIN, AND `/api/v1/activity` FOUND THE ROUTE A SIXTH TIME.** All three hold RCJ records (64/53/21) and **ZERO Tier 3 candidates**. **KENTUCKY IS A NEGATIVE THAT NAMES ITS OWN AWARD DATE TWICE, BOTH PASSED**: `ruralhealthplan.ky.gov` is a **DEDICATED RHTP DOMAIN no other state has**, carrying **NINE RFAs** with *awarded*/*awardee*/*recipients*/*selected* at **ZERO each**, and its RFAs print *"July 10, 2026: **Notification of Award to Grantees**"* and *"August 26, 2026 Anticipated **Notification of Award to Recipients**"*. **§6.2 PASSES ON CMS'S OWN NOA — THE FOURTH STATE — AND IT IS THE FIRST THAT IS AN ORIGINAL**: Action Type **"New"**, Federal Award Date **12/29/2025**, equal to its budget period start, where NV/CA/CT are all *"Revision (Budget)"* at +52/+92/+206 days. **THE FIRST DIRECT OBSERVATION THAT THE TWO FIELDS AGREE WHEN THERE HAS BEEN NO REVISION**, which is what session 36 argued from three revised documents and could not observe. **A FALSE POSITIVE WAS CAUGHT WRITING THAT**: the NOA's terms instruct the recipient to *"utilize Revision (Budget) amendment type"* — boilerplate on EVERY NOA — so a whole-document search reports every NOA as a revision; the check is scoped to the header and a test asserts the phrase DOES occur later. **AND THE OBVIOUS SEARCH IS POISONED**: *"Notice of Award"* occurs **TEN times** on Kentucky's funding pages and **NOT ONE is a state award** — every one is CMS's, shipped as an attachment — so it is deliberately EXCLUDED from the tripwire. **NEW YORK'S CONTRACT START PASSED THE DAY BEFORE THIS RAN**: RCHI allocates **$76,190,022** and its guidance says *"Contracts for funded grantees will begin on **September 1, 2026**"*; DOH's own 2026-08-12 deck says *"**91 Applications, $156,000,000 total request**"* and *"**Reviews and Funding Recommendation In Progress**"* — §0.3 in the state's own numbers, **OVERSUBSCRIBED TWO TO ONE**. Connecticut's passed date was an announcement, Louisiana's were windows, Kentucky's are notifications; **NEW YORK'S IS A CONTRACT START**, one step further down. **AND ITS ELIGIBLE CLASS IS NEW TO THIS REPOSITORY AND THE STRONGEST YET**: *"A **hospital must be included** as either the lead applicant or the partner Organization"* — **HOSPITALS ARE MANDATORY IN EVERY AWARD AND NEED NOT BE THE RECIPIENT** (the lead may be a 501(c)(3)), which is neither Illinois's *hospitals only* (`Yes`) nor New Hampshire's *among others* (`Unclear`). §0.3 with the trap one step closer; read it off the AWARD, never off the rule. **NORTH CAROLINA IS NOT A NEGATIVE AND IS DELIBERATELY NOT EXTRACTED.** It is **THE SECOND FLORIDA** — invisible to both discovery layers, largest allotment in the group ($213,008,356) — and has published **TWO NAMED ROSTERS: 39 Mobile Integrated Health recipients ($10,000,000) and 5 NC ROOTS Hub Leads**, 44 named recipients and **NOT ONE PER-RECIPIENT DOLLAR**. **38 of the 39 are county EMS agencies**; **Cape Fear Valley** is the one hospital-affiliated recipient. **THE HUB LEADS ARE NOT MISSOURI'S HUB ANCHORS AND ONE WORD DECIDES IT**: NCDHHS calls them *"programmatic and **FIDUCIARY** leads"* where Missouri's FAQ says its Anchors *"will not act as the fiscal agent"* — so these five ARE pass-through recipients. **UNC appears under TWO SPELLINGS** across two documents from one agency (§2 forbids merging), and **contracts were to be finalised by 2026-06-01**, which has passed. **THE SECOND TIER — where the hospital money is — NAMES NOBODY.** **TWO MORE DIGEST MECHANISMS, THE EIGHTH AND NINTH**: Kentucky's SharePoint re-rolls **CssLink GUIDs AND `__VIEWSTATE`** at once (3 fetches, 3 digests; byte count DOES move, unlike CA's and LA's), and **NCDHHS injects a DYNATRACE `rpid`** per request — 4 fetches, 3 digests, **AND FETCHES 1 AND 2 IDENTICAL**, the first time this project has caught the pair-invisible case with the failing pair in hand (session 34's California lesson, a fourth time). Both are attribute-borne and absorbed free. **KY and NY read `INVESTIGATED_NO_LIST`** (6 states → **8**), rebuilt from `R/03k`'s constants; **NC stays `NOT_EXTRACTED`, which is TRUE**, because `INVESTIGATED_NO_LIST` would be a false claim about a state with two rosters. All three probes ran LIVE and report UNCHANGED. **NO HOSPITAL DOLLAR MOVED: none of the three contributes a row or a dollar to any bucket** — `docs/session37_footer_tier_ky_ny_nc.md`) (Session 36 — CONNECTICUT AND NEW MEXICO ARE ON WATCHES, THE §6.2 DATE TEST IS PINNED TO THE BUDGET PERIOD, AND LOUISIANA IS A NEGATIVE WHOSE SEVEN ANNOUNCEMENT WINDOWS HAVE ALL CLOSED. **THE WATCHES**: Connecticut `trig_01DRHN1x1WfzPjC3dpgay8s2`, **MONDAYS AND THURSDAYS 15:40 UTC** (twice-weekly — its award date PASSED 2026-08-17); New Mexico `trig_01DbiFhMJhaZXzYB4gi5JpwX`, **TUESDAYS 18:40 UTC** (weekly, and the reason is STATED: Wisconsin and Connecticut earned twice-weekly because their states published an award DATE, and New Mexico publishes NONE — four procurements read *"Currently under evaluation"* and one *"TBD"*; a submissions deadline is the start of an evaluation, not an award, so this is Missouri's footing). Both probes ran LIVE FIRST and report UNCHANGED. **THE §6.2 DATE TEST WAS ALREADY CORRECT AND IS NOW DEFENDED.** The anchor is **2025-12-29** for all 50 states, parsed from CMS's own announcement, and `R/02b` OPENS NO STATE NOA PDF AT ALL — so the "Federal Award Date" field has never reached it. What was missing is the reason: `rhtp_build_noa_dates()`'s docstring said *"a state that later proves to have a different date is a data correction and not a code change"*, which invites exactly the wrong correction now that three states publish CMS's own NOA. **THREE OF THREE CARRY A LATER FEDERAL AWARD DATE, ALL "Revision (Budget)"** — NV `-01-02` 02/19/2026 (+52d), CA `-01-02` 03/31/2026 (+92d), CT **`-01-03` 07/23/2026 (+206d)** — against a budget period still starting **12/29/2025** on every one. The field is the date of the LATEST REVISION; **CONNECTICUT IS ON ITS SECOND**, which is why its gap is widest, so **THE ERROR GROWS WITH EVERY REVISION** and every future state NOA will carry a wider one. **MEASURED, NOT ASSUMED**: keying on the later date stops Connecticut's build in two places, and the first reads *"NOFO #26OHS001 no longer postdates Connecticut's Notice of Award"* — a defect in the anchor that presents as a finding about Connecticut — while two dated CT candidates (2026-04-07, $1,500,000 and $3,800,000) quarantine as `PROVENANCE_PREDATES_NOA`. Four tests read all three archived NOAs and drive the counterfactual, POSITIVE-CONTROLLED (moving the committed anchor to 2026-07-23 fails 19 tests; the CSV was restored byte-identical). Insert-only, no data moved. **THE QUEUE IS NEARLY EXHAUSTED AS A DISCOVERY SIGNAL, AND THE NUMBERS SAY SO**: 25 states worked (1,332 candidates), **14 states open holding 34 candidates between them, the largest FIVE ROWS** (DE 5, AZ 4, CO 4, WV 4, MS 3, RI 3, then eight at 2 or 1) — and **TWELVE STATES CARRY NO RCJ SIGNAL AT ALL, HOLDING $2.36 BILLION OF ALLOTMENT** (NC $213.0M, KY $212.9M, NY $212.1M, FL, AR, TN, WY, SC, MN, HI, MA, NJ), eleven of them `NOT_EXTRACTED` with no CMS release either. **FLORIDA IS THE TWELFTH AND IT IS THE PROOF**: no CMS release, no RCJ candidate, and **81 extracted awards worth $188,201,256** already here, reconciled exactly against the Governor's own PDF. The next phase is states with NO SIGNAL, not the queue's tail. **LOUISIANA LED THAT QUEUE AND IS A NEGATIVE WITH SEVEN CLOSED WINDOWS — the tightest clock in this project.** LDH's programme page carries an *"IMPORTANT DATES - BUDGET YEAR 1"* block giving a **"Notice of Intent to Contract Announcements"** window for EVERY one of its seven Budget Year 1 opportunities — *"Late July to mid August"* **x3** and *"Mid to late August"* **x4**, 3+4=7 matching the funding page's seven *"Strategic Funding Opportunity Title"* headings exactly — and **ALL SEVEN CLOSED BEFORE THIS RAN**. Connecticut was the first negative here whose award date had passed, with ONE solicitation; **LOUISIANA HAS SEVEN, AND THE STATE PUBLISHED THE DATES ITSELF**. **THE ADVISORY COUNCIL DECK IS SHARPER AND IT IS RCJ'S SOURCE**: slide 18, headed *"RHTP Funding Cycle Budget Year 1"*, columns **Activity | # Applications Received | PROJECTED BY 1 Funding | Anticipated Announcement** — Rural Clinician Credit Bank 136/$10M, **Capital Improvement 160/$41.60M**, Telehealth 79/$4.71M, Collaborative Provider 37/$3M, APM 31/$30M, Care Conveners 25/$3.5M, Food is Medicine 37/$2.7M. **505 APPLICATIONS RECEIVED, NOT ONE AWARD NAMED** (§0.3 in the state's own table). **§0.1 IS OKLAHOMA'S TIER DEFECT WITH THE STATE'S OWN HEADING REFUTING IT**: all six RCJ candidates are rows of that ONE table — the "awardee" is the **ACTIVITY** column (fund uses, not organisations, so §6.1's `PROGRAM_NAME_AS_AWARDEE` on **SIX OF SIX**, and `named_recipient_test` reads **PASS** on every one) and the amount is the **PROJECTED** column times a million. Oklahoma's reports had to be read to their glossary; **Louisiana says PROJECTED in the heading**. **AND RCJ DROPS THE LARGEST ROW** — Capital Improvement, 160 applications (the most), $41.60M (the largest), awards of $100,000–$10,000,000, **THE CAPITAL ONE LIKELIEST TO REACH A HOSPITAL** — so its six sum to $53,910,000 against the deck's seven at **$95,510,000**, understating the table it mined by **$41,600,000**. Michigan deflated one row per ORGANISATION; Louisiana deflates by DROPPING A ROW. **NOT ONE candidate is a named Louisiana organisation of any kind**, so unlike California and New Mexico there is no named-hospital exposure at all. **THE POOL TO WATCH IS CAPITAL IMPROVEMENT** and its class is **HOSPITALS AMONG OTHERS** (*"Critical Access Hospitals (CAHs), Rural hospitals"* beside RHCs, FQHCs, EMS, behavioral health and independent rural practices) — New Hampshire's FHC class, NOT Illinois's ICAHN class. **THE POSITIVE CONTROL IS UNUSUALLY CHEAP AND STRONG: Louisiana names the FORM its announcement will take and the WINDOW it will take it in**, so the silence is measured against the state's own stated intention — **and even what it has PROMISED is not a roster**, the deck's next-steps slide offering only *"a complete list of obligated funds **by entity type**"*. **THE §0.3 CONTROL IS MACHINE-READABLE AND THREE TIMES CALIFORNIA'S**: `rhtla.net/api/facilities` is **3,576 named Louisiana facilities including 305 HOSPITALS**, with addresses and coordinates, on the programme's own second domain, and *"award"*, *"amount"*, *"$"*, *"RHTP"* and *"fund"* occur **ZERO TIMES EACH** — the dataset behind the *"Is my location rural?"* tool. **ONE HOST IS UNREADABLE**: `rhtla.net`'s landing page serves an unrendered Vue application (`{{ copy.brand }}` arrives as literal text), Maine's CGI Advantage and Connecticut's CTsource in a new costume, so it reads **UNKNOWN** (§0.4). **THE HOST WANTS A `Mozilla/5.0` PREFIX AND THAT IS NOT MICHIGAN'S EXCEPTION**: the project's own agent **403**, bare `Mozilla/5.0` **403**, the RFC convention *"Mozilla/5.0 (compatible; AHA-RHTP-Tracker/0.1; +https://www.aha.org)"* **200** — the form that works CARRIES OUR NAME AND CONTACT URL, so it is session 10's medicaid.gov answer (identifying honestly IS the fix) and not session 27's inversion, and `robots.txt` is **404, genuinely absent**, where Michigan's is 403. `la_agent_for()` refuses any agent lacking both tokens on ANY host, with a test. **THE SEVENTH DIGEST MECHANISM, AND THE SECOND THAT IS ATTRIBUTE-BORNE**: Cloudflare **Email Address Obfuscation** XOR-encodes a mailto with a **RANDOM ONE-BYTE KEY on every render** into an `href` and a `data-cfemail` attribute — three fetches, THREE distinct digests, **EXACTLY 169,500 bytes every time**. California's `antispambot()` on another platform and CONSTANT-LENGTH like it, so a byte-count check passes it; attribute-borne like Connecticut's `?v=` stamp, so the tag-stripping reduction absorbs it free (reduced text identical at 21,249 chars). **ONE PARSE DEFECT FOUND AND FIXED**: the deck's column headings and first data row are painted contiguously, so a lazy capture swallowed *"Anticipated Announcement"* into row 1's activity — fixed with a lookbehind, asserted. And the name match normalises to **LETTERS ONLY**, because the producer paints *"Food isMedicine"* and *"Rural Clinician CreditBank"* (session 32's run-spacing finding). Louisiana is on Routine `trig_01BqyRZ4d8S8EvKdtCN9nmX5`, **WEDNESDAYS AND SATURDAYS 12:50 UTC**; it ran live and reports UNCHANGED, reads `INVESTIGATED_NO_LIST` in both rebuilt tables, and **CONTRIBUTES NO ROW AND NO DOLLAR TO ANY BUCKET** — `docs/session36_ct_nm_watch_date_test_louisiana.md`) (Session 35 — CALIFORNIA IS ON A WEDNESDAY/SATURDAY WATCH, CONNECTICUT'S AWARD DATE PASSED SIXTEEN DAYS AGO, AND NEW MEXICO STACKS THREE DEFECTS IN ONE CANDIDATE SET. **CALIFORNIA**: Routine `trig_013vujTBLopT2gSmNwWJ94ig`, **WEDNESDAYS AND SATURDAYS 17:00 UTC** — the probe ran LIVE first and reports UNCHANGED on all three pages with the tripwires passing, and the cadence is offset from Wisconsin's and Maine's Tue/Fri so the four watches spread across the week. Its prompt carries the three things a future session would otherwise re-derive: that the FILE DIGEST IS KNOWN USELESS on hcai.ca.gov and must not be "simplified" back, that every CalRHT pool is hospitals AMONG OTHERS (§0.3), and that the SRHRP must not be let back in. **`R/03ab`'S HEADER WAS CORRECTED**: it still claimed the host's digests were STABLE and a file digest was the change test — the wrong claim session 34 made and then caught, with `ca_probe()`'s own docstring already carrying the correction. A header that says the opposite of the code it heads is §2.1's hazard; the CA CSVs rebuild BYTE-IDENTICAL. **CONNECTICUT IS THE FIRST NEGATIVE IN THIS PROJECT WHOSE AWARD DATE IS ALREADY IN THE PAST.** DSS leads **$154,249,105.53** across **30 projects** and its Documents page carries EIGHT documents, NONE a roster. OHS's **NOFO #26OHS001** ($1.8M, up to 5 awards of $100k–$1M) closed to applications **2026-07-07**, and OPM's own timeline gives *"Grant Awards Announced ... **AUGUST 17, 2026**"* — **SIXTEEN DAYS BEFORE THIS RAN**, no roster anywhere reachable. Wisconsin's date was a month out, California's was inside the fortnight; **Connecticut's has gone by**. The state says it itself: *"Parties interested in being subrecipients ... are encouraged to **check back**"*. **§6.2 PASSES ON CMS'S OWN NOTICE OF AWARD, THE THIRD STATE AFTER NEVADA AND CALIFORNIA** — and it carries **CALIFORNIA'S TWO-DATES TRAP A SECOND TIME AND WIDER**: Federal Award Date **07/23/2026**, action type **"Revision (Budget)"**, budget period still **12/29/2025**, so a date test keyed on the later date reads Connecticut's award as **SEVEN MONTHS LATE** (California's was three) and would quarantine every genuine Connecticut row. **ONE CLOSURE, UNARRANGED**: the NOA names Daniel Mize Sinclair and Julie Vigil, and DSS's own 2026-04-10 release names the same two in the same roles. **§0.1 IS OKLAHOMA'S TIER DEFECT PLUS A MECHANISM NOT PREVIOUSLY RECORDED**: all seven candidates are BUDGET-NARRATIVE LINE ITEMS whose awardees are the administering agencies (DPH $21,714,915, OHS $7,689,978, DMHAS TWICE) or budget COLUMN HEADINGS (Carelon as *"Contractor 1"*, AHEC as *"Proposal: W03-..."*) — and **RCJ PRICES DOCUMENT REVISIONS AS SEPARATE AWARDS**, carrying Carelon's ONE $3,800,000 line twice and DMHAS's one line at **TWO DIFFERENT AMOUNTS** ($5,749,236 and $5,600,000). New Hampshire's CDFA defect caused by REVISION rather than spelling, and cleaner because the underlying document is provably one line. The seven sum to **$49,854,129**, the survey's own figure, ASSERTED. The narrative says it is a plan in its own words: *"Personnel salaries will be updated **once awarded**"*. **NOT ONE Connecticut candidate is a named hospital**, so its $0 is not at risk from a name-keyed read. **THE CONTROLS**: OHS publishes NAMED decisions (*"Approves UCONN Health Affiliate's Acquisition of Waterbury Hospital"*) so the silence is the programme's; DSS's ONLY RHTP release announces a **LEADERSHIP TEAM of four PEOPLE** — Missouri's governance roster one tier further from money; and **CTsource is UNREADABLE/UNKNOWN** (§0.4, Maine's precedent), with `biznet.ct.gov` 403. **THE FIFTH DIGEST MECHANISM, AND THE FIRST THAT IS PER-NODE**: `portal.ct.gov` stamps a `?v=<yyyymmddHHMMSS>` asset version that is the SERVING NODE'S build time — six fetches, **80,531 bytes every time, FIVE distinct digests, one repeating**. **AND IT SHARPENS CALIFORNIA'S LESSON RATHER THAN REPEATING IT**: a back-to-back pair across TWO PAGES OF ONE HOST gave **SAME on one and DIFFER on the other in the same minute**, so a "SAME" result is not evidence of stability even for the page it was run on. The reduction discards ATTRIBUTES, so it absorbs this for free (8,983 chars identical across all six). **NEW MEXICO IS THE FIRST CANDIDATE SET TO STACK THREE RECORDED DEFECTS AT ONCE.** HCA has opened **SIX** procurements and every one is pre-award in HCA's own words — four *"Currently under evaluation"*, Rooted in New Mexico *"Submissions due: **September 4, 2026**"* (TWO DAYS AFTER THIS RAN), the Data Hub *"TBD"*. **NOT ONE NAMED RECIPIENT.** All seven candidates are the **Rural Health Care Delivery Fund**, and they fail three ways: **(1) THE WRONG PROGRAMME** — the Governor's own release says *"$50 million **in state funding**"*, a fund *"originally established in **2023**"* topped up *"during the **October 2025 special session**"* (a state appropriation predating the NOA), and **HCA's own RFA deck calls it *"a $50 million STATE investment"* while containing RHTP / "Rural Health Transformation" / CMS / "federal" ZERO TIMES EACH** — the strongest form of the negative, because that deck is the document RCJ sourced the rows from. HCA's site architecture agrees: the RHCDF sits under the Primary Care Council, its page's THREE mentions of "Rural Health Transformation" are **ALL THE NAVIGATION MENU**, it carries **NO CMS FOOTER AT ALL**, and its one CMS mention is the **Turquoise Care 1115 Medicaid waiver approved 2024-07-25**. **(2) THE WRONG SECTION** (Nebraska's) — every row filed under a **FUTURE** opportunity's title while the names are a **PAST** award roster further down the same page, with the capture PARTIAL: seven of the roster's first EIGHT, **dropping Gallup Community Health** (Texas's 32-of-33, Kansas's Greeley a third time). **(3) THE $1 PLACEHOLDER** (Missouri's, Maine's) — every row at $1, so New Mexico's whole survey figure is **$7**, **AND THAT IS WHAT HIDES THE OTHER TWO**, because a row priced at $1 reads as missing data rather than as the wrong programme. **TWO of the seven are NAMED NEW MEXICO HOSPITALS** (Alta Vista Regional, Cibola General), so only the placeholder keeps this from being California's $5,475,000. **THE POSITIVE CONTROL AND THE §0.1 NEGATIVE SIT FOUR ITEMS APART ON ONE NEWS FEED**: *"New Mexico awards $50 million to 41 rural healthcare organizations"* (STATE, awarded, named, SIX HOSPITALS on it) beside *"NM opens $47 million fund for rural health projects"* (RHTP, open, unnamed) — a hunt scanning for "awards"+"rural"+a large figure takes the state one every time. The pool to watch is **Healthy Horizons, $76.2M and SIX REGIONAL HUBS** that *"direct funding to providers, Tribal health programs, community organizations..."* — Missouri's ToRCH shape, providers AMONG OTHERS, §0.3, a `PASS_THROUGH_*` question when it lands. **THE SIXTH DIGEST MECHANISM**: the WordPress **Complianz** plugin re-rolls a randomly-drawn post URL inside a script body — California's `antispambot()` one plugin over, except **California's re-roll was CONSTANT-LENGTH and this one is NOT** (199,369 vs 199,464 bytes), so a byte-count check passes California's and fails this. **THREE FETCHES SECONDS APART WERE BYTE-IDENTICAL** while the copy twenty minutes earlier was not — California's lesson confirmed a THIRD time by a THIRD mechanism. **BOTH STATES READ `INVESTIGATED_NO_LIST`**, both rebuilt from `R/03k`'s constants rather than hand-edited, both probes ran LIVE and report UNCHANGED, and **NEITHER CONTRIBUTES A ROW OR A DOLLAR TO ANY BUCKET** — `docs/session35_california_watch_connecticut_new_mexico.md`) (Session 34 — MAINE IS ON A TWICE-WEEKLY SCHEDULE, AND CALIFORNIA'S ELEVEN NAMED HOSPITALS ARE A SEISMIC PROGRAMME. **MAINE**: Routine `trig_01ALmis3jAgiLXVb6ZDLCj8Y`, **TUESDAYS AND FRIDAYS 16:20 UTC**, first fire 2026-09-04 — Wisconsin's cadence rather than Alaska's, because Maine is stale BY APPOINTMENT and one appointment has ALREADY PASSED (Maine DOE's 2026-08-31 award announcement, no roster), with the RHEF cohort beginning SEPTEMBER and EMR contracts executing in the FALL. **CALIFORNIA IS A NEGATIVE WITH THE TIGHTEST DATE YET, AND ITS CANDIDATE SET IS THE MOST DANGEROUS THIS PROJECT HAS MET.** HCAI brands RHTP as **CalRHT** and has opened FOUR opportunities, ALL headed **"(Closed)"** and NONE naming a recipient — Accelerator Partners $39,010,000, WDRR $54,170,000, EHR Modernization $11,650,000, FM-OB Fellowship $6,500,000, **$111,330,000 and 47.7% of the allotment**. Their own grant guides put award notification in **AUGUST / SEPTEMBER 2026** and grant agreements in **SEPTEMBER / OCTOBER 2026**, and **WDRR's window closed AUGUST 31, 2026 — TWO DAYS BEFORE THIS RAN**. **§0.1 IS TEXAS'S DEFECT WITH MAINE'S RATIO**: all **ELEVEN OF ELEVEN** California Tier 3 candidates are NAMED CALIFORNIA HOSPITALS with real amounts on real executed HCAI awards, and every one is the **Small and Rural Hospital Relief Program** — funded by the **CALIFORNIA ELECTRONIC CIGARETTE EXCISE TAX** (HSC 130075) for seismic compliance under the **ALFRED E. ALQUIST HOSPITAL FACILITIES SEISMIC SAFETY ACT** (HSC 129675), on a page mentioning "RHTP" **ZERO** times, "Rural Health Transformation" ZERO and "federal" ZERO. An extractor built from the list would publish **$5,475,000** of state cigarette-tax money as California's RHTP hospital dollars. **THE AGGREGATOR GAVE IT AWAY AND NOBODY READ IT**: RCJ's own descriptions are *"MTCAP, MTCAR, NPC 5 Evaluation ... SPC-4D Evaluation"* — seismic engineering deliverables. And RCJ carries **COMPONENTS, NOT GRANTS**: George L Mee appears twice at $500,000 and $280,000, which is HCAI's own published **$780,000** grant. **ONE PAGE CARRIES ALL THREE FAILURE MODES AND IS ARCHIVED AS BOTH CONTROLS**: the SRHRP page is the POSITIVE control (*"29 grants (totaling $17.2 million) have been awarded"*, five named and priced — so CalRHT's silence is CalRHT's), the §0.1 NEGATIVE, and **THE LARGEST §0.3 TRAP IN THE PROJECT** — *"SRHRP Eligible Hospitals"*, **102 NAMED California hospitals**, an ELIGIBILITY table sitting DIRECTLY BENEATH real awards. **§6.2 IN ITS STRONGEST FORM AND A NEW POSITION ON SESSION 27'S AXIS**: California publishes **CMS'S OWN NOTICE OF AWARD** (Award# RHTCMS332078-01-02, AL 93.798, budget period 12/29/2025–10/30/2026, $233,639,308.47) — Nevada's evidence a second time — and its footer is the **STRONG** form (*"The **CalRHT program** is supported by"*) and is **DEMOTED ANYWAY**, because the NOA is better evidence than any footer. Every previous demotion here was of a weak footer. **TWO DATES ON ONE DOCUMENT**: the NOA's Federal Award Date is **03/31/2026** and its action type is **"Revision (Budget)"**, while the budget period still starts **12/29/2025** — a date test keyed on "Federal Award Date" would read California's award as three months late. **THE ONE-CENT DISAGREEMENT IS PINNED (§8)**: CMS says .47, five HCAI publications say .46, and HCAI's FM-OB guide says .47. **EVERY ELIGIBLE CLASS IS HOSPITALS AMONG OTHERS** — New Hampshire's FHC answer, not Illinois's ICAHN answer. The §6.2 sweep goes **90 rows in 8 states -> 101 in 9**, catching all eleven **by TWO independent filters at once** (registry AND date, because the registry supplies a programme date for rows RCJ dates not at all). **THREE INVISIBLE-CHARACTER FINDINGS**: a ZERO-WIDTH SPACE in HCAI's WDRR heading and a literal CURLY APOSTROPHE both broke assertions with nothing to point at, and the SRHRP table has **105 `<tr>` rows and 102 hospitals** — TWO ARE BLANK SPACERS, so a row count reports 104. This file's own lesson one tier down. **AND A WRONG CLAIM THIS SESSION MADE AND THEN CAUGHT**: two fetches of each page THREE SECONDS APART returned the SAME SHA-256, so the file said a FILE digest was the change test — and the first live probe reported two of three pages CHANGED half an hour later with NOTHING changed. **TWO FETCHES SECONDS APART IS NOT A STABILITY TEST**, and the two mechanisms are both new to this project: a **CACHE VARIANT** (the CalRHT page served with or without a ~15 KB ElasticPress asset block, 142,605 or 157,732 bytes) and **WordPress `antispambot()` RE-ROLLING EMAIL ENTITIES** on every render (same length, different bytes, identical rendered text — a byte-count check passes it). Every earlier host failed through a PER-REQUEST nonce, which a back-to-back pair exposes; California's fails through a cache variant, which a back-to-back pair is GUARANTEED to miss. `ca_reduce_html()` absorbs both (reduced text identical, 11,162 chars) and the probe compares a CONTENT digest — the FOURTH mechanism across five hosts, and only MAINE's digests have actually held. California reads `INVESTIGATED_NO_LIST`; **CONNECTICUT and NEW MEXICO now lead the RCJ_ONLY queue at 7 each** — `docs/session34_maine_watch_california_seismic.md`) (Session 33 — MAINE NAMES ELEVEN RURAL HOSPITALS AND AWARDS NONE OF THEM. It led the RCJ_ONLY queue at 12 Tier 3 candidates and **ELEVEN OF THE TWELVE ARE NAMED MAINE HOSPITALS** — no state has reached the top of that queue looking more like a hospital extraction, and it is not one. DHHS *"has identified and **INVITED** 11 rural hospitals to participate"* in the $30M Rural Hospital Efficiency Fund; they did not apply; and the State's OWN RHTP Advisory Committee deck five days later says *"**Award amount and approved budget will be confirmed after start of participation** in the HE Cohort"*, against a timeline whose "HE Cohort begins" milestone is **SEPTEMBER** and whose "Funding approved and provided" milestone is **FALL/WINTER**. This ran 2026-09-02. **THE ROW-COUNT/DOLLAR PAIRING IS INVERTED**: Nevada and Iowa publish named hospitals with NO amounts, so the danger is reporting $0 without the row count; **MAINE'S DANGER IS COUNTING THE ELEVEN**, worth $30,000,000 and 15.8% of its allotment, and `me_assert_named_hospitals_are_not_recipients()` asserts BOTH halves at once — that the partition returns NO BUCKET AT ALL for Maine, and that eleven named hospitals nevertheless exist here, outside the award file. They live in `me_rhef_cohort.csv`, **NO `amount` COLUMN**, NOT in `test_state_union.R` (Missouri's device, and Maine's ratio is eleven of eleven against Missouri's 14 of 27). **ONE AWARD ACTION: the University of New England's Shaw Institute, $12,000,000**, announced on the DHHS **BLOG** and not its news index — a hunt reading one channel misses Maine's only priced award. It is `UNIVERSITY_OR_AHC` + `NON_HOSPITAL` because UNE's subrecipient class is STATED and contains no hospital (*"organizations in Maine's Public Health Districts and ... each Tribal Health Center"*) — Missouri's MEMSA coding, NOT New Hampshire's FHC. **EVERY OTHER CHANNEL IS PRE-AWARD WITH A DATE ON IT**: EMR contracts execute in the FALL, APM payments issue SEPT/OCT, and **Maine DOE's award announcement date of 2026-08-31 HAD PASSED TWO DAYS BEFORE THIS RAN** with no roster. **§6.2 WITH A MEASUREMENT FROM THE OTHER DIRECTION**: all four DHHS RHTP pages carry the weak *"This **program** is supported by ... $190,008,051.09"* footer, and **the Maine DOE opportunity carries NO CMS FOOTER AT ALL** while being unambiguously RHTP in its own words — so on Maine's estate the footer is **neither necessary nor sufficient**, and three programme-scoped sentences from three publishers carry the provenance. **THE POSITIVE CONTROL IS THE LARGEST THIS PROJECT HAS USED**: Maine's RFP archive, 1,406 solicitations, **1,362 with a NAMED AWARDED VENDOR**, DHHS among the issuers, and NOT ONE RHTP row — with the **NEGATIVE CONTROL INSIDE IT**, HRSA's *Small Rural Hospital Improvement Program*, awarded, DHHS, "Rural Hospital" in its title. Reading it exposed a defect worth recording: **the page carries TWO TABLES WHOSE COLUMN ORDERS DIFFER**, and a positional read swaps title with solicitation number, destroying both controls — columns are now resolved BY HEADER per table and an unresolvable header is REFUSED. **§0.1 IS MISSOURI'S DEFECT A SECOND TIME AND WORSE**: RCJ's eleven names match DHHS's roster **NAME FOR NAME** (asserted) and it carries every one at **$1** — the wrong KIND of action, invisible to any amount check — while pricing the one real award exactly. `--probe` ran LIVE and reports UNCHANGED on all four watched pages — `docs/session33_maine_invited_cohort.md`) (Session 32 — THE PDF READER NOW CARRIES RUNS, AND IOWA IS EXTRACTED BEHIND IT. `rhtp_pdf_lines()` broke a line only on VERTICAL movement, so two table cells painted at one `y` merged into one string and only the first `x` survived — "Adair County Memorial Hospital Greenfield", session 21's mangled-recipient failure one column over. The scanner now emits one row per painted RUN and the line model is COMPOSED from them (`rhtp_pdf_run_table()` -> `rhtp_pdf_runs()` / `rhtp_pdf_compose_lines()` / `rhtp_pdf_lines()`), with a run's line id advancing on EXACTLY the condition that used to flush a line — so the old output is reproduced BY CONSTRUCTION. **PROVED BEFORE IOWA WAS TOUCHED: all 41 committed PDFs return IDENTICAL page/x/y/text under both readers, and all NINE PDF-reading states (KS, MD, GA, NE, IN, OK, NV, MO, WI) rebuild with all 55 files under `data/reference/` BYTE-IDENTICAL**; the 8 xlsx renders differ only in `dcterms:created` and were reverted. NOTHING THRESHOLDS A GAP: the split is at Iowa's own producer's boundaries, which had to be so — 18093's second column mixes counties with CITIES (Greenfield is the seat of Adair County), so a county vocabulary could never have closed it. RUNS ARE RETURNED UNTRIMMED and that is measured: 199 of 328 runs in one notice END IN A SPACE and 150 are nothing but whitespace, so trim-then-paste welds words. MARYLAND is why the merge was PRESERVED rather than replaced — `md_read_primary_care()` reads a table "whose columns are too close to separate" and wants the line. **IOWA: 264 AWARD ACTIONS ACROSS TEN OPERATIVE NOTICES, 151 DISTINCT AWARDEES, 152 NAMED-HOSPITAL AWARD ACTIONS AND $0** — Nevada's shape and the largest instance of it. §0.2 INSIDE ONE DOCUMENT SERIES: the CMS footer's subject is programme-scoped (the STRONG form) but its AMOUNT is two tiers — the EIGHT Jan/Feb notices carry the RFP's pool, the THREE June ones carry Iowa's $209,040,063.71 ALLOTMENT, and the eleven sum to $854,852,514.73. (Session 31 recorded 7/4 and ~$845.8M; the counts are 8 and 3, corrected from the documents.) **SO NO FOOTER FIGURE ENTERS THE AWARD FILE AT ALL**, not even `round_amount` — they live in `ia_notice_footers.csv` with the tier each carries. THE NOTICES NEVER NAME THE PROGRAMME (RHTP/Rural Health Transformation/Healthy Hometowns = 0 occurrences, asserted every run), so the provenance is the programme page plus TWO other publishers. TWO INDEPENDENT READINGS CONFIRM THE PARSE: RCJ's ten Centers of Excellence names match NAME FOR NAME, and RCJ names a cell the notice WRAPS in full. 18330 SUPERSEDES 18093 (+Marengo Memorial, asserted) — `docs/session32_line_model_runs_iowa.md`) (Session 31 — THE PASS-THROUGH MARKER IS NOW A MONEY-MOVEMENT TEST, and session 30's two open items are both closed. `RHTP_PASS_THROUGH_MARKERS` carried two POSITIONAL patterns that matched where a SERVICE lands rather than where a DOLLAR goes; the positive control is the spec's own worked negative — Georgia's GHA obstetrical-carts note fired on `to hospitals` — and it is now a test that requires it NOT to. Both branches that depend on the test read ONE definition, `RHTP_MONEY_TO_HOSPITALS_MARKERS`, so they can never drift apart; what separates them is §10.2's SECOND clause (`award_made`), which is the distinction the spec draws. **BLAST RADIUS: EXACTLY TWO ROWS, 51 OF 53 REFERENCE CSVs BYTE-IDENTICAL, AND EVERY HOSPITAL PARTITION IDENTICAL BEFORE AND AFTER** — AL's Cahaba ($430,304, "at four Alabama hospitals") and KS's Salina ($932,310, "infrastructure to rural hospitals"), both `Unclear` -> `No`, both outside every bucket either way. ONE CONSEQUENCE IS A DISCLOSURE THAT GREW: `KS_RECIPIENT_FORM_NOT_STATED` goes 22 rows / $39,249,763 -> 23 / $40,182,073, because Salina's single flag slot was spent on `ELIGIBILITY_NOT_RECEIPT` and the form question KDHE's silence actually raises could not surface behind it. THE MANDATORY `determination_basis` REPAIRED: Oregon's six override rows led with "§10.2 DIRECT" against their own `PASS_THROUGH_UNRESOLVED` coding, and Nevada's two had NO SUCH COLUMN; a `test_state_union.R` invariant now fails on any empty or contradictory basis across every state at once. WISCONSIN IS ON A SCHEDULE — `R/03y --probe`, Routine `trig_01PEixRDWkHzpet4krJtye3b`, TUESDAYS AND FRIDAYS 14:00 UTC, twice-weekly because its window is open NOW; it ran live and reports UNCHANGED, and the 403 `/contracts/` path is still 403. THE THIRD ROTATING-DIGEST MECHANISM: dhs.wisconsin.gov injects an Akamai Boomerang RUM BEACON with a per-request nonce into every HTML response (Nevada rotates page CONTENT, Missouri a script SRC, Wisconsin a script BODY). IOWA INVESTIGATED AND NOT EXTRACTED: `/activity` found the route a FIFTH time, and hhs.iowa.gov's Healthy Hometowns page carries a **"Where to Find Funding Awardees"** section linking **ELEVEN Notices of Intent to Award** across nine RFPs, mostly named Iowa hospitals, with **NO per-recipient amount anywhere** (Nevada's shape). §0.2 INSIDE ONE DOCUMENT SERIES: the CMS footer carries the RFP's POOL in the seven Jan/Feb notices and Iowa's $209,040,063.71 STATE ALLOTMENT in all four June ones — summing the eleven gives ~$845.8M against a $209M allotment. RCJ's names are RIGHT and it holds 2 of 11 documents at $0 or $1. **NO EXTRACTOR WAS BUILT**, and the reason is stated: `rhtp_pdf_lines()` merges table cells sharing a `y`, yielding "Adair County Memorial Hospital Greenfield" — session 21's mangled-recipient failure one column over — `docs/session31_marker_fix_wisconsin_watch_iowa.md`) (Session 30 — WISCONSIN IS A NEGATIVE WITH A DATE ON IT: four DHS opportunities all "application period now closed", none naming a recipient, and DHS's own 2026-07-23 council deck saying **"Award announcements: September"** against three of them — so Wisconsin is at solicitation stage by a margin of DAYS, and it is the one `INVESTIGATED_NO_LIST` state whose window is open NOW. §0.1's newest mechanism: **RCJ prices 16 rows against a document whose TEXT LAYER CARRIES NONE OF THOSE FIGURES** — they are the 16 WTCS technical-college districts on a FORMULA allocation, sourced against WTCS's own roster. The $61M to watch is Rural Technology Transformation, whose eligible class is PRE-IDENTIFIED from the rural health facility list in Wisconsin's CMS application — hospital-weighted, and §0.3 exactly. THE PASS-THROUGH ELIGIBILITY SWEEP: all 19 `PASS_THROUGH_*` rows re-checked against their sources' eligibility language; **every coding holds and NOTHING was changed**. Two findings reported, both worth $0 today — the generic pass-through marker is NOT a money-movement test (it fires on the spec's own GHA carts negative), and six rows carry a `determination_basis` contradicting their own coding. MISSOURI's first scheduled probe fired and reports UNCHANGED — three hours BEFORE the bid opening it watches) (Session 29 — MISSOURI IS ON A SCHEDULE: `R/03w --probe` and Routine `trig_01RyGxdGNv6rrF8t6bT5fQdK` (Wednesdays 15:00 UTC) watch the ToRCH Care Smart Growth IFB, where Missouri's HOSPITAL money is. It compares a CONTENT digest, not a file digest, because dss.mo.gov rotates an Incapsula cache-buster on EVERY page — Nevada's rotating widget on a new host and host-wide. NEW HAMPSHIRE extracted: GO-NORTH, 2 Council-approved administrators, $66,547,394 sourced, and ZERO named subrecipients — its hospital RFA had not been PUBLISHED. FHC is NOT ICAHN: same shape, opposite coding, and the ELIGIBLE CLASS is why. Every nh.gov host is Akamai-403 to this environment. memsa.org re-tested and STILL UNREACHABLE — recorded with its four-agent evidence, and archive.org now says the Wayback Machine holds NO SNAPSHOT of it) (Session 28 — KANSAS'S PROVENANCE WIRED: the two independent, programme-scoped sources that were already committed and unread are now the §6.2 gate, the slide-deck footer is demoted to corroborating the amount, and `ks_year1_awardees.csv` rebuilds BYTE-IDENTICAL. And the "$8,000.18 publisher disagreement" turns out to be the AWARD DECK ALONE — KDHE's other two publications both say $221,898,007.82, which is CMS's table to the cent. MISSOURI extracted: 2 award actions, $7,232,660.43, ZERO hospital dollars — and a NAMED 27-ORGANISATION ROSTER, 14 of them hospitals, in which NOT ONE ROW IS AN AWARD)
...
| Stage 00 — CMS trigger list | `R/00_cms_press_monitor.R` | **Re-run Session 41: 9 states -> 12 — AR (2026-08-31), HI (2026-09-01), IN (2026-09-03); medicaid.gov lags on IN only and now carries VA, so the lag is under six days. Five test pins re-based.** Rewritten Session 14, re-run Session 15. `cms.gov/newsroom` (rural health topic) is PRIMARY, `medicaid.gov` SECONDARY, the two unioned. 9 states, unchanged — no state has announced since 2026-08-28. Session 15 fixed two defects the re-run exposed: the warm-index run warned on every pass, and the archive manifest listed itself — `docs/session15_virginia_dmas_negative.md`** |
...
| RCJ 50-state survey | `R/03k_rcj_state_survey.R` | **Built and run (Session 16); `INVESTIGATED_NO_PROBE` added Session 43; Mississippi moved to `EXTRACTED` Session 46, THE FIRST STATE TO LEAVE `INVESTIGATED_NO_LIST`. 38 of 50 states hold Tier 3 candidates; 29 were RCJ_ONLY and never investigated. §0.1 signal, never a dollar figure. THE DISPOSITION IS NOW FOUR-WAY: `EXTRACTED` 26, `INVESTIGATED_NO_LIST` 8 (worked, archived, AND probed — a re-checkable negative), `INVESTIGATED_NO_PROBE` 6 (worked and written down, NO probe, NOT re-checkable), `QUEUED` 10. `SURVEY_EXTRACTED_STATES`, `SURVEY_INVESTIGATED_NO_LIST_STATES` and `SURVEY_INVESTIGATED_NO_PROBE_STATES` are the constants; both tables are REBUILT from them and never hand-edited** |
...
| **Texas Year 1** | `R/03n_tx_year1_probe.R` | **Built and run (Session 19). A NEGATIVE. HHSC has published no recipient-level RHTP award list — Texas is at solicitation stage — and NOT ONE of RCJ's 68 Tier 3 candidates is an RHTP award: 53 are a state-appropriated programme (88th Legislature Rider 88), 9 are Medicaid, 6 are budget-narrative line items. Carries a tripwire with a positive control — `docs/session19_flow_fix_texas_negative.md`** **SESSION 46 MADE ITS `--probe` READ-ONLY, AND THAT IS THE DEFECT THIS REPOSITORY TOOK LONGEST TO NOTICE**: `tx_probe()` called `tx_fetch_sources(force = TRUE)` from session 19 until session 46, so every Routine firing re-downloaded eleven sources INTO `data/evidence/TX/` and re-dated the manifest — and because `tx_validate()` reads the archive, the probe was comparing the live site against bytes it had just downloaded. **A roster appearing on an RFA page would have been ARCHIVED FIRST AND ASSERTED AGAINST SECOND.** It now fetches to memory, compares a content digest and runs the check against the LIVE text, probing both state-funded CONTROL pages as well as the four subjects so it fails in both directions. See §2.2 — `docs/session46_probe_hygiene_mississippi_alaska.md` |
...
| **§0.1 state-attribution sweep** | `R/02c_state_attribution_sweep.R` | **Built and run (Session 42). §0.1 FAILURE MODE 6 — THE RECORD IS FILED UNDER THE WRONG STATE.** Reads all 5,056 committed records and asks whether the only US state a record names is a state OTHER than the one RCJ filed it under. Longest match first (session 14's West Virginia lesson); a record naming BOTH is not flagged; `<State> County|Parish|Township|City` excluded generically from the §7.1 vocabulary. **23 flagged, every one HAND-READ. TEN are another state's, in FIVE states** — WY←UT (5, incl. Utah's own $195.7M allotment as an `UNASSIGNED` WYOMING row), ND←AR (2), MO←MI, UT←OK, WA←FL — **so Wyoming is the largest and not the only one, and UTAH IS ITS MIRROR. AND NOT ONE OF THE TEN IS TIER 3**, the only tier an extractor reads, so the defect has **NOT reached a single award file**. **All EIGHT Tier 3 flags are FALSE POSITIVES with legible causes** — a state name inside the recipient's own legal name (*Providence Health & Services–Washington*, a real ALASKA awardee), a bare county in a county list, a street address. **It FLAGS and a human READS**: widening the exclusion list until the output is empty would suppress the ten real ones with the eight false, so verdicts live in `SWEEP_VERDICTS` and the build REFUSES both an unread flag and a stale verdict |
...
| **California Year 1** | `R/03ab_ca_year1_probe.R` | **Built and run (Session 34). A NEGATIVE, and the one with the tightest date so far — and its candidate set is the most dangerous this project has met.** HCAI brands RHTP as **CalRHT** and has published NO recipient-level award list: **four opportunities, all headed "(Closed)", none naming a recipient** — Accelerator Partners $39,010,000, WDRR $54,170,000, EHR Modernization $11,650,000, FM-OB Fellowship $6,500,000, **$111,330,000 / 47.7% of the allotment**. Their own grant guides say award notification is **AUGUST / SEPTEMBER 2026** and grant agreements **SEPTEMBER / OCTOBER 2026**; **WDRR's window closed AUGUST 31, 2026, two days before this ran**. **NOT ONE of RCJ's 11 candidates is an RHTP subaward, and ELEVEN OF ELEVEN ARE NAMED CALIFORNIA HOSPITALS** carrying real amounts on real executed HCAI awards — the **Small and Rural Hospital Relief Program**, funded by the **California Electronic Cigarette Excise Tax** for **seismic compliance**, on a page mentioning RHTP **zero** times. Texas's defect with Maine's ratio, worth **$5,475,000**. **§6.2 PASSES ON CMS'S OWN NOTICE OF AWARD** (Nevada's evidence a second time) and the footer — uniquely the **STRONG** form — is demoted anyway. **ONE PAGE IS BOTH CONTROLS AND THE §0.3 TRAP**: the SRHRP page's 29 named awards are the positive control, its cigarette-tax funding the negative, and its **102 named ELIGIBLE hospitals** the largest §0.3 table in the project — `docs/session34_maine_watch_california_seismic.md` |
| **Connecticut Year 1** | `R/03ac_ct_year1_probe.R` | **Built and run (Session 35). A NEGATIVE, and THE FIRST ONE IN THIS PROJECT WHOSE AWARD DATE HAS ALREADY PASSED.** DSS is lead agency for **$154,249,105.53** across **30 projects**, and its RHTP Documents page carries EIGHT documents of which NOT ONE is a roster. **One recipient-level solicitation has run and its award date has gone by**: OHS's **NOFO #26OHS001** ($1.8M Year 1, up to 5 awards of $100k–$1M) closed to applications **2026-07-07**, and OPM's own published timeline gives *"Grant Awards Announced ... **AUGUST 17, 2026**"* — **sixteen days before this ran**, with no roster on any reachable host. The state says it itself: *"Parties interested in being subrecipients ... are encouraged to **check back**"*. **§6.2 PASSES ON CMS'S OWN NOTICE OF AWARD** — the THIRD state after Nevada and California — and it carries **CALIFORNIA'S TWO-DATES TRAP A SECOND TIME AND WIDER**: Federal Award Date **07/23/2026**, action type **"Revision (Budget)"**, budget period still starting **12/29/2025**, so a date test keyed on the later date would read Connecticut's award as **SEVEN MONTHS LATE**. **NOT ONE of RCJ's 7 candidates is a subaward: all seven are BUDGET-NARRATIVE LINE ITEMS** whose awardees are the administering state agencies (Oklahoma's TIER defect) — and **A MECHANISM THIS PROJECT HAD NOT RECORDED: RCJ PRICES DOCUMENT REVISIONS AS SEPARATE AWARDS**, carrying Carelon's ONE $3,800,000 line TWICE and DMHAS's one line at TWO DIFFERENT AMOUNTS. NOT ONE candidate is a named hospital. **AND THE FIFTH DIGEST MECHANISM, THE FIRST THAT IS PER-NODE** — `portal.ct.gov` stamps a `?v=<timestamp>` asset version that is the SERVING NODE's build time: six fetches, 80,531 bytes every time, FIVE distinct digests — `docs/session35_california_watch_connecticut_new_mexico.md` |
| **New Mexico Year 1** | `R/03ad_nm_year1_probe.R` | **Built and run (Session 35). A NEGATIVE, and THE FIRST CANDIDATE SET TO STACK THREE RECORDED DEFECTS AT ONCE.** HCA has opened **SIX** RHT procurements and every one is pre-award in HCA's own words — four *"Currently under evaluation"*, Rooted in New Mexico *"Submissions due: **September 4, 2026**"* (two days after this ran) and the Data Hub *"Submissions due: TBD"*. **NOT ONE NAMED RECIPIENT.** **NOT ONE of RCJ's 7 candidates is an RHTP subaward, and all seven fail three different ways at once**: (1) **the wrong PROGRAMME** — all seven are the **Rural Health Care Delivery Fund**, which the Governor's own release calls *"$50 million **in state funding**"* from a fund *"originally established in **2023**"* and topped up *"during the **October 2025 special session**"*, while HCA's own RFA deck calls it *"a $50 million **STATE** investment"* and contains RHTP / "Rural Health Transformation" / CMS / "federal" **ZERO TIMES EACH**; (2) **the wrong SECTION** — every row is filed under a **FUTURE** opportunity's title while the names are a **PAST** award roster further down the same page (Nebraska's defect), and the capture is partial, **dropping Gallup Community Health** from the roster's first eight; (3) **the $1 PLACEHOLDER** — every row priced at $1, so New Mexico's whole survey figure is **$7**, **AND THAT IS WHAT HIDES THE OTHER TWO**. **TWO of the seven are named New Mexico hospitals**, so only the $1 placeholder keeps this from being California's $5,475,000. **THE POSITIVE CONTROL AND THE §0.1 NEGATIVE SIT FOUR ITEMS APART ON ONE NEWS FEED.** The pool to watch is **Healthy Horizons, $76.2M and six regional HUBS** — Missouri's ToRCH shape, providers AMONG OTHERS, §0.3. **THE SIXTH DIGEST MECHANISM**: the WordPress Complianz plugin re-rolls a randomly-drawn post URL inside a script body at VARIABLE length — `docs/session35_california_watch_connecticut_new_mexico.md` |
| **Louisiana Year 1** | `R/03ae_la_year1_probe.R` | **Built and run (Session 36). A NEGATIVE, AND THE ONE WHOSE ANNOUNCEMENT WINDOWS HAVE ALL CLOSED AT ONCE.** Louisiana led the RCJ_ONLY queue at 6 candidates / $53,910,000 and has published NO recipient-level award list. What it HAS published is **SEVEN Budget Year 1 solicitations and a "Notice of Intent to Contract Announcements" window for every one** — *"Late July to mid August"* **x3** and *"Mid to late August"* **x4**, 3+4=7 matching the funding page's seven *"Strategic Funding Opportunity Title"* headings — **and all seven closed before this ran on 2026-09-02**. Connecticut was the first negative here whose award date had passed, with ONE solicitation; **Louisiana has seven, and published the dates itself**. **NOT ONE of RCJ's 6 candidates is a subaward, and not one is a named organisation of any kind**: all six are rows of slide 18 of the 2026-08-20 Advisory Council deck, headed *"RHTP Funding Cycle Budget Year 1"* — the "awardee" is its **ACTIVITY** column (fund uses, §6.1's `PROGRAM_NAME_AS_AWARDEE` on **six of six**, `named_recipient_test` PASS on every one) and the amount its **"PROJECTED BY 1 Funding"** column times a million. Oklahoma's TIER defect with the state's own heading refuting it, and **505 applications received with not one award named** (§0.3). **AND RCJ DROPS THE LARGEST ROW** — Capital Improvement, 160 applications, **$41,600,000**, the capital one likeliest to reach a hospital — so its six understate the deck's seven ($95,510,000) by **$41,600,000**. **THE §0.3 CONTROL IS MACHINE-READABLE**: `rhtla.net/api/facilities`, 3,576 named facilities including **305 hospitals**, with **no money in it at all**. **THE SEVENTH DIGEST MECHANISM**: Cloudflare Email Address Obfuscation, a random one-byte XOR key per render, **constant-length at 169,500 bytes** — `docs/session36_ct_nm_watch_date_test_louisiana.md` |
| **Kentucky Year 1** | `R/03af_ky_year1_probe.R` | **Built and run (Session 37). A NEGATIVE, AND THE FIRST THAT NAMES ITS OWN AWARD-NOTIFICATION STEP AND DATE — TWICE, BOTH PASSED.** Kentucky was one of TWELVE states with NO RCJ Tier 3 signal. `ruralhealthplan.ky.gov` is a **dedicated RHTP domain no other state has** and carries **NINE RFAs**, none naming a recipient (*awarded*, *awardee*, *recipients*, *selected*, *Notice of Intent* = **ZERO each**). Its CMHC RFA prints *"July 10, 2026: **Notification of Award to Grantees**"* and its CHW RFA *"August 26, 2026 Anticipated **Notification of Award to Recipients**"* — both re-derived against `Sys.Date()` every run. **§6.2 PASSES ON CMS'S OWN NOTICE OF AWARD, THE FOURTH STATE — AND THE FIRST THAT IS AN ORIGINAL RATHER THAN A REVISION**: Action Type **"New"**, Federal Award Date **12/29/2025**, equal to its budget period start, where NV/CA/CT are all *"Revision (Budget)"* at +52/+92/+206 days. The first direct observation that the two fields agree absent a revision, which is exactly what session 36 pinned the date test on. **§0.2 NINE TIMES OVER**: that NOA is attached to EVERY RFA, so each ships a document whose only figure is the **$212,905,590.56 allotment** against a stated **$800,000 maximum award**. **AND THE OBVIOUS SEARCH IS POISONED** — *"Notice of Award"* occurs TEN times on Kentucky's funding pages and NOT ONE is a state award. The designated pass-through (Foundation for a Healthy Kentucky, §7's route) names no Hub Lead either — `docs/session37_footer_tier_ky_ny_nc.md` |
| **New York Year 1** | `R/03ag_ny_year1_probe.R` | **Built and run (Session 37). A NEGATIVE WHOSE CONTRACT START PASSED THE DAY BEFORE THIS RAN.** RCHI allocates **$76,190,022** (35.9% of the allotment) and its own guidance says *"Contracts for funded grantees will begin on **September 1, 2026** and end on June 30, 2027"*; this ran 2026-09-02. Connecticut's passed date was an announcement, Louisiana's were windows, Kentucky's are notifications — **New York's is a CONTRACT START**, one step further down the process. DOH's own 2026-08-12 deck says *"**91 Applications, $156,000,000 total request**"* and *"**Reviews and Funding Recommendation In Progress**"* — **§0.3 in the state's own numbers, oversubscribed two to one**. **THE ELIGIBLE CLASS IS NEW TO THIS REPOSITORY AND THE STRONGEST YET**: *"A **hospital must be included** as either the lead applicant or the partner Organization"* — hospitals MANDATORY in every award and NOT necessarily the recipient, which is neither Illinois's `Yes` nor New Hampshire's `Unclear`. The POSITIVE CONTROL is DOH's 2026 press index: **fifteen named award announcements**, and *"Rural Health Transformation"* occurs **exactly once** — a funding OPPORTUNITY. `nyscr.ny.gov` reads **UNREADABLE/UNKNOWN** (§0.4) — `docs/session37_footer_tier_ky_ny_nc.md` |
| **North Carolina Year 1** | `R/03ah_nc_year1_sources.R` | **Built and run (Session 37); EXTRACTED (Session 38). 44 NAMED RECIPIENTS ACROSS TWO ROSTERS, AND `amount` IS EMPTY ON ALL 44.** It is **THE SECOND FLORIDA**: invisible to BOTH discovery layers, the largest allotment in that group ($213,008,356). **39 Mobile Integrated Health recipients ($10,000,000, a POOL figure in `round_amount`) and 5 NC ROOTS Hub Leads (NO figure at all).** Nevada's and Iowa's shape — but **INVERTED**: those two publish named HOSPITALS with no amounts, so their danger is quoting the $0 without the row count; **North Carolina's 44 are EMS agencies and regional pass-through leads, so it contributes 0 rows and $0 to EVERY bucket** (Maine's and Missouri's shape). **THE HEADER'S 38-OF-39 WAS 37**: two names carry no EMS token, not one — **Cape Fear Valley** and **Clay County**, both left at §8's standing fallback and QUEUED, because promoting Cape Fear Valley on this pipeline's knowledge of its parent is the §0.4 failure while NCDHHS's own sentence calls all 39 *"local EMS agencies"*. **THE TWO SPELLINGS OF THE REGION 4 HUB LEAD CLASSIFY DIFFERENTLY** — *"University of North Carolina Hospitals"* → HOSPITAL_OR_SYSTEM/DIRECT/`Yes`, *"UNC Health"* → the fallback/`No` — so a fuzzy merge changes a **CODING**, not a count; NCDHHS's own *"public academic medical center"* settles it as `UNIVERSITY_OR_AHC` (Oregon's OHSU precedent) and neither machine answer is used. **ALL FIVE HUB LEADS ARE `PASS_THROUGH_UNRESOLVED` + `Unclear`** — hospitals AMONG OTHERS, New Hampshire's FHC class. **AND THREE OF THEM RAISE A CONDITION THIS PROJECT HAD NOT RECORDED: THE SOURCE STATES A FORM §8 DOES NOT CARRY** (*"Managed Care Organization"* ×2, *"comprehensive care management provider"*). No code was invented (§2); both questions are in the review queue at $0. **THE ROOTS ROWS' `round_amount` IS `NA`, WHICH IS §0.2 IN THE DATA** — the only currency on either ROOTS document is the ALLOTMENT. **THE SECOND TIER NAMES NOBODY** and is on a weekly Routine — `docs/session38_nc_extraction_ny_class.md` |
| **Arkansas Year 1** | `R/03ai_ar_year1_awardees.R` | **Built and run (Session 40). 31 ORGANISATIONS, 37 PRICED AWARD ACTIONS, $149,177,618.45 — 71.5% of the $208,779,396 allotment — AND THE FIRST STATE FILE IN THIS REPOSITORY WHOSE AWARDS ARE PUBLISHED AT TWO GRAINS BY TWO PUBLISHERS.** `arkansasrhtp.com` is a DEDICATED RHTP DOMAIN, the second after Kentucky's and the FIRST THAT HAS AWARDED; Arkansas held **ZERO RCJ Tier 3 candidates and no CMS release** — FLORIDA'S SHAPE a third time after North Carolina, and the largest of the three. **READ `award_pool` AND REMEMBER THE THREE COUNTS**: 31 organisations, **37 award actions** (organisation x initiative — six organisations hold an award under both, and this is the grain Arkansas PRICES), **50 project awards** (the Governor's count, the grain Arkansas DESCRIBES, in `ar_year1_projects.csv`). The two publishers reconcile **to the cent at three levels**: the round, each initiative, and each of the 37 pairs. **IT NEEDS SESSION 32'S RUN MODEL** — the three amount columns are painted at one y, so the line model returns an UNPARSEABLE `"$2,571,095.00$0.00$2,571,095.00"`, and `ar_assert_line_model_merges()` asserts that it still does. **EVERY ROW IS AN INTENT**: Arkansas's own words are *"the details of each grant will not be finalized until DFA signs an official agreement with each grantee"*, so all 37 are `NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No` + `AMOUNT_PRELIMINARY` (§8's existing code; no code was invented). **THE HOSPITAL FLOOR IS 9 ROWS / $21,792,687.96 against 16 organisations / $100,723,693.49 on §8's standing fallback** — the ninth instance and the largest in dollars, one-directional, ceiling $122,516,381.45. **TWO OF FOUR INITIATIVES ARE STILL TO COME** (RISE AR, HEART — both closed, NEITHER publishing an award date), so the figure is a PARTIAL YEAR and `--probe` is on a twice-weekly Routine — `docs/session40_arkansas_extraction.md`** |
| **Wyoming Year 1** | `R/03aj_wy_year1_awardees.R` | **Built and run (Session 42). 77 AWARD ACTIONS — 75 ACROSS SIX RECIPIENT-LEVEL TABLES IN ONE DOCUMENT PLUS TWO FROM ITS MINUTES — $173,859,752 to a named recipient, and 31 NAMED-HOSPITAL ROWS WORTH $72,661,324, the second largest in this repository after Georgia.** The FOURTH zero-signal state with a published, named, priced roster (FL, NC, AR, WY): no CMS release, ZERO RCJ Tier 3 candidates, and the roster is in a GOOGLE DRIVE FOLDER `health.wyo.gov` links behind *"View Documents In Google Drive"*. **IT NEEDS SESSION 32'S RUN MODEL AND THE ASSERTION SAYS WHAT THE LINE MODEL COSTS**: six rows carry the recipient name in a SEPARATE PAINTED RUN, so `rhtp_pdf_lines()` reads Initiative 1.1 as **SIXTEEN hospitals and $43,044,174**, orphaning **$5,156,000 in silence** — Arkansas's assertion in the mirror (there the line model WELDS, here it SPLITS). **EVERY ROW IS A COMMITTEE APPROVAL, NOT AN EXECUTED AGREEMENT** — obligation deadline END OF OCTOBER 2026 (the minutes) / 2026-10-01 (WDH's timeline), so `NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No` + `AMOUNT_PRELIMINARY` (no code invented, §2). **1.1's FORM IS STATED BY WYOMING AND THAT MOVES $7,525,331**: §8's name rule misses Powell Valley, Cody Regional and Crook County, while the column is headed *"Hospital"* and the budget narrative gives ONE Facility Type — **Illinois's eligible class, not New Hampshire's**. **AND THE TWO 18s ARE NOT THE SAME 18** (three names differ each way). **1.2's hospitals are joined ON THE EIN and never on the name** (Georgia's precedent), and **Sheridan Memorial Hospital carries TWO EINs in one document**. **THE APPLICANT TRAP IS THE LARGEST HERE**: 56 named applicants were NOT awarded, 40 under Initiative 3.1 alone requesting **$38,794,342**, mostly NAMED WYOMING HOSPITALS. §6.2 passes on **CMS's OWN NOTICE OF AWARD, the FIFTH state**, and session 36's date pin holds a fifth time (+136 days). $38,618,260 goes to the WYOMING INNOVATION PARTNERSHIP, a sole-source MASTER FISCAL AGENT named by the minutes AND the budget summary, coded `PASS_THROUGH_UNRESOLVED`/`Unclear` on New Hampshire's FHC footing and NOT Illinois's ICAHN (Missouri's Hub Anchor test inverted — DSS's anchors are NOT fiscal agents and are out of its file; WIP IS one and is in). $30,877,990 — 15.1% — is approved at POOL level NAMING NOBODY AT ALL — `docs/session42_wyoming_extraction_wrong_state_defect.md` |
| **Mississippi Year 1** | `R/03ak_ms_year1_awardees.R` | **EXTRACTED (Session 46). 167 AWARDS, $104,115,146.80, ACROSS THREE PROGRAMMES — AND THE FIRST STATE TO LEAVE `INVESTIGATED_NO_LIST`**, which is what that code is for: a re-checkable negative with a probe and a Routine watching the channel the state itself named, which re-opened the state the day it published. The Governor's 2026-09-14 release names and prices all 167 (RTG 97 / $47,406,374.18, RCGC 43 / $43,334,338.62, TCE 27 / $13,374,434). **THE $0.80 GAP IS TRUNCATION**, not a discrepancy. **TWO ODD-SHAPE ROWS AND ONLY ONE COSTS A DOLLAR**: a missing space before a hyphen drops $2,500,000 and the total catches it; an organisation whose legal name contains `" - "` costs a NAME and a COUNTY and **no total anywhere notices**. **68 named-hospital award actions / $47,454,812.18**, against **96 rows / $56,022,017.62** on §8's standing fallback — one-directional, so the floor is genuine and $103,476,829.80 is the ceiling; **nothing promoted (§0.4)**. **A PARTIAL YEAR**: workforce and psychiatric emergency services are dated to **2026-10-14 .. 2026-10-29** by the release itself, two more open in October, and $101,792,073 is in no public roster. On Routine `trig_01RmgWna5oXyoMCXYrVqGVvE`, **TUESDAYS AND FRIDAYS 09:40 UTC**, now watching the NEXT tranche — `docs/session46_probe_hygiene_mississippi_alaska.md` |
| **South Carolina Year 1** | `R/03ao_sc_year1_awardees.R` | **EXTRACTED (Session 47). 228 AWARDS, $167,299,900.69, ACROSS 8 PROJECTS IN 4 INITIATIVES — AND THE FIRST STATE TO LEAVE `INVESTIGATED_NO_PROBE`.** It left the way that code's own note said it would: **by the state publishing**, not by anyone writing the missing probe, so South Carolina went `INVESTIGATED_NO_PROBE` -> `EXTRACTED` **without passing through `INVESTIGATED_NO_LIST`** and nothing had to be retracted. **THE DOCUMENT PUBLISHES NO TOTAL OF ANY KIND** — no grand total, no subtotal, no `Total:` row — so $167,299,900.69 is the SUM OF THE ROWS and the completeness rests on three things from OUTSIDE the arithmetic: **CMS independently states "228 grants"**, CMS's headline is "$167 Million", and **every project numbers 1..n with no gap**. **BOTH MISSISSIPPI DEFECTS HAVE AN ANALOGUE.** A row number is printed **`I2`**, so a parser keying on `^[0-9]+$` drops Rebound Behavioral Health and **$120,000** — which is why rows are anchored on the AMOUNT column and the numbers are the completeness check instead. And a **hyphen with NO SPACES**, painted as three runs, costs a **NAME rather than a dollar**: South Carolina spells one organisation **THREE ways** (17 `Prisma Health-Upstate`, 1 `Prisma Health- Upstate`, 1 `Prisma Health (Oconee)`) and **no total notices**. **THE SPACING WAS MEASURED FROM THE PDF'S OWN `/Widths`** — 45 boundaries, 43 unspaced (worst residual 37 em-units, which is KERNING) against 2 spaced at 205.6 and 209.8 versus a space of 203 — so the reader needed **no fix**: this producer paints its spaces as runs, and the widths PROVE it rather than repair it. **33 ROWS WRAP with the number and amount at the MIDPOINT and interleaved line ids**, so `rhtp_pdf_lines()` cannot assemble them at all (Arkansas's line model MERGES, Wyoming's SPLITS, **South Carolina's INTERLEAVES**). **NO CMS FOOTER ON EITHER DOCUMENT** — and session 39's footer has GONE from the programme page, recorded as an observation about the SOURCE (§0.4); the provenance is **STRUCTURAL**, the list's four Initiative headings being EXACTLY the state's four RHTP initiatives with the fifth absent as the page says. **A PARTIAL YEAR**: the **Tech Catalyst Fund**, $32,730,351, runs through the South Carolina Research Authority (§7) and has named nobody. **SESSION 50 TYPED 147 OF THE 150 UNSTATED-FORM ROWS AND SOUTH CAROLINA IS NOW THE LARGEST NAMED-HOSPITAL STATE IN THIS REPOSITORY: 55 rows / $56,587,137.77 -> 113 / $115,840,714.95**, +58 rows / $59,253,577.18 and nothing out (one-directional by construction). 30 rows are `FQHC_OR_RHC`, 58 `HOSPITAL_OR_SYSTEM`, **19 `STATE_AGENCY` — the SCDBHDD rows session 47 deliberately left alone, now typed directly at $0 effect** — 14 `OTHER`, 14 `PHYSICIAN_PRACTICE`, 11 `NONPROFIT_CBO`, 1 `LOCAL_GOVT_OR_PUBLIC_HEALTH`. **AND THE AGENCY BENCHMARK'S TWO HALVES NOW DISAGREE**: % of awards lands on it almost exactly (49.6% against ~50%) and % of dollars OVERSHOOTS (69.2% against ~60%, 65.9% with Acadia subtracted), reported overshooting rather than adjusted. THREE ROWS REMAIN on the fallback ($749,320). Session 47's reading, kept because it is what the award list alone supported: 55 named-hospital rows / $56,587,137.77 against 150 rows / $92,759,791.23, one-directional, ceiling $149,346,929, and **NOTHING PROMOTED (§0.4) IN EITHER DIRECTION**: Self Regional ($17.7M), McLeod, AnMed, Prisma and Tidelands are not promoted UP, and the nineteen **SCDBHDD** rows are not demoted to `STATE_AGENCY`. **THE TWO MUSC SPELLINGS CLASSIFY OPPOSITE WAYS AND MOVE $3,345,533** — North Carolina's UNC finding in a state that PRICES its rows — and both machine answers are KEPT. **ZERO RCJ Tier 3 candidates** against 33 RCJ records — `docs/session47_south_carolina_extraction.md`. **ON A ROUTINE AS OF SESSION 48**: `trig_01UdVg36W1aWGTDKs3jfHtfV`, **THURSDAYS 08:30 UTC**, watching the **Tech Catalyst Fund** — weekly rather than twice-weekly because South Carolina publishes **no award date** for it (North Carolina's and New Mexico's footing), and 08:30 because the fifteen Routines already running occupy a continuous band from 09:40 to 21:10 UTC. It ran live and reports UNCHANGED |
| **Delaware Year 1** | `R/03al_de_year1_awardees.R` | **Built and run (Session 44). FOUR AWARD ACTIONS, THREE ORGANISATIONS, `amount` EMPTY ON EVERY ROW — AND §0.3a's OWN WORKED EXAMPLE ARRIVING AS REAL DATA.** DHSS *"announced **awards** to establish four new school-based health centers"* on 2026-07-29, naming Nemours Children's Health, TidalHealth and **Beebe Healthcare twice**. Nevada's, Iowa's and North Carolina's shape at the smallest scale yet: **4 named-hospital rows and $0**. **§8's NAME RULE MISSES ALL THREE ORGANISATIONS** — the classifier returns `NONPROFIT_CBO`/`LOW` for each, so left to the machine Delaware has NO hospital rows, which is §0.3a's defect reproduced IN CODE; *"Beebe Medical Center"* classifies `HOSPITAL_OR_SYSTEM`/`HIGH`, so the two spellings decide the state. **THE OVERRIDE IS THE SPEC'S** (§0.3a names all three) and the machine's answer is kept on every row. **TWO FIGURES ARE DELIBERATELY OUT OF THE FILE**: the $195,000 Year 1 BUDGET (a plan, §0.3 — $195,000/4 = $48,750 is nobody's figure) and the $157,394,963.86 ALLOTMENT (§0.2). **ONE CLOSURE, ARRANGED BY NOBODY**: the fifteen live Year 1 budgets match the fifteen parsed from the budget-narrative PDF in session 8 **to the cent** — `docs/session44_delaware_extraction_mississippi_footer.md` |
| **Idaho Year 1** | `R/03am_id_year1_awardees.R` | **Built and run (Session 44). ONE NAMED AWARDEE, NO AMOUNT, NO HOSPITAL.** DHW's funding page: *"CLOSED 7/10/26: Maternal and Child Health Initiatives — Awardee: **Comagine Health** — Perinatal Quality Collaborative OB Readiness — Anticipated award start date: Aug. 14, 2026"*. An INTENT in Idaho's own future tense, and Idaho contributes to **NO bucket at all** (Maine's and Missouri's outcome by a third route). **IT IS DELAWARE'S CONTROL**: the same classifier returns the same fallback for Comagine Health and there it is the RIGHT answer, because Idaho states no form and no governing document does either. **THE WEAKEST FOOTER SUBJECT MET SO FAR** — *"**This website** is supported by"* — and its figure is the allotment. RCJ carries it as **"Co-Imagine Health" at $1** with no source document; the match is this project's reading, not a source's (§0.4), and the row is built from the state page. **ELEVEN opportunities are open or just posted** with vendor conferences into September 2026 — the most active of the fourteen |
| **Ohio Year 1** | `R/03an_oh_year1_awardees.R` | **Built and run (Session 44). ONE NAMED, PRICED AWARD — $10,000,000 to OHIO UNIVERSITY — AND THE FIRST CMS FOOTER HERE WHOSE FIGURE IS THE SUBAWARD.** Announced by Governor DeWine 2026-07-01. `UNIVERSITY_OR_AHC` → `NON_HOSPITAL`, so **$0 of hospital dollars** (Maine's UNE row, same reason). **§0.2 GAINS A THIRD POSITION ON ITS AXIS**: every other footer here prints the ALLOTMENT (Tier 1) or an RFP POOL (Tier 2); Ohio's prints *"a financial assistance award totaling **$10,000,000**"*, the award itself. **AND THE TIER CHECK CANNOT SEE IT** — $10,000,000 collides with no allotment, so declared `SOLICITATION` it is **ACCEPTED**, and it is Tier 3. Session 37 said the tier is not in the footer's GRAMMAR; Ohio adds it is not always in its ARITHMETIC either. Asserted, not patched. **A PARTIAL YEAR** — *"Additional contracts will be awarded in the coming months"*, so $192,030,262 is in no public roster — and ODH's own page is STALE, still reading *"the State of Ohio will submit an application. If awarded..."*. **RCJ IS RIGHT ABOUT OHIO**, name and figure and document, which is rare |
...
| Alaska Year 1 | `R/03h_ak_year1_awardees.R` | **Refreshed Session 46: 185 -> 244 intents to award, $181,871,366 -> $239,186,195 (+59 actions, +$57,314,828, 0 revised, 0 withdrawn); hospital rows 32 -> 43, $49,686,225 -> $62,396,425.** THE FIRST STATE ON A SCHEDULE — DOH overwrites one url weekly, so `--probe` runs Mondays as Routine `trig_01U4RxGWMH8yg37UKupHqTki`. **ALASKA'S OWN WEEKLY TABLE ACCOUNTS FOR THE GROWTH EXACTLY**: Week 5 (32 projects) + Week 6 (27) = 59, against a stated cumulative $239M / 244 Projects. **THE SINGLE-WEEK FORM OF THAT CONTROL FAILED, CORRECTLY** — it held only because every previous refresh landed exactly one week later — and it now sums the trailing weeks, with the money tolerance DERIVED from Alaska's own weeks-vs-cumulative gap rather than chosen. **THREE CONSTANTS HAD BEEN DOING TWO JOBS EACH**: `AK_CMS_ANCHOR_FILE` is split out of `AK_PRIOR_FILE` (the 142-project anchor does not roll); the Planning count is re-based onto that anchor because **ALASKA HAS RESUMED PLANNING AWARDS, 19 -> 26**; and revisions are measured against the earliest snapshot with the dates DERIVED, so this refresh did not silently drop Southcentral Foundation's +$4,306,887 note. **TWO NEW ORG-TYPE TOKENS, BOTH REFUSED BY THE CLASSIFIER FIRST** — *"Health Care Providers"* / *"Health care provider"* are SERVICE tokens, and *"Education Organization (Not Public University)"* is Alaska re-wording its own. Built Session 12 |
...
| **Verification queue 2 — ingest + 3 policies** | `R/03ap_verification_queue_2.R` | **Built and run (Session 49). THE RETURNED QUEUE: 399 rows, ALL ANSWERED, over 500 committed award rows in 21 states — and the workbook is READ-ONLY, pinned by SHA-256.** `NAMED_HOSPITAL` goes **747 rows / $599,533,409 / 18 states -> 865 / $706,793,190 / 19**, +118 rows and **+$107,259,781.21 net**; NEITHER pool bucket moved, because every `_FLOW` question is still open. **ARKANSAS BECOMES THE LARGEST NAMED-HOSPITAL STATE**, ahead of Georgia, and **NORTH CAROLINA ENTERS THE PARTITION AT ALL** for one row and $0. **72 of the 120 rows that moved in carry NO AMOUNT** (IA 64, NV 7, NC 1), so Iowa alone gains 64 named-hospital award actions and not one dollar. **THREE POLICY CHANGES**: §8 gains **`OTHER`** (35 rows — a determined form §8 does not carry, NEVER a hospital type, and a row carrying it must state the form in `determination_basis`); §10.2 gains the **hospital-foundation row**, applied across EVERY committed state file (7 organisations / 10 rows / $17,853,500) and limited by the word **NAMED**, which is what keeps it off New Hampshire's **$66,547,394** association foundation; and **`basis_type`** records what each answer rests on — **306 `GENERAL_KNOWLEDGE`, 80 `ORG_WEBSITE`, 13 `STATE_SOURCE`, and NOT ONE of the 111 hospital answers on a state source**. **THE AWARD'S OWN URL DOES NOT BUY `STATE_SOURCE`** (238 of 399 bases carry one) and `GENERAL_KNOWLEDGE` stays at `LOW`, so a reader can SUBTRACT. **SALINA REGIONAL'S FLOW WAS HELD** and the three rows sharing its basis sentence were not — queued as `VQ_INKIND_AFTER_RETYPE` rather than buried. **SIOUX CENTER HEALTH IS A HOSPITAL, SO IOWA'S $50,000,000 CENTERS OF EXCELLENCE POOL IS HOSPITAL-ONLY** — a statement about WHO, with the $50M still TIER 2 and attached to nobody — `docs/session49_verification_ingest_policy_changes.md` |
| **Unstated-form typing — MS + SC** | `R/03aq_unstated_form_typing.R` | **Built and run (Session 50). THE TWO STATES `verification_queue_2.xlsx` NEVER REACHED, TYPED DIRECTLY UNDER SESSION 49'S THREE POLICIES.** Mississippi was not in the workbook at all and South Carolina's rows were deliberately kept out of the shared queue (session 47), so between them they carried the two largest open form questions left here — 96 rows / $56,022,017.62 and 150 / $92,759,791.23. **169 of the 173 organisations are typed and FOUR are refused**, with `basis_type` on every row. **+74 rows and +$80,696,969.67 INTO `NAMED_HOSPITAL`, and NOTHING OUT** — one-directional by construction, because every open row was already `No`. **THE EVIDENCE IS MOSTLY A FEDERAL RECORD, WHICH IS WHAT MAKES THIS DIFFERENT FROM THE QUEUE PASS**: CMS's Hospital, FQHC and RHC Enrollment files carry the state's own awardee string as an `ORGANIZATION NAME` or DBA on 70 of the 74 rows, worth $71,354,639.67 at `ORG_WEBSITE`/`MEDIUM`; only **$9,342,330 rests on a hand-read bridge** at `GENERAL_KNOWLEDGE`/`LOW`, and all four are named so a reader can subtract them. **SOUTH CAROLINA BECOMES THE LARGEST NAMED-HOSPITAL STATE** (113 rows / $115.8M) and Mississippi fifth (84 / $68.9M). **WINSTON COUNTY MEDICAL FOUNDATION RESOLVES WITHOUT §10.2's FOUNDATION ROW**: CMS carries that exact string as an `ORGANIZATION NAME` against CCN 250027 and the IRS gives it as *"Dba Winston Medical Center"* with Schedule H — the foundation IS the hospital, not an arm of one. **ACADIA HEALTHCARE CO. IS TYPED AND QUEUED AT THE SAME TIME** ($5,565,253, `SC_ACADIA_PARENT_SCOPE`): a hospital company, but the awardee is the corporate parent over a mixed South Carolina estate. **THE TYPING IS AN OVERLAY, NOT A COLUMN EDIT** — the plan is derived from the BUILDERS and not from the committed CSVs, so an already-applied file cannot produce a silently empty plan — `docs/session50_unstated_form_typing.md` |
| **FL ceiling, GA pools, allotment gaps** | `R/03ax_fl_ga_ceiling_pools_gaps.R` | **Built and run (Session 57). SESSION 58 ADDED `--apply`, which re-codes exactly Florida's five Unclear rows (all `No`; FL ceiling = floor = 26.2%) and must run after the 03ap overlay; Georgia's ceiling is recorded as STRICT. Session 57's original: A REPORT, NO ROW RE-CODED.** Settles Florida's five Unclear rows against archived CMS enrolment, NPPES and IRS records (one EXACT, one BRIDGE, one NEGATIVE-only) as PROPOSALS in `fl_ceiling_resolution.csv`; records that Georgia publishes no split for its two mixed pools (`ga_mixed_pool_split_search.csv`); decomposes both allotment gaps with every unsourced remainder labelled UNEXPLAINED (`fl_ga_allotment_gap_components.csv`). Georgia's revised CMS NOA is image-only and its budget section is transcribed, held to its own total and to DCH's footer |
| **Year 1 completion status** | `R/03aw_year1_completion_status.R` | **Built and run (Session 56). A REPORT, NO ROW RE-CODED.** `year1_completion_status.csv`: 31 rows, one per EXTRACTED state, giving allotment, priced subawards, priced plus round-level, both percentages, and `year1_status` with the sentence and archived file it rests on. COMPLETE needs a completeness STATEMENT **and** no recorded unawarded initiative, and the build refuses anything else. `year1_complete_hospital_share.csv` reports the hospital share for COMPLETE states only, as floor and ceiling. It refuses a cited evidence path that is not in the archive, and that check caught a wrong path on its first run |
| **Tennessee Year 1** | `R/03ay_tn_year1_awardees.R` | **Built and run (Session 59). 53 named HART awards, no amounts; 2 hospital rows ($0) coded against TDH's own government/nonprofit label (§0.3a). RAMP is the STATE-money §0.1 control. `--probe` on Routine `trig_01Ad384uvudquMnQXVHokXTX`, Sat 09:30Z — `docs/session59_tennessee_wa_footer_five_routines_nuvita.md`** |
| **New Jersey Year 1** | `R/03bf_nj_year1_awardees.R` | **Built and run (Session 61). 103 priced awards / $83,060,837 in six sections from the state's allocations PDF (no total, no CMS footer; the Governor's '$83 million' and '103 projects' corroborate). 35 hospital rows / $35,275,076 typed on archived CMS NJ enrolment files; AtlantiCare Health Services is the FQHC; Virtua the one LOW bridge. `--probe` on Routine `trig_01UPrPJ31h2G3aLBc68QyB12`, Sat 12:10Z** |
| **Newsroom sweep** | `R/03bg_newsroom_sweep.R` | **Built and run (Session 61). One probe over the 18 no-release states' newsroom indexes (21 pages, 14 states; MA MD NH IL recorded unreadable and re-tested). Trips on a NEW headline pairing RHTP and award language. Logs as state NEWSROOM. Routine `trig_01PFM6vKqJWHtik1CcZ7ZemA`, Mon/Thu 16:50Z** |
| **Queued-state watches** | `R/utils_page_watch.R`, `R/03az`–`R/03bc` | **Built (Session 59). Read-only probes for CO (Mon/Thu), VA (Tue/Fri), ND (Wed), WA (Thu), each on a Routine. Every tripwire is driven both ways in `test_queued_state_probes.R`** |
| **Enrolled hospital operators + OTHER forms** | `R/03bj_enrolled_hospital_operator.R` | **Built and run (Session 71). §10.2's AHC / enrolled-operator rule as an overlay after session 49's: exact legal-name sweep over 21+ archived CMS enrolment files, every hit hand-read (39), 33 rows re-typed, 10 held. Plus the determined-form check on every OTHER row. `--report`, `--apply`, `--totals`** |
| **Rural cut of NAMED_HOSPITAL** | `R/03ar_rural_cut_report.R` | **Built and run (Session 51). A REPORT, NO ROW RE-CODED.** 151 of 939 named-hospital rows / $157,997,116.60 (20.1%) carry a source-backed rural designation: Georgia's AHEAD roster (87), Oregon's OHA "rural hospitals" (35), Wyoming's 1.1 CAHs (18), and 11 MS/SC rows that CMS enrols as CAH or REH, read from ARCHIVED enrolment files by the row's cited CCN. 692 rows / $562,739,209.13 in 18 states carry nothing, which is the gap and is reported as one. Delaware's "Sussex County (rural)" is the SCHOOLS' county and does not count (§0.3a applied to rurality). Florida's two "URBAN" rows are recorded as not rural — `docs/session51_tier_bucket_refusals_rural_cut_live_probes.md` |
...
| Tests | `tests/testthat/test_00_cms_press_monitor.R`<br>`test_01_retrieve_rcj.R`<br>`test_02_normalize.R`<br>`test_03_state_registry.R`<br>`test_03b_budget_narratives.R`<br>`test_03c_cms_abstracts.R`<br>`test_03d_ga_great_health.R`<br>`test_03e_fl_year1_awardees.R`<br>`test_03f_pa_year1_awardees.R`<br>`test_03g_al_year1_awardees.R`<br>`test_03h_ak_year1_awardees.R`<br>`test_03i_sd_rht_contracts.R`<br>`test_03j_sd_year1_announcements.R`<br>`test_utils_recipient_classification.R`<br>`test_state_union.R`<br>`test_flow_table_parity.R` | `test_03m_or_year1_awardees.R`<br>`test_03n_tx_year1_probe.R`<br>`test_03o_ks_year1_awardees.R`<br>`test_03p_md_year1_awardees.R`<br>`test_03q_state_completeness_recheck.R`<br>`test_utils_pdf_text.R`<br>`test_03r_ne_year1_awardees.R`<br>`test_03s_in_year1_awardees.R`<br>`test_03t_ok_year1_awardees.R`<br>`test_03u_nv_year1_awardees.R`<br>`test_03v_mi_year1_awardees.R`<br>`test_03w_mo_year1_awardees.R`<br>`test_03x_nh_year1_awardees.R`<br>`test_03y_wi_year1_probe.R`<br>`test_03z_ia_year1_awardees.R`<br>`test_03aa_me_year1_awardees.R`<br>`test_03ab_ca_year1_probe.R`<br>`test_03ac_ct_year1_probe.R`<br>`test_03ad_nm_year1_probe.R`<br>`test_03ae_la_year1_probe.R`<br>`test_03af_ky_year1_probe.R`<br>`test_03ag_ny_year1_probe.R`<br>`test_03ah_nc_year1_sources.R`<br>`test_utils_footer_tier.R`<br>`test_03ai_ar_year1_awardees.R`<br>`test_03aj_wy_year1_awardees.R`<br>`test_02c_state_attribution_sweep.R`<br>`test_utils_footer_cms_share.R`<br>`test_03ak_ms_year1_awardees.R`<br>`test_03al_de_year1_awardees.R`<br>`test_03am_id_year1_awardees.R`<br>`test_03an_oh_year1_awardees.R`<br>`test_utils_probe_log.R`<br>`test_03ao_sc_year1_awardees.R`<br>`test_utils_name_tripwire.R`<br>`test_03ap_verification_queue_2.R`<br>`test_03aq_unstated_form_typing.R`<br>**Session 50 added `test_03aq_unstated_form_typing.R` (108), and its weight sits on the LIMITS rather than on the arithmetic, because the pass moves $80,696,969.67 into the hospital total across two states nobody verified.** The tests that carry it are the ones that would fail if the pass reached further than it says: that it touches ONLY rows carrying §8's standing fallback; that it is ONE-DIRECTIONAL (no row leaves a hospital bucket); that the flow test is re-run EXACTLY where the re-type crosses §10.2's `DIRECT` row and nowhere else; that a `GENERAL_KNOWLEDGE` answer stays `LOW` and **nothing reaches `HIGH`**; and that the three hand-read bridges are priced at LOW and each says *why* it is a judgement. **TWO STEM TRAPS ARE DRIVEN BY NAME** — *Singing River Services* is the Region 14 community mental health centre and not Singing River Health System, *George Regional Health and Rehab* is a nursing facility and not George Regional Hospital — and **one test requires no decision to quote the state's own description column**, which is §0.3a as a test rather than as a rule. **ONE TEST EXISTS TO FAIL IF A LATER SESSION ADJUSTS TOWARD THE BENCHMARK**: it pins that South Carolina's dollar share OVERSHOOTS ~60% at 69.2% and is reported overshooting. Iowa's three pin the pool row's tier, its `POOL_NAMED_HOSPITALS` attribution, and that all 264 award actions still carry an empty amount.<br>**Session 49 added `test_03ap_verification_queue_2.R` (212), and its weight sits on the LIMITS rather than on the arithmetic, because this pass moves more hospital money than any before it ($107,259,781 net) on evidence the project had previously refused.** The tests that carry it are COUNTERFACTUALS: that §10.2's hospital-foundation row does NOT reach New Hampshire's **$66,547,394** association foundation or Nevada Rural Hospital Partners (driven by name, flow and dollar); that `basis_type` CANNOT be talked into `STATE_SOURCE` by the award's own URL, which 238 of the 399 bases carry; that a foundation whose PARENT is not a hospital (a hospice, an FQHC, a conversion grantmaker) stays put; and that Kansas's two Citizens spellings DIVERGE by $146,476, which is the rule working rather than a defect. **FOUR TESTS DRIVE THE GUARDRAILS DIRECTLY**: that `mo_hub_anchors.csv` and `me_rhef_cohort.csv` carry no `amount`, no `flow_type` and no `distributed_to_hospital` and are absent from `STATE_FILES` (so 19 Missouri hospitals and 11 Maine ones reach NO bucket); that all four `_FLOW` rows' flows are unchanged; that **Salina Regional was HELD while the three rows sharing its basis sentence moved**; and that the ONE unanswerable row had NOTHING written to it — `UNKNOWN` is not a §8 value and is not in any state file. And one asserts §6.2 on Iowa: the Centers of Excellence pool is hospital-only AND its $50,000,000 is still `SOLICITATION` with `amount` empty on all 264 rows. **Session 48 added `test_utils_name_tripwire.R` (93), and the tests that carry its weight are the ones that could pass for the wrong reason.** A tripwire firing on everything is useless and one firing on nothing is worse, so both directions are driven on REAL COMMITTED ARCHIVES: **LOUISIANA's two snapshots of one page** (LDH re-dated all seven windows and named NOBODY -- the digest MOVED and the name diff must be silent IN BOTH DIRECTIONS) and **NEW MEXICO's two** (session 35's archive against session 46's page -- it must name all six hubs, and it does). It also drives the counterfactual that **NM_AWARD_POSTED's ten phrases match HCA's sentence ZERO times**, rather than repeating session 46's finding on trust. **THREE TESTS PIN DEFECTS THE TESTS THEMSELVES FOUND**: the trailing full stop that made every sentence-final name read as new, the ZERO-WIDTH character that renames *"Gila Regional Medical Center"* to *"Regional Medical Center"*, and the EMPTY BASELINE -- which is the guard that caught Missouri's `hub_roster` being a **PDF**. One test asserts every wired page has a real baseline TODAY (3+ names, silent against itself), and one **reads the source of every file in `R/`** and requires any file offering `--probe` to run the name tripwire, with Alaska and South Dakota as the two named exemptions. **Session 47 added `test_03ao_sc_year1_awardees.R` (127), and its weight sits on the PARSE, because South Carolina's award list is the first document here that publishes NO TOTAL OF ANY KIND — so reconciling the rows to their own sum is circular and three other things have to do that job: the per-project 1..n numbering, CMS's independent "228 grants", and the y-band assembly refusing to emit a row with an empty name, each tested. It DRIVES the `I2` counterfactual (a number-anchored parser loses a row and $120,000), drives a dropped row and a NEW malformed label and the recorded defect DISAPPEARING, and pins the glyph-width measurement by its SEPARATION — 43 unspaced boundaries against 2 spaced, with the worst residual far below half a space — rather than by a tolerance. Two tests read the raw runs directly: one shows the Prisma line has NO space run and that pasting with a separator invents a name South Carolina never printed, the other that the Self Regional line HAS one. And one pins that SC is ABSENT from `classification_review_queue.csv`, because this session was told to flag South Carolina's unstated-form rows in its own files only. Both survey tests moved from SIX worked-but-unprobed states to FIVE — South Carolina left by PUBLISHING — and `test_state_union.R` now combines **twenty-eight** state files. **Session 46 added `test_utils_probe_log.R` (36) and it is the one to read first, because the defect it pins went unnoticed for TWENTY-SEVEN SESSIONS.** It drives the evidence guard in all four directions (a probe that ADDS, REWRITES or DELETES an archived file is refused; one that leaves it alone is accepted), and the REWRITE case is Texas's exactly — the snapshot carries **mtime** because `force = TRUE` rewrote files whose BYTES had not changed and a digest-only check calls that clean. It pins that a fired tripwire is **logged AND re-raised**, that a refused host logs `ERROR` rather than `TRIPWIRE` (§0.4), that all four probe return shapes normalise, and that a probe with no per-page detail records `(unreported)` rather than **inventing a page name**. **TWO OF ITS TESTS READ THE SOURCE OF EVERY FILE IN `R/`**: one requires any file offering `--probe` to route it through `rhtp_probe_run()`, the other that no `*_probe` body calls a `_fetch` writer — the check that a twentieth state file cannot quietly reintroduce the defect. And one pins the **`isTRUE()` is not vectorised** bug: a first draft reported every page of a multi-page probe as UNCHANGED, which is indistinguishable from a quiet week. `test_03ak_ms_year1_awardees.R` gains the PARSE tests — each odd-shape row driven in the order a reader makes the mistake, and the LIFECORE row asserted **on the name and not on the money**, because every total reconciles on that defective parse. `test_03ae_la_year1_probe.R` and `test_03ad_nm_year1_probe.R` both pin WHY their control halted, and `test_03ad` drives the counterfactual that **none of `NM_AWARD_POSTED`'s ten old phrases matched HCA's selection sentence**. `test_03h_ak_year1_awardees.R` pins that the single-week cycle control would now fail and that the CMS anchor is a DIFFERENT file from the rolling prior. **Built — 57 files, 6,143 assertions; all pass and 1 self-skips (the stage 00 first-run branch, now that the CSV exists). Zero quota. Run `Rscript tests/run_tests.R`.** **Session 44 added 431 assertions (4,740 -> 5,171) across FIVE new files.** `test_utils_footer_cms_share.R` (71) is the one to read first: it drives Mississippi's headline being **ACCEPTED** as a Tier 2 pool, pins that the footer's own three numbers close to the cent, requires the parser to be INERT on the 100% form (all eighteen phrasings measured in the corpus), drives the REFUSAL TO COMPUTE when a partial share prints no dollar figure, and **pins `RHTP_FOOTER_ALLOTMENT_MARGIN` at $10,000 with the reason**. `test_03al_de_year1_awardees.R` (45) prices Delaware's counterfactual — with the classifier unaided the state has NO hospital rows — and drives the $48,750 derivation, the fifth-award tripwire and the session-8 budget closure. `test_03ak_ms_year1_awardees.R` (43) requires the tripwire NOT to fire on the page's own "selected applicants"/"sub-award execution" language. `test_03an_oh_year1_awardees.R` (31) pins the footer LIMIT — the tier check accepts a Tier 3 figure as a pool — and prices Ohio's own counterfactual at $10,000,000. `test_03am_id_year1_awardees.R` (28) is Delaware's control: same machine answer, opposite verdict. **The queue pins moved BECAUSE THE WORK WAS DONE**: the QUEUED bucket is now the TEN states left ($1,916,786,628 / 24 candidates) and the fifty split **25 / 9 / 6 / 10**, with a test requiring DE, ID and OH to have reached `EXTRACTED` and MS `INVESTIGATED_NO_LIST` rather than the weaker code. Session 43 added **23 assertions** (4,717 -> **4,740**) across `test_03k` and `test_00b` for the new `INVESTIGATED_NO_PROBE` code: they pin its membership, its presence in BOTH vocabulary columns, that the three worked statuses are DISJOINT, that its note still says it is WEAKER and that the next action is to WRITE THE PROBE, that the QUEUED bucket is EXACTLY the fourteen low-candidate states ($2,668,093,442 / 34 candidates), and that the fifty states split 22 / 8 / 6 / 14 with every state carrying a disposition. `test_state_union.R` now combines **twenty-eight** state files, twenty-seven states — and NEITHER Missouri's 27-row Hub Anchor roster, NOR Maine's 11-row invited cohort, NOR Arkansas's 50-row project file is among them. The first two are not awards at all; **Arkansas's is the same $149,177,618.45 twice, at a finer grain**, which is the sharper hazard.|
...
### Deliverable 1 — state files (session 54 adds VT, CT, WV)

| | Rows | Total published | Hospital rows | Hospital dollars |
|---|---:|---:|---:|---:|
| FL | 81 | — | see `fl_year1_awardees.csv` | — |
| **GA** | **158** | $197,148,327 | **108 named hospitals** (+ Emory, $0, session 71) | **$90,277,580** |
| **PA** | 66 | $42,198,310 | 27 | **$24,149,111** |
| **AL** | 138 | $143,745,821 | **70** | **$83,548,287** (session 71: + UAB, South Alabama (AHC) and AltaPointe on CMS enrolments) |
| **AK** | **244** | **$239,186,195** | **44** | **$66,955,876** (preliminary; ROLLING; session 71 + Providence H&S-WA, CCN 020001) |
| **SD** (contracts) | **26** | **$9,223,177** (18 admin $7,344,025 + 8 Rural Strong $1,879,152, INSIDE the $31.5M round) | **5** | **$716,800** (session 64) |
| **SD** (announced rounds) | 2 | $121,500,000 | **0 named** | $0 |
| **IL** | 1 | $50,008,264 | **0 named** | **$0 named / $50,008,264 pooled** |
| **OR** | 278 | $175,312,365 | **57** | **$54,520,575** (intents to award; session 71 + 7 OHSU rows, CCN 380009, AHC) |
| **KS** | 46 | $80,020,499 | **30** | **$52,239,500** (VERIFIED session 49 — no longer a floor) |
| **MD** | 41 | $78,625,071 | **8** | **$23,661,116** (award OFFERS; VERIFIED session 49, and it moved BOTH ways) |
| **NE** | **70** (+21 unpriced) | **$41,687,307** ($36,137,615 awards + $5,549,692 Initiative 5.3 INTENTS) | **60** | **$14,431,596 named + $18,156,856 pooled-but-named** (session 64) |
| **IN** | **7** | **$860,088** (one row; a 5-YEAR value) | **0** | **$0** — every recipient is a vendor |
| **OK** | **69** | **$3,572,121** (+$600,000 unnamed) | **27** | **$1,353,503** (session 71 + Choctaw Nation, CCN 370172) |
| **NV** | **156** | **$87,400,000** (POOL level only; RHIT, RHOAP, Tribal and the 14 new Flex rows carry no published total) | **44 award actions** (session 71) | **$0 — NEVADA PUBLISHES NO PER-RECIPIENT AMOUNT** |
| **MI** | **145** | **$101,318,437** (a TOTAL, not a floor; +6 state agencies session 64) | **4** | **$2,259,121** — session 71 + the Regents of the University of Michigan (CCN 230046, AHC) |
| **LA** | **6** | **$1,965,788** named (+$10,736,208 for 48 unnamed, in `round_amount`) | **1** | **$1,500,000 — Ochsner Clinic Foundation, CCN 190036 (session 71); '20 hospital-setting awards' still names none of the 20** |
| **MO** (awards) | **22** | **$7,232,660** (2 priced) | **20** | **$0 — 20 SMRP hospitals, NO AMOUNTS PUBLISHED (session 54); the 2 partnerships reach no hospital** |
| **VT** | **111** | **$84,540,011.33** (EXECUTED; PARTIAL list; +$2,635,000 GMCB MOU out of file) | **27** | **$22,641,819.43** (incl. Mary Hitchcock NH $8.82M) |
| **CT** | **4** | **$49,980,000** (ROUNDED) | **2 + 1 pooled pair** | **$33,350,000 named + $12,650,000 POOL_NAMED (Windham + CHH, unsplit)** |
| **WV** | **7** | **$6,444,803** (a PARTIAL YEAR) | **2** | **$1,224,000** |
| **MO** (hub anchors) | **27** | **— NO AMOUNT PUBLISHED** | **19 named** (14 -> 19, verified session 49) | **$0 — NOT AWARDS, AND STRUCTURALLY UNABLE TO BECOME ANY. READ BELOW** |
| **NH** | **2** | **$66,547,394** (one row priced) | **0** | **$0 — NO SUBRECIPIENT NAMED AT ALL** |
| **IA** | **264** | **— NO PER-RECIPIENT AMOUNT PUBLISHED** (session 51 removed session 50's Tier 2 pool row) | **218 award actions** (session 71: + University of Iowa Health Care ×2, bridges) | **$0. IOWA PRICES NOBODY. READ THE ROW COUNT** |
...
| **NC** (MIH) | **39** | **$10,000,000 (POOL level only)** | **1** | **$0 — NO PER-RECIPIENT AMOUNT. Cape Fear Valley verified session 49** |
| **NC** (ROOTS Hub Leads) | **5** | **— NO FIGURE PUBLISHED AT ALL** | **1** | **$0 — UNC Hospitals DIRECT on its enrolment (CCN 340061, session 71); the other four pass-through, hospitals AMONG OTHERS** |
| **AR** (round 1) | **37** | **$149,177,618** (THRIVE + PACT) | **20** | **$104,397,118** (session 71 + UAMS, CCN 040016, AHC) |
| **AR** (round 2) | **43** | **$54,685,069** (RISE AR + HEART; Year 1 COMPLETE, session 69) | **14** | **$23,939,844** (session 71 + UAMS; CARTI still queued) |
| **WY** | **77** | **$173,859,752** (+$30,877,990 approved NAMING NOBODY) | **35** | **$84,369,024** (session 71 + University of Utah GME, CCN 460009, AHC, out of state) |
| **DE** | **4** | **— NO PER-RECIPIENT AMOUNT** | **4 award actions** | **$0 — THREE ORGANISATIONS, FOUR AWARDS, NOBODY PRICED. READ BELOW** |
| **ID** | **1** | **— NO AMOUNT PUBLISHED** | **0** | **$0 — one named awardee, not a hospital** |
| **OH** | **1** | **$10,000,000** | **0** | **$0 — a UNIVERSITY; and a PARTIAL year, more contracts promised** |
| **TN** | **53** | **— NO PER-RECIPIENT AMOUNT** (a PARTIAL year: "initial recipients") | **2** | **$0 — Macon Hospital, Inc (bridge, LOW) and Cookeville RMC Foundation (§10.2, LOW). READ THE ROWS** |
| **MS** | **167** | **$104,115,146.80** (a PARTIAL YEAR — 3 of 5 programmes) | **84** | **$68,898,205** (TYPED session 50; 2 rows still unresolved) |
| **SC** | **228** | **$167,299,900.69** (a PARTIAL YEAR — 4 of 5 initiatives) | **113** | **$115,840,715** (TYPED session 50 — the LARGEST in this table) |
...
```
                          ROWS       DOLLARS  STATES
SESSION 54: NAMED_HOSPITAL 1,033 / $902,386,742.75 / 24 states (+VT 27 / $22.6M,
CT 2 / $33.35M, WV 2 / $1.2M, MO 20 / $0); POOL_NAMED_HOSPITALS 2 / $30,806,856.12
(NE + CT's unsplit Hartford HealthCare pair, both Tier 3). The block below is
session 51's and is kept as the audit trail.
NAMED_HOSPITAL        :    939   787,490,160      19   **SC 115.8M (113 rows)** ·
                                                       AR 92.4M · GA 90.3M ·
                                                       WY 75.1M · **MS 68.9M** ·
                                                       AL 68.3M · AK 62.4M ·
                                                       KS 52.2M · OR 50.7M ·
                                                       ...
                                                       **DE 4 rows, $0** ·
                                                       **NC 1 row, $0**
POOL_NAMED_HOSPITALS  :      1    18,156,856       1   NE 18.2M — the Nebraska High Value
...
**SESSION 51 MOVED NO NAMED-HOSPITAL ROW AND NO DOLLAR, AND TOOK IOWA OUT OF
`POOL_NAMED_HOSPITALS`.** That bucket is Nebraska alone again, one Tier 3 award,
because a bucket must not mix tiers (§0.2). `NAMED_HOSPITAL` is **939 rows /
$787,490,159.53 / 19 states**. Session 50 wrote .80, and a loose test tolerance
let the $0.27 slip pass. **Of those dollars, $157,997,116.60 (151 rows, 20.1%)
carry a rural designation a source states, and $562,739,209.13 (692 rows,
71.5%) carry none either way**: `rural_cut_by_state.csv`,
`docs/session51_tier_bucket_refusals_rural_cut_live_probes.md`.

**SESSION 50 MOVED 74 ROWS AND $80,696,969.67 INTO `NAMED_HOSPITAL` AND NOTHING
OUT, AND IT IS THE FIRST PASS THAT TYPED A STATE'S UNSTATED-FORM ROWS DIRECTLY
RATHER THAN QUEUEING THEM.** Mississippi and South Carolina were the two states
`verification_queue_2.xlsx` never reached — Mississippi was not in it, and South
Carolina's rows were kept out of the shared queue on purpose (session 47) — and
between them they carried the two largest open form questions left here: 96 rows
/ $56,022,017.62 and 150 / $92,759,791.23. **169 of the 173 organisations are now
typed, four are refused, and the evidence is mostly a FEDERAL RECORD rather than
general knowledge**: $71,354,639.67 of the movement rests on CMS's Hospital,
FQHC and RHC Enrollment files carrying the state's own awardee string as an
`ORGANIZATION NAME` or DBA, and only $9,342,330 on a hand-read bridge. **SOUTH
CAROLINA BECOMES THE LARGEST NAMED-HOSPITAL STATE IN THIS REPOSITORY**, ahead of
Arkansas (55 rows / $56.6M → 113 / $115.8M), and Mississippi goes 68 / $47.5M →
84 / $68.9M. **THE PASS IS ONE-DIRECTIONAL BY CONSTRUCTION** — every open row
was already `distributed_to_hospital = No` — so both states' floors were genuine
floors and no row left a bucket. **`POOL_NAMED_HOSPITALS` GAINS ITS SECOND ROW
AND ITS SECOND STATE, AND THE TWO ARE NOT THE SAME KIND OF FIGURE**: Nebraska's
$18,156,856 is a Tier 3 award amount from a signed notice of award, Iowa's
$50,000,000 is the **Tier 2** pool its own CMS footer attaches to the RFP.
See `IA_COE_POOL_IS_TIER_2` — `docs/session50_unstated_form_typing.md`.

**SESSION 49 MOVED 118 ROWS AND $107,259,781.21 NET INTO `NAMED_HOSPITAL`, AND
NEITHER POOL BUCKET MOVED AT ALL** — nothing in that pass touched a pass-through
award, because every `_FLOW` question is still open. **ARKANSAS IS NOW THE
LARGEST NAMED-HOSPITAL STATE IN THIS REPOSITORY**, ahead of Georgia for the
first time (9 rows / $21.8M → 18 / $92.4M), and **NORTH CAROLINA ENTERS THE
PARTITION AT ALL** for one row and $0, which is what takes the state count 18
→ 19. **SEVENTY-TWO OF THE 120 ROWS THAT MOVED IN CARRY NO AMOUNT** — Iowa 64,
Nevada 7, North Carolina 1 — so a reader working down the dollar column sees
none of them. **TWO ROWS CAME OUT**, Maryland's Choptank Community Health
System and Mountain Laurel Medical Center, verified FQHCs on their own
websites, −$3,034,792: the exact downward exposure `MD_RECIPIENT_FORM_NOT_STATED`
had named since session 22.

**AND A STATE CAN CONTRIBUTE TO NONE OF THE THREE WHILE PUBLISHING A COMPLETE
NAMED ROSTER. NORTH CAROLINA IS THE THIRD SUCH STATE AND THE LARGEST.** Its 44
named recipients are all award rows in `nc_year1_awardees.csv` — not a separate
non-award file, as Missouri's anchors and Maine's cohort are — and every one is
either a county EMS agency (`No`) or a regional pass-through lead (`Unclear`),
so the partition returns **no bucket at all** for it. That is the third distinct
way a state reaches $0, and the three must not be summarised together: Nevada
and Iowa are **named hospitals with no amounts** (rows, no dollars); Maine and
Missouri keep their named hospitals **outside the award file**; North Carolina's
award file is complete and **contains no hospital**.

**DELAWARE IS THE THIRD STATE TO MOVE THE ROW COUNT AND NOT THE DOLLAR, AND
IT IS THE SPEC'S OWN §0.3a WORKED EXAMPLE ARRIVING AS DATA.** DHSS *"announced
**awards** to establish four new school-based health centers in Sussex County
through the state's Rural Health Transformation Program"* on 2026-07-29 and
named four: **Nemours Children's Health**, **TidalHealth** and **Beebe
Healthcare twice**. FOUR AWARD ACTIONS, THREE ORGANISATIONS, AND NO
PER-RECIPIENT AMOUNT — so `amount` is empty on all four rows and Delaware
contributes **4 named-hospital rows and $0**. READ THE ROW COUNT.

**AND §8's NAME RULE MISSES ALL THREE RECIPIENTS, WHICH IS MEASURED AND IS THE
WHOLE POINT.** `rhtp_classify_recipient_type()` returns `NONPROFIT_CBO` at
`LOW` for *"Beebe Healthcare"*, *"Nemours Children's Health"* and
*"TidalHealth"* — not one carries a token the rule recognises — so **left to
the machine Delaware has NO hospital rows at all**, which is §0.3a's own
defect reproduced in code thirty-seven sessions after it was found by hand.
*"Beebe **Medical Center"*, the same organisation under `DE Verify.xlsx`'s
spelling, classifies `HOSPITAL_OR_SYSTEM` at `HIGH` — North Carolina's two
spellings of UNC a third time, and here it decides whether the state has any
hospital rows.

**THE OVERRIDE IS THE SPEC'S, NOT A SESSION'S (§0.4).** §0.3a names all three
organisations as *"hospitals and health systems"* and states the coding
outright, and the reviewer instructions code all three `Yes`; the classifier's
own answer is preserved on every row in `recipient_type_source`, so it is
auditable and reversible. **Idaho is the control that proves this is about
evidence rather than a weak classifier**: Comagine Health gets the SAME
machine answer and keeps it, because Idaho states no form and no governing
document does either.

**TWO DELAWARE FIGURES ARE DELIBERATELY OUT OF THE AWARD FILE.** DHSS's
programme page gives *"School-Based Health Centers ... Year 1 Budget:
$195,000.00"* — a BUDGET LINE in the state's plan, not an announced round
total (§0.3), so `round_amount` is `NA` and it lives in `de_year1_status.csv`;
**$195,000 / 4 = $48,750 is nobody's published figure** and a guard refuses
it. And the release's only currency figure is the **$157,394,963.86
ALLOTMENT** (§0.2). One closure, arranged by nobody: the fifteen Year 1
budgets on the live page match the fifteen parsed from Delaware's
budget-narrative PDF in session 8 **to the cent**, summing to $141,655,467.48
both ways.

**MISSISSIPPI IS 167 AWARDS IN A PRESS RELEASE, AND TWO OF ITS ROWS BREAK THE
ORDINARY SHAPE IN DIFFERENT WAYS — ONLY ONE OF WHICH ANY TOTAL CAN SEE.** The
Governor's 2026-09-14 release names and prices every one of 167 awards across
the three programmes the funding page had said were complete: **RTG 97 /
$47,406,374.18, RCGC 43 / $43,334,338.62, TCE 27 / $13,374,434**. The 167 sum to
**$104,115,146.80** against a stated **$104,115,146** — a **$0.80 TRUNCATION**
(nine rows carry cents summing to $3.80, and rounding would have given
$104,115,147). Neither figure is corrected (§8).

**READ BOTH ODD-SHAPE ROWS BEFORE TOUCHING THIS PARSE.** *RCGC 23* prints
*"North Mississippi Medical Center, Inc. - Lee County- $2,500,000"* — **the
space before the hyphen is missing** — so a `" - "` split finds no amount and
**drops the row**: 166 awards, short by $2,500,000 and by the largest award in
the section. **The total catches that one.** *RCGC 30* prints *"Northeast Mental
Health - Mental Retardation Commission d.b.a. LIFECORE Health Group - Monroe
County - $27,600"* — **the organisation's own legal name contains `" - "`** — so
a left-to-right split reads the awardee as "Northeast Mental Health" and the
**county** as "Mental Retardation Commission d.b.a. LIFECORE Health Group".
**The money is untouched**, the row count is still 167, every total still
reconciles to the cent, and the file quietly contains an organisation that does
not exist in a county that does not exist. **So the row count and the total
agree on a defective parse and the second defect is invisible to both.** A third
shape is not a defect and is still a silent drop: *TCE 9* prints **"Lafayette
Co."** where the other 166 print "County".

**167 AWARD ACTIONS ARE 103 DISTINCT AWARDEE STRINGS**, and Memorial Health
System alone holds twelve — one row per AWARD ACTION, not per organisation
(Michigan's lesson, session 27). **LIFECORE Health Group holds two awards under
two spellings** ($27,600 and $30,610) and they are **not merged** (§2).

**Mississippi's line is a FLOOR and the uncertainty is larger than it — the
unstated-form question a TWELFTH time and ONE-DIRECTIONAL.** The Governor
publishes a recipient, a county, an amount and a description of the WORK, and
nothing about the recipient's form: **96 of 167 rows / $56,022,017.62 — 53.8% of
the round — carry §8's standing fallback**, every one already `No`, so
**$47,454,812.18 is a genuine floor and $103,476,829.80 a genuine ceiling**.
Several read as FQHCs or physician practices to anyone who knows Mississippi
(Delta Health Center, Aaron E Henry, GA Carmichael, Mantachie Rural Health Care,
Hattiesburg Clinic P.A. with six awards). **Nothing was promoted (§0.4)**;
queued as `MS_RECIPIENT_FORM_NOT_STATED`. **The descriptions are not used for
typing** — a description describes an ACTIVITY and §0.3a judges the RECIPIENT
(Arkansas's precedent), and a test requires every `recipient_type` to be
reproducible from the NAME alone.

> **SESSION 50 TYPED 69 OF THE 71 ORGANISATIONS AND THE FLOOR IS NO LONGER A
> FLOOR.** 94 of the 96 rows now carry a determined form with `basis_type` on
> every one — 23 `FQHC_OR_RHC`, 27 `OTHER`, 25 `PHYSICIAN_PRACTICE`, 16
> `HOSPITAL_OR_SYSTEM`, 3 `NONPROFIT_CBO` — and **16 rows / $21,443,392.49 moved
> INTO `NAMED_HOSPITAL` and none moved out**, taking Mississippi to **84 rows /
> $68,898,204.67**. Twelve of the sixteen hospital rows rest on CMS's own
> Hospital Enrollment file carrying the state's awardee string as an
> `ORGANIZATION NAME` or DBA against a CCN; **three are hand-read bridges priced
> at LOW so a reader can subtract them** — Webster Healthcare Services
> ($500,000), Ochsner MS ($3,000,000) and Southwest Mississippi Regional Center
> ($277,077). **TWO ORGANISATIONS ARE REFUSED AND KEEP THE FALLBACK AND ITS
> FLAG**: CAMHP Foundation ($250,000) and **Delta Health Transformation Council,
> Inc. ($3,000,000)**, whose stem matches BOTH the Greenville hospital and the
> FQHC, so a stem match could type it either way and neither would be a
> determination (§2). Queued as `UF_FORM_NOT_DETERMINABLE`.

**AND MISSISSIPPI'S YEAR IS PARTIAL, DATED BY MISSISSIPPI.** The release says
the workforce and psychiatric emergency services initiatives *"are currently
being reviewed and will be announced in the next 30 to 45 days"* — **2026-10-14
to 2026-10-29** — and that two further opportunities launch in October.
**$101,792,073 of the allotment is in no public roster.**

**ALASKA'S LINE MOVED AGAIN AND WILL MOVE AGAIN: 185 → 244 AWARD ACTIONS,
$239,186,195** (+59, +$57,314,828, **0 revised and 0 withdrawn**), with hospital
rows **32 → 43** and hospital dollars **$49,686,225 → $62,396,425**. Alaska's own
weekly table accounts for it exactly — Week 5 (32 projects) + Week 6 (27) = 59,
the diff of two archived workbooks stated independently by the state. **Alaska
has also RESUMED PLANNING AWARDS, 19 → 26**, which had been static since
2026-08-28 and was asserted unchanged; CMS's 142 counts **Implementation** only,
so that split is now pinned on the 2026-08-28 anchor rather than on "last time".

**OHIO IS ONE PRICED ROW AND ZERO HOSPITAL DOLLARS, AND ITS FOOTER IS A NEW
POSITION ON §0.2's AXIS.** Governor DeWine announced *"the first Rural Health
Transformation Program award to Ohio University"*, **$10,000,000**, on
2026-07-01 — `UNIVERSITY_OR_AHC` → `NON_HOSPITAL`, Maine's UNE row for the
same reason. **Its CMS footer prints $10,000,000 — the SUBAWARD**, not the
allotment and not a pool, the first such footer here. **The tier check cannot
see it**: $10,000,000 collides with no allotment, so declared `SOLICITATION`
it is ACCEPTED. Session 37 said the tier is not in the footer's GRAMMAR; Ohio
adds that it is not always in its ARITHMETIC either, and what identifies it is
the DOCUMENT. Ohio's Year 1 is PARTIAL — *"Additional contracts will be
...
**nothing to any bucket**. **Re-derived a fourth time in session 40 from the
twenty-two committed files**: `NAMED_HOSPITAL` is **578 rows / $410,119,935
across 14 states**. **Re-derived a fifth time in session 42 from the
twenty-three committed files**: **609 rows / $482,781,258 across 15 states**.
Wyoming adds 31 rows and **$72,661,324**, which is the second largest
single-state contribution in the table after Georgia's and larger than
Arkansas's and Alabama's. Arkansas is the first state in three sessions to move the
DOLLAR figure rather than only the row count — Iowa and North Carolina added
152 rows and 0 rows at $0 apiece — and it adds 9 rows and $21,792,688.)*
...
**WYOMING IS THE SECOND LARGEST NAMED-HOSPITAL STATE IN THIS TABLE AND ITS
FIGURE IS A FLOOR, AND THE THREE THINGS THAT MADE IT ARE ALL SOURCE-BASED.**
$72,661,324 across 31 rows: Initiative 1.1's eighteen Critical Access Hospitals
($48,200,174), five hospital lead agencies under 1.2's EMS regionalization
($11,000,000), Cheyenne Regional Medical Center's GME award ($2,193,968.90), six
Initiative 3.1 technology awards ($11,116,521) and Sheridan Memorial Hospital's
care-coordination allocation ($150,660).

**1.1's recipient FORM IS STATED BY WYOMING, NOT INFERRED FROM ITS NAME, AND
§8's NAME RULE ALONE MISSES THREE OF THE EIGHTEEN.** *Powell Valley Health Care
Inc*, *Cody Regional Health* and *Crook County Medical Services District* carry
no hospital token and are worth **$7,525,331**. WDH's own award table heads its
first column **"Hospital"**, the initiative is *"Critical Access Hospital -
Basic"*, and the Year 1 Revised Budget Narrative's Eligible Applicants block
gives **one** Facility Type — *"Critical Access Hospital (18 Total)"*. Leaving
them on §8's standing fallback would assert the form is undetermined where the
state has stated it (session 38's UNC precedent, session 39's MCO precedent).
**The eligible class is Illinois's, not New Hampshire's** — and here it does not
even need §10.2's pass-through row, because in all 21 cases the recipient IS the
hospital. **AND THE TWO 18s ARE NOT THE SAME 18**: three names differ each way,
which is §0.3 (a plan is not an award) and must not read as corroboration.

**1.2's hospitals are joined ON THE EIN AND NEVER ON THE NAME** — Georgia's
application-number precedent (session 22), an exact key from the same publisher
in the same document, and not the fuzzy match §2 forbids. §0.3a judges the
...
centre coded `DIRECT`. **The fifth does not close on that key** — *Sheridan
Memorial Hospital* carries **83-6000241 in Initiative 1.1 and 92-0606087 in
Initiative 1.2**, two EINs in one document, recorded and not resolved (§8).

**Wyoming's unstated-form question is the TENTH and is one-directional: 24 of
its 75 approvals-table rows / $5,716,842**, all `No` today, so the floor above is genuine and $78,378,166 is
the ceiling. **NOTHING WAS PROMOTED (§0.4)**, and the two refusals are named:
...
Care Inc"* in 1.1 by any means §2 permits ($752,302); and *"Campbell County
Health (CCH) Emergency Medical Servic"* is TRUNCATED by the producer at its cell
edge and its EIN is not in the 1.1 table ($1,700,000). Both queued.

**WYOMING'S LARGEST SINGLE RECIPIENT IS `Unclear`, AND IT IS IN THE AWARD FILE
FOR MISSOURI'S REASON INVERTED.** Initiatives 2.1 and 2.3 are a **sole-source
master fiscal agent contract with the Wyoming Innovation Partnership,
$38,618,260**, named by the minutes' motion AND by the budget summary's own
Notes column. Missouri's 27 Hub Anchors are OUT of its award file because DSS's
FAQ says they *"will not act as the fiscal agent"*; **WIP is IN because
Wyoming's motion says it IS one**, and a fiscal agent receives the money it
administers. **It codes like New Hampshire's FHC and not like Illinois's
ICAHN**: ICAHN is `Yes` because Illinois restricted eligibility to hospitals
only, while WIP administers *"statewide individual and institutional
workforce/nursing grants"* — individuals and institutions, §0.3 exactly — so it
is `PASS_THROUGH_UNRESOLVED` + `Unclear`, in **neither** bucket, and
`wy_assert_administrator_is_unresolved()` fails if either sentence leaves its
document. A session that tidied the two codings into one would publish
**$38.6M** as hospital-bound money on this pipeline's authority.

**AND $30,877,990 — 15.1% OF WYOMING'S YEAR 1 — NAMES NOBODY AT ALL.** The
fiscal-agent sweep, three competitive RFPs and three administrative lines are
approved at POOL level against no recipient of any kind. They are in
`wy_year1_status.csv`, which has **no `amount` column**, and `wy_reconcile()` is
what puts the halves back together: $174,126,752 with a named recipient plus
$30,877,990 without is $205,004,742, the §7.1 allotment to a dollar.

**READ THE ROW COUNT AND THE POOL BEFORE QUOTING ANY WYOMING FIGURE.**
`round_amount` repeats an initiative's approved pool on every row of it, so
summing that column gives **$2,177,412,637** for a state that approved
$205,004,742 (Georgia's trap; `wy_reconcile()` does it correctly and an
assertion refuses the naive total). And two of the 75 rows — the Northern
Arapaho and Eastern Shoshone care-coordination allocations, $267,000 between
them — **name nobody at all** and carry an empty `amount` (Oklahoma's ROOTS
device), which is why the six tables' rows sum to $135,241,492 and not $135,508,492 — and why `sum(amount)` over all 77 rows is $173,859,752, the extra $38,618,260 being the two MINUTES rows.
...
All **twenty** states' files union on the leading 19 columns with zero values outside §8,
...
- **THE 50-STATE DISPOSITION AFTER SESSION 46, which is the line to read
  before opening either of the two files below.** `EXTRACTED` **27** ·
  `INVESTIGATED_NO_LIST` **8** · `INVESTIGATED_NO_PROBE` **5** (HI MA MN NJ TN
  — worked, no probe, NOT re-checkable) · `QUEUED` **10**.
  **SOUTH CAROLINA IS THE FIRST STATE TO LEAVE `INVESTIGATED_NO_PROBE`, AND IT
  LEFT BY THE STATE PUBLISHING RATHER THAN BY ANYONE WRITING THE PROBE** —
  straight to `EXTRACTED`, never through `INVESTIGATED_NO_LIST`, with nothing
  retracted. That is the weaker code doing its one job: it described what this
  repository had done, not what South Carolina had.
  **MISSISSIPPI IS THE FIRST STATE TO LEAVE `INVESTIGATED_NO_LIST`, AND THAT IS
  WHAT THE CODE IS FOR**: a re-checkable negative — evidence archive, probe and
  Routine, watching the channel the state itself named — which re-opened the
  state the day it published. Session 44 put it there; session 46 parsed the
  167 awards the Routine was waiting for. **The QUEUED
  bucket is now exactly the TEN low-candidate states left — AZ CO MT ND RI UT
  VA VT WA WV, $1,916,786,628 and 24 candidates between them.** Session 43
  read all fourteen and deliberately moved none of them, because four held
  work; **session 44 did that work, which is the only route out of this
  bucket** — Delaware, Idaho and Ohio to `EXTRACTED` (award file, archive,
  probe) and Mississippi to `INVESTIGATED_NO_LIST` (archive, probe, Routine).
  A test pins both halves. Every state still sits in a bucket that says what
  was done to it, and "queued" means nobody has built a file for it rather
...
- **`data/reference/state_trigger_queue.csv` — 50 rows (Session 16; refreshed
  ...
  than CMS alone.
- **`data/reference/de_year1_awardees.csv` — 4 rows (Session 44), AND
  `amount` IS EMPTY ON EVERY ONE.** Delaware's four school-based health centre
  awards, announced 2026-07-29 by DHSS: **Nemours Children's Health**,
  **TidalHealth** and **Beebe Healthcare TWICE** — four award actions, THREE
  organisations. **`round_amount` IS `NA` DELIBERATELY**: the state's own
  School-Based Health Centers *"Year 1 Budget: $195,000.00"* is a BUDGET LINE
  on the programme page, not an announced round total (§0.3), and it lives in
  `de_year1_status.csv` instead — dividing it four ways gives $48,750, which
  no publisher has stated. **Every `recipient_type` is an OVERRIDE of the
  shared classifier**, which returns `NONPROFIT_CBO` at `LOW` for all three
  organisations; the type comes from spec §0.3a and the machine's answer is
  kept on every row in `recipient_type_source`. Rebuild with
  `Rscript R/03al_de_year1_awardees.R --build`; `DE_year1_awardees.xlsx` is a
  render whose first sheet is the warning.
- **`data/reference/de_year1_status.csv` — 16 rows (Session 44), AND IT HAS NO
  `amount` COLUMN.** All fifteen Delaware initiatives with the Year 1 budget
  DHSS publishes for each (in `year1_budget`, never `amount`), plus the
  news.delaware.gov channel control. **The fifteen budgets match the fifteen
  parsed from Delaware's budget-narrative PDF in session 8 TO THE CENT**,
  summing to $141,655,467.48 both ways. It also records where `DE Verify.xlsx`
  rows 8 and 9 belong — the Continuous Glucose Monitoring initiative, which
  has published no award notice this environment can reach, which is why those
  two rows are NOT in the award file (§0.4).
- **`data/reference/id_year1_awardees.csv` — 1 row (Session 44).** Idaho's one
  named awardee: **Comagine Health**, Maternal and Child Health Initiatives,
  *"Anticipated award start date: Aug. 14, 2026"*, **no amount**. An INTENT in
  Idaho's own future tense. `NONPROFIT_CBO` on §8's standing fallback, which
  is the RIGHT answer here because Idaho states no form — **the control that
  shows Delaware's override is about evidence and not about a weak
  classifier**. Idaho contributes to **no hospital bucket at all**. Rebuild
  with `Rscript R/03am_id_year1_awardees.R --build`.
- **`data/reference/oh_year1_awardees.csv` — 1 row (Session 44), AND IT IS THE
  ONLY SINGLE-ROW PRICED STATE FILE.** **Ohio University, $10,000,000**,
  announced by Governor DeWine 2026-07-01 — `UNIVERSITY_OR_AHC` →
  `NON_HOSPITAL`, so **$0 of hospital money** (Maine's UNE row). **ITS CMS
  FOOTER PRINTS $10,000,000, WHICH IS THE SUBAWARD** — not the allotment and
  not a pool, the first such footer in this repository, and one the tier check
  cannot classify because it collides with no allotment (§0.2). **A PARTIAL
  YEAR**: *"Additional contracts will be awarded in the coming months"*.
  Rebuild with `Rscript R/03an_oh_year1_awardees.R --build`.
- **`data/reference/ms_year1_awardees.csv` — 167 rows (Session 46), AND IT IS
  A PARTIAL YEAR.** Mississippi's three announced programmes, parsed from the
  Governor's 2026-09-14 release: **RTG 97 / $47,406,374.18, RCGC 43 /
  $43,334,338.62, TCE 27 / $13,374,434**. `sum(amount)` is **$104,115,146.80**
  against a stated $104,115,146 — a **$0.80 truncation**, and neither figure is
  corrected (§8). **READ `award_pool` FIRST, AND REMEMBER 167 ACTIONS ARE 103
  DISTINCT AWARDEE STRINGS** — Memorial Health System alone holds twelve, so
  this is one row per AWARD ACTION and never per organisation (Michigan's
  lesson). **68 rows / $47,454,812.18 are named hospitals**, against **96 rows
  / $56,022,017.62 on §8's standing fallback** — one-directional, ceiling
  $103,476,829.80, **nothing promoted (§0.4)**. Two rows break the ordinary
  `<org> - <county> - $<amount> - <desc>` shape and **only one costs a
  dollar**; a third prints "Lafayette Co." for "County". Rebuild with
  `Rscript R/03ak_ms_year1_awardees.R --build`.
- **`data/reference/ms_year1_status.csv` — 11 rows (Session 46), AND IT STILL
  HAS NO `amount` COLUMN.** The per-recipient money lives in
  `ms_year1_awardees.csv`; a status table that acquired an amount column would
  be two files claiming one figure (Texas's device, Arkansas's reason for
  keeping its project file out of the union). **THREE rows now read
  `AWARDED_ROSTER_PUBLISHED`** where session 44 had `SELECTED_NOT_ANNOUNCED`;
  **TWO read `CLOSED_AWARD_DATE_PUBLISHED_PENDING`** and carry the window
  Mississippi dated itself (**2026-10-14 .. 2026-10-29**); one records the two
  October opportunities. The **Governor's newsroom** row has inverted — it no
  longer proves the channel CAN carry an award announcement, it proves it
  DOES — and DOM's Completed Procurements page still carries the RHTP-titled
  **HORNE LLP** consultant award of 2025-08-13, **138 days BEFORE the NOA**,
  which session 20's sweep had already flagged `PROVENANCE_PREDATES_NOA`.
- **`data/reference/sc_year1_awardees.csv` — 228 rows (Session 47), AND IT IS A
  PARTIAL YEAR.** South Carolina's four awarded initiatives, parsed from *"SC
  RHTP Year 1 Award List"* (posted 2026-09-15): Connections to Care 62,
  Leveling Up 8, Wellness Within Reach 54, Shoring Up to Sustainability 104.
  `sum(amount)` is **$167,299,900.69** — **and the document states NO total of
  any kind**, so that figure is the sum of the rows and its corroboration comes
  from outside: CMS's own *"will support 228 grants"* and *"$167 Million"*
  (2026-09-17), plus each project numbering 1..n with no gap. **READ
  `award_pool` FIRST, AND REMEMBER 228 ACTIONS ARE 130 AWARDEE STRINGS AND
  FEWER ORGANISATIONS** — Newberry County Memorial Hospital alone holds TWELVE
  across FOUR projects, and several bodies are spelled more than one way
  (*"Allendale County Hospital"* / *"... (ACH)"*), so 130 is an UPPER BOUND.
  **55 rows / $56,587,137.77 are named hospitals**, against **150 rows /
  $92,759,791.23 on §8's standing fallback** — one-directional, ceiling
  $149,346,929.00, **nothing promoted (§0.4)**. `round_amount` is `NA` on every
  row. Rebuild with `Rscript R/03ao_sc_year1_awardees.R --build`.
- **`data/reference/sc_year1_status.csv` — 9 rows (Session 47), AND IT HAS NO
  `amount` COLUMN.** Four initiatives reading `AWARDED_ROSTER_PUBLISHED`; the
  **Tech Catalyst Fund** reading `NOT_YET_OPENED` (the fifth of five, worth
  $32,730,351, administered through the South Carolina Research Authority —
  §7's designated pass-through route, which has named nobody); **MB# 26-026**
  reading `SUPERSEDED`, which is the email-notification shape session 39 found
  being retired by the roster; and **BOTH §0.1 NEGATIVE CONTROLS**, the $48.2M
  ...
  (2023-06-23), both linked from the RHTP page and both disposed of by §6.2's
  date test alone. **THE NINTH ROW IS THE OPEN QUESTION ITSELF**:
  `SC_RECIPIENT_FORM_NOT_STATED`, 150 rows / $92,759,791.23, recorded here
  because a verification pass is in progress in a separate workbook and it is
  **deliberately NOT in `classification_review_queue.csv` this session**.
- **`data/reference/sc_rcj_candidate_disposition.csv` — 1 row (Session 47).**
  South Carolina carries **ZERO** Tier 3 candidates against 33 RCJ records
  while publishing 228 priced award actions — Florida's shape a fifth time, and
  at $167.3M the third largest of the five after Florida and Wyoming. **It is
  not quite their shape**: those four were invisible to BOTH layers, while
  South Carolina reached the trigger list through CMS's newsroom on 2026-09-17,
  **two days AFTER SCDHHS had already posted the roster**, so the trigger
  followed the publication rather than finding it.
...
- **`data/reference/nh_rcj_candidate_disposition.csv` — 5 rows (Session 29).**
  ...
  **documents**, not awards. **16 are administrators for whom no reachable
  ...
  run.
- **`data/reference/ca_year1_status.csv` — 6 rows (Session 34), AND IT HAS NO
  `amount` COLUMN.** What each California RHTP channel publishes, with an
  assertion refusing an amount column (Texas's device), because California has
  named no recipient and published no per-recipient figure.
  **`ca_year1_awardees.csv` does not exist and a test asserts its absence.**
  Four rows are CalRHT pools, all `CLOSED_UNAWARDED` and all with award dates
  **inside the fortnight this ran**. One is HCAI's newsroom — the CHANNEL
  control, which carries award announcements in a recognisable form and
  mentions CalRHT **zero** times. **The sixth row is the SRHRP and is
  `AWARDED_BUT_NOT_RHTP`**: it publishes a real roster, for a state
  cigarette-tax seismic programme, and its `publishes_roster` reads *"Yes — FOR
  A DIFFERENT PROGRAMME"*. The pool to watch is **Accelerator Partners, up to
  $39,010,000**, whose own guide calls it *"support for **hospitals** or other
  organizations in rural regions"* — hospitals AMONG OTHERS, New Hampshire's
  FHC class and not Illinois's, so §0.3 governs it either way.
- **`data/reference/ca_rcj_candidate_disposition.csv` — 1 row (Session 34).**
  Why each of RCJ's 11 California Tier 3 candidates is **not** an RHTP subaward.
  **All 11 are the Small and Rural Hospital Relief Program and ALL ELEVEN ARE
  NAMED CALIFORNIA HOSPITALS** — Community Memorial Hospital Ojai, George L Mee
  Memorial (twice), Hazel Hawkins Memorial, Kern Valley Healthcare District,
  Mad River Community (twice), Mountains Community, Oak Valley Hospital
  District, Plumas District (twice) — **$5,475,000**, every row a real executed
  award from **the same agency that administers CalRHT**. Nothing about them
  looks wrong. What disqualifies them is the programme: HCAI's own page names
  the **California Electronic Cigarette Excise Tax** (HSC 130075) and the
  **Alfred E. Alquist Hospital Facilities Seismic Safety Act** (HSC 129675).
  **THE AGGREGATOR'S OWN DESCRIPTIONS SAY SO AND NOBODY READ THEM**: *"MTCAP,
  MTCAR, NPC 5 Evaluation ... SPC-4D Evaluation"* are seismic engineering
  deliverables. **And RCJ carries COMPONENTS, not GRANTS** — George L Mee's two
  rows at $500,000 and $280,000 are HCAI's own published **$780,000** grant, so
  even the row count is not the award count. The counts are re-derived from
  `stage2_record_table.rds` on every run, and the disposition **refuses** a
  candidate it does not cover.
- **`data/reference/ct_year1_status.csv` — 6 rows (Session 35), AND IT HAS NO
  `amount` COLUMN.** What each Connecticut RHTP channel publishes, with an
  assertion refusing an amount column (Texas's device), because Connecticut has
  named no recipient and published no per-recipient figure.
  **`ct_year1_awardees.csv` does not exist and a test asserts its absence.**
  The row that matters is **NOFO #26OHS001**, whose stage reads
  `CLOSED_AWARD_DATE_PASSED`: OPM's own published timeline gives *"Grant Awards
  Announced ... **August 17, 2026**"* and this ran 2026-09-02. Its eligible
  class LEADS WITH HOSPITALS — *"Hospitals and health systems, federally
  qualified health centers (FQHCs), behavioral health providers..."* — which is
  hospitals AMONG OTHERS, New Hampshire's FHC class and not Illinois's, so §0.3
  governs it either way. One row is the **CTsource Contracting Portal** and
  reads `UNREADABLE`/`UNKNOWN`: the state directs subrecipients there and it is
  an external stateful application this environment cannot search, so whether an
  RHTP contract has been executed inside it is a statement about **our access**,
  never about Connecticut (§0.4).
- **`data/reference/ct_rcj_candidate_disposition.csv` — 3 rows (Session 35).**
  Why each of RCJ's 7 Connecticut Tier 3 candidates is **not** an RHTP subaward.
  **All seven are BUDGET-NARRATIVE LINE ITEMS** — four are state agencies named
  in the narrative's own *"Required reporting information for **subrecipient**"*
  sections (DPH $21,714,915, OHS $7,689,978, DMHAS twice), two are one planned
  contractor (*"Contractor 1 Carelon Behavioral Health, Inc."*, $3,800,000
  **counted twice**), and one is a proposal name (*"Proposal: W03-Area Health
  Education Center (AHEC) Expansion"*, §6.1's `PROGRAM_NAME_AS_AWARDEE`).
  Oklahoma's TIER defect, **plus a mechanism this project had not recorded: RCJ
  PRICES DOCUMENT REVISIONS AS SEPARATE AWARDS.** Connecticut published the
  narrative twice, so Carelon's single line is carried twice and DMHAS's single
  line is carried at **two different amounts** ($5,749,236 and $5,600,000) —
  New Hampshire's CDFA defect caused by REVISION rather than by spelling. The
  seven sum to **$49,854,129**, which is `rcj_state_survey.csv`'s own figure for
  Connecticut, and that reconciliation is **asserted**. The counts are
  re-derived from `stage2_record_table.rds` on every run, and the disposition
  **refuses** a candidate it does not cover.
- **`data/reference/nm_year1_status.csv` — 8 rows (Session 35), AND IT HAS NO
  `amount` COLUMN.** What each New Mexico RHTP channel publishes, with an
  assertion refusing an amount column. **`nm_year1_awardees.csv` does not exist
  and a test asserts its absence.** Six rows are RHT procurements and **not one
  is awarded, in HCA's own words**: four read *"Currently under evaluation"*,
  Rooted in New Mexico *"Submissions due: September 4, 2026"* and the Rural
  Health Data Hub *"Submissions due: TBD"*. **The eighth row is the RURAL HEALTH
  CARE DELIVERY FUND and is `AWARDED_BUT_NOT_RHTP`** — it publishes a real named
  roster per cycle, for a **New Mexico state fund**, and its `publishes_roster`
  reads *"Yes — FOR A DIFFERENT PROGRAMME"*. The pool to watch is **Healthy
  Horizons, $76.2M and six regional HUBS**, which *"direct funding to providers,
  Tribal health programs, community organizations..."* — Missouri's ToRCH shape
  with a class of providers AMONG OTHERS, so §0.3 governs it and it is a
  `PASS_THROUGH_*` question when it lands.
- **`data/reference/nm_rcj_candidate_disposition.csv` — 1 row (Session 35).**
  Why each of RCJ's 7 New Mexico Tier 3 candidates is **not** an RHTP subaward.
  **All seven are the Rural Health Care Delivery Fund, and they fail three ways
  at once.** (1) **The wrong PROGRAMME**: the Governor's own release says *"41
  rural health care providers and facilities will receive a combined **$50
  million in state funding** from the Rural Health Care Delivery Fund"*, a fund
  *"originally established in **2023**"* topped up *"during the **October 2025
  special session**"* — a state appropriation predating the 2025-12-29 NOA — and
  HCA's own RFA deck calls it *"a $50 million **STATE** investment"* while
  containing RHTP, "Rural Health Transformation", "CMS" and "federal" **ZERO
  TIMES EACH**. HCA's site architecture agrees: the RHCDF sits under the Primary
  Care Council, and its page's three mentions of "Rural Health Transformation"
  are **ALL THE NAVIGATION MENU**, with **no CMS footer at all**. (2) **The
  wrong SECTION** (Nebraska's defect): every row is filed under a **FUTURE**
  opportunity's title while the names are a **PAST** award roster further down
  the same page — and the capture is partial, taking seven of the roster's first
  EIGHT names and **dropping Gallup Community Health**. (3) **The $1
  PLACEHOLDER** (Missouri's, Maine's): every row priced at $1, so New Mexico's
  whole `rcj_federal_amount_sum` is **$7** — **and that is what HIDES the other
  two**, because a row priced at $1 reads as missing data rather than as the
  wrong programme. **TWO of the seven are named New Mexico hospitals** (Alta
  Vista Regional Hospital, Cibola General Hospital), so only the placeholder
  keeps this from being California's $5,475,000. The counts are re-derived from
  `stage2_record_table.rds` on every run.
- **`data/reference/la_year1_status.csv` — 9 rows (Session 36), AND IT HAS NO
  `amount` COLUMN.** What each Louisiana RHTP channel publishes, with an
  assertion refusing an amount column (Texas's device).
  **`la_year1_awardees.csv` does not exist and a test asserts its absence.**
  **SEVEN of the nine rows read `CLOSED_AWARD_DATE_PASSED`** — every Budget
  Year 1 solicitation, each against the *"Notice of Intent to Contract
  Announcements"* window LDH published for it. The pool to watch is **Rural
  Health Facilities Capital Improvement, $41.60M projected, 160 applications,
  awards of $100,000–$10,000,000**, whose eligible class is *"Rural Health
  ...
  **Critical Access Hospitals (CAHs), Rural hospitals**, Rural EMS providers,
  ...
  practices"* — hospitals AMONG OTHERS, New Hampshire's FHC class and not
  Illinois's, so §0.3 governs it. One row is the **Rural Health Atlas
  (rhtla.net)** and reads `UNREADABLE`/`UNKNOWN`: an unrendered Vue application
  this environment cannot execute, so what it surfaces is a statement about
  **our access**, never about Louisiana (§0.4).
- **`data/reference/la_rcj_candidate_disposition.csv` — 1 row (Session 36).**
  Why each of RCJ's 6 Louisiana Tier 3 candidates is **not** an RHTP subaward.
  **All six are rows of ONE table** — slide 18 of the Advisory Council deck,
  headed *"RHTP Funding Cycle Budget Year 1"*. The "awardee" is its **ACTIVITY**
  column (*Alternative Payment Model*, *Care conveners / navigation network*,
  *Food is Medicine*, *Telehealth*, *Rural Clinician Credit Bank*,
  *Collaborative Provider Model*) — **fund uses, not organisations**, so §6.1's
  `PROGRAM_NAME_AS_AWARDEE` on **six of six**, and `named_recipient_test` reads
  **PASS** on every one. The amount is its **"Projected BY 1 Funding"** column
  times a million: Oklahoma's TIER defect, and where Oklahoma's glossary had to
  be read, **Louisiana says PROJECTED in the column heading**. **AND IT DROPS
  THE LARGEST ROW** — Capital Improvement, $41,600,000 — so the six sum to
  $53,910,000 against the deck's seven at $95,510,000, understating the table
  it mined by **$41,600,000**, and the row it drops is the **capital** one.
  Three things are asserted together, which is what makes the tier defect
  provable rather than claimed: the names match the ACTIVITY column, the
  amounts match the PROJECTED column **to the dollar**, and the gap to the
  deck's total is **exactly** the Capital row. The counts are re-derived from
  `stage2_record_table.rds` on every run.
- **`data/reference/ky_year1_status.csv` — 11 rows (Session 37), AND IT HAS NO
  `amount` COLUMN.** What each Kentucky RHTP channel publishes, with an
  assertion refusing an amount column (Texas's device).
  **`ky_year1_awardees.csv` does not exist and a test asserts its absence.**
  **TWO rows read `CLOSED_AWARD_DATE_PASSED`** — the CMHC Support RFA
  (*"July 10, 2026: Notification of Award to Grantees"*) and the Community
  Health Worker Specialized Certificate RFA (*"August 26, 2026 Anticipated
  Notification of Award to Recipients"*), both re-derived against `Sys.Date()`
  every run rather than typed. One row is the **Foundation for a Healthy
  Kentucky**, the designated pass-through admitted by §7 (Illinois/ICAHN's
  route), which describes the Hub Lead **role** and names no Hub Lead. The
  pool to watch is **Rural Community Hubs for Chronic Care Innovation**, whose
  Hub Lead RFA closed 2026-06-01 for three Area Development Districts.
- **`data/reference/ky_rcj_candidate_disposition.csv` — 1 row (Session 37).**
  Kentucky carries **ZERO** RCJ Tier 3 candidates against 53 RCJ records in
  total, all `SOLICITATION`, `STATE_ALLOTMENT` or `UNASSIGNED`. **The
  aggregator is RIGHT about Kentucky**, which is the unusual case: the state
  has awarded nobody publicly, so there is nothing to get wrong. A zero here is
  a fact about the DISCOVERY LAYER and never about the state (§0.1) — Florida
  had 81 awards and no candidate either. The count is re-derived from
  `stage2_record_table.rds` on every run.
- **`data/reference/ny_year1_status.csv` — 6 rows (Session 37), AND IT HAS NO
  `amount` COLUMN.** **`ny_year1_awardees.csv` does not exist and a test
  asserts its absence.** The row that matters is **RCHI**, reading
  `CLOSED_AWARD_DATE_PASSED`: **$76,190,022** for Budget Period 1, applications
  closed, and its own guidance saying *"Contracts for funded grantees will
  begin on **September 1, 2026**"* against a run on 2026-09-02. **READ ITS
  ELIGIBLE CLASS BEFORE CODING ANY NEW YORK PASS-THROUGH**: *"A hospital must
  be included as either the lead applicant or the partner Organization"* —
  hospitals MANDATORY and not necessarily the recipient, which is neither
  Illinois's nor New Hampshire's class. One row is the **NYS Contract
  Reporter** and reads `UNREADABLE`/`UNKNOWN`: a stateful search application
  behind a free account, so whether an RHTP contract has been executed inside
  it is a statement about **our access**, never about New York (§0.4).
- **`data/reference/ny_rcj_candidate_disposition.csv` — 1 row (Session 37).**
  New York carries **ZERO** Tier 3 candidates against 21 RCJ records — the
  fewest of any state investigated so far — and that is correct: it has awarded
  nobody publicly.
- **`data/reference/nc_year1_awardees.csv` — 44 rows (Session 38), AND
  `amount` IS EMPTY ON EVERY ONE.** North Carolina's two published rosters:
  **Mobile Integrated Health 39** (announced 2026-06-08, *"$10 million to 39
  local EMS agencies"*) and **NC ROOTS Hub Leads 5** (2026-05-01, five
  organisations across **SIX** regions — Trillium holds Regions 2 and 5, so a
  row count is neither a region count nor an organisation count). **Read
  `award_pool` and `round_amount` before using any figure, and NEVER sum
  `round_amount` down the column** — the MIH pool repeats on each of its 39
  rows, so the column sums to **$390,000,000** for a round of $10,000,000
  (Georgia's trap, Nevada's device; `nc_reconcile()` sums distinct pools).
  **THE ROOTS ROWS CARRY NO `round_amount` AT ALL**, deliberately: NCDHHS
  publishes no per-hub and no ROOTS pool figure, and the only currency on
  either ROOTS document is the **$213,008,356.47 ALLOTMENT** (§0.2). **North
  Carolina contributes 0 rows and $0 to ALL THREE hospital buckets** — its 39
  MIH rows are `EMS_OR_PSAP`/`No` (37 of them; two are queued) and its 5 Hub
  Leads are `PASS_THROUGH_UNRESOLVED`/`Unclear`. **AND THE UNSTATED-FORM
  QUESTION IS THE NINTH AND BY FAR THE SMALLEST: 5 of 44 rows.** Kansas,
  Maryland, Nebraska, Oklahoma, Nevada, Michigan, Missouri and Iowa carry 22 to
  102 rows on §8's standing fallback, because their publishers give a name and
  no form. North Carolina's recipients mostly **state their own form in their
  names** — 37 of the 39 read "<County> County EMS", which §8's name rule takes
  at `HIGH` and which agrees with NCDHHS's own class sentence, two independent
  readings. That agreement is what makes the two exceptions visible rather than
  two of thirty-nine unknowns. Rebuild with
  `Rscript R/03ah_nc_year1_sources.R --build`.
- **`data/reference/nc_year1_status.csv` — 6 rows (Session 37), AND IT HAS NO
  `amount` COLUMN — BUT NORTH CAROLINA IS NOT A NEGATIVE.** Two rows read
  `AWARDED_ROSTER_PUBLISHED` and carry a `named_recipients` count: **39**
  Mobile Integrated Health recipients ($10,000,000, 2026-06-08) and **5** NC
  ROOTS Hub Leads (2026-05-01), **44 named recipients between them and NOT ONE
  PER-RECIPIENT DOLLAR**. **`nc_year1_awardees.csv` EXISTS AS OF SESSION 38** —
  the extraction was deliberately deferred in session 37 so it could be
  reported first, and `nc_assert_not_extracted()` has been retired for its
  opposite, `nc_assert_extracted()`. What `R/03ah`'s header said an extraction
  had to get right, and what it got. The amounts are POOL figures — `amount` is
  empty on all 44 rows and the MIH pool sits in `round_amount`, the ROOTS
  round carrying no figure at all. **Cape Fear Valley** is the one recipient
  among the 39 that does not read as a county EMS agency and was **NOT
  promoted** (§0.4) — nor was **Clay County**, the second name carrying no EMS
  token, which is why session 37's *"38 of 39"* is really **37**; both are
  queued. **UNC's two spellings CLASSIFY DIFFERENTLY**, so the merge §2 forbids
  would change a coding and not just a count; NCDHHS's own *"public academic
  medical center"* settles it as `UNIVERSITY_OR_AHC`. And the ROOTS page's only
  currency figure is the **ALLOTMENT** (§0.2), which is why those five rows'
  `round_amount` is deliberately `NA`. North Carolina now reads **`EXTRACTED`** in both
  rebuilt tables — never `INVESTIGATED_NO_LIST`, which would have been a false
  claim about a state with two rosters.
- **`data/reference/ar_year1_awardees.csv` — 37 rows (Session 40).** Arkansas's
  Year 1 award actions, one per **organisation x initiative**, parsed from
  DF&A's own *"List of Organization and Award amounts"* with session 32's RUN
  model — its three amount columns are painted at one y, so the line model
  welds them into an unparseable string. **`sum(amount)` is $149,177,618.45,
  which is Arkansas's own `Total:` row to the cent**, and the two initiative
  columns are $55,713,829.20 (THRIVE) and $93,463,789.25 (PACT). **READ THE
  THREE COUNTS BEFORE QUOTING ANY OF THEM: 31 organisations, 37 award actions,
  50 projects**, and Arkansas publishes no organisation-to-project map.
  **NEVER SUM `organisation_award_total` DOWN THE COLUMN** — it repeats on both
  rows of the six organisations holding an award under both initiatives, so the
  column sums to **$250,274,844.36** (Georgia's trap; `ar_reconcile()` does it
  correctly and an assertion refuses the naive total). **Every row is an
  INTENT**: DF&A signs an official agreement with each grantee afterwards, so
  all 37 are `NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No` +
  `AMOUNT_PRELIMINARY`. **16 organisations / 21 rows / $100,723,693.49 carry
  §8's standing fallback** against a $21,792,687.96 named-hospital floor.
  Rebuild with `Rscript R/03ai_ar_year1_awardees.R --build`.
- **`data/reference/ar_year1_projects.csv` — 50 rows (Session 40), AND IT IS
  THE SAME MONEY AS THE AWARD FILE AT A FINER GRAIN.** The Governor's
  2026-08-27 release prices **all 50 project awards individually** and
  describes each; the 50 sum to the award list's three figures **to the cent**,
  which is the strongest corroboration in this repository outside Florida's and
  was arranged by nobody. **NEVER ADD IT TO `ar_year1_awardees.csv`**: those 37
  rows and these 50 are one $149,177,618.45, so adding them DOUBLES the state.
  It is deliberately **not** in `test_state_union.R` — Missouri's and Maine's
  device, but for the opposite reason: those files are not awards at all, while
  this one is the same awards twice. It exists for the project DESCRIPTION,
  which the award list does not carry, and it keeps the release's own spellings
  in `awardee_as_published` beside the award list's in `awardee`, because
  **nine organisations are spelled differently and one pair classifies
  differently** — *"Arkansas Children's Hospital"* is a named hospital worth
  $301,400 and *"Arkansas Children's"* is not (§2 forbids merging them away).
- **`data/reference/ar_year1_status.csv` — 4 rows (Session 40), AND IT HAS NO
  `amount` COLUMN.** What each of Arkansas's four initiatives has published.
  THRIVE and PACT read `AWARDED_ROSTER_PUBLISHED`; **RISE AR and HEART read
  `CLOSED_NO_AWARD_DATE_PUBLISHED`** — applications closed 2026-07-24 and
  2026-08-07 and Arkansas publishes no award date for either, which is
  Missouri's and North Carolina's footing. Its pool figures live in
  `initiative_awarded_total`, never `amount`, so nothing summed out of it can
  read as a per-recipient figure (Texas's device). **The initiative PAGES
  cannot be used as a signal** — all four carry the same forward-looking
  sentence, including the two that have awarded $149M.
- **`data/reference/ar_rcj_candidate_disposition.csv` — 1 row (Session 40).**
  Arkansas carries **ZERO** Tier 3 candidates and had no CMS release either —
  `trigger_source = NEITHER` on **both** discovery layers — while publishing 37
  priced award actions and $149,177,618.45. **A zero here is a fact about the
  DISCOVERY LAYER and never about the state (§0.1)**, and Arkansas is the third
  proof of that after Florida and North Carolina.
- **`data/reference/wy_year1_awardees.csv` — 75 rows (Session 42).** Wyoming's
  Year 1 award actions, from **SIX recipient-level tables inside ONE document**
  — the Rural Health Transformation Advisory Committee's *"Award Approvals -
  8.11.26"*, read with session 32's RUN model because six of its rows carry the
  recipient name in a SEPARATE PAINTED RUN. **`sum(amount)` is $135,241,491.74**;
  the six tables approve **$135,508,491.74** between them and the difference is
  the two care-coordination rows that NAME NOBODY, whose figures sit in
  `round_amount` (§6.2, Oklahoma's ROOTS device). **NEVER SUM `round_amount`** —
  it repeats an initiative's pool on every row of it and the column totals
  **$2,177,412,637** (Georgia's trap); `wy_reconcile()` does it correctly.
  **READ `award_pool` AND `award_source` FIRST**, and remember that another
  **$30,877,990** — 15.1% of Wyoming's Year 1 — is approved at POOL level
  naming nobody at all. **Every row is a COMMITTEE APPROVAL, not an executed agreement**:
  Wyoming's obligation deadline is END OF OCTOBER 2026, so all 75 are
  `NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No` + `AMOUNT_PRELIMINARY`.
  **31 rows / $72,661,323.90 are named hospitals**, against 24 of the 75
  approvals-table rows / $5,716,842 on §8's standing fallback. Rebuild with
  `Rscript R/03aj_wy_year1_awardees.R --build`.
- **`data/reference/wy_year1_status.csv` — 15 rows (Session 42), AND IT HAS NO
  `amount` COLUMN.** What each of Wyoming's fifteen budget-summary lines has
  published, with its pool figures in `initiative_original_budget` and
  `initiative_approved_award` so that nothing summed out of it can read as a
  per-recipient amount (Texas's device). **SIX lines read
  `APPROVED_ROSTER_PUBLISHED`; TWO read
  `APPROVED_TO_NAMED_ADMINISTRATOR_NO_SUBRECIPIENT` (the Wyoming Innovation
  Partnership, $38,618,260, which IS in the award file); and SEVEN read
  `APPROVED_AT_POOL_LEVEL_NO_ROSTER` and total $30,877,990** — the fiscal-agent
  sweep ($17,612,195), three competitive RFPs, 4.3, State Policy Action support
  and administration. That is
  a fact about what Wyoming has PUBLISHED and never a claim that it awarded
  nothing (§0.4).
- **`data/reference/wy_rcj_candidate_disposition.csv` — 3 rows (Session 42).**
  Wyoming carries **ZERO** Tier 3 candidates against 29 RCJ records while
  publishing 75 priced award actions — Florida's shape a fourth time after North
  Carolina and Arkansas, and the fourth proof that a zero here is a fact about
  the DISCOVERY LAYER and never about the state (§0.1). **AND FIVE OF THE 29 ARE
  UTAH'S DOCUMENTS**, including *"Utah RHTP Cooperative Agreement Award: $195.7
  million for Year 1"* — Utah's own allotment, carried as an `UNASSIGNED`
  WYOMING row. §0.1 **FAILURE MODE 6**; see `rcj_state_attribution_sweep.csv`.
- **`data/reference/rcj_state_attribution_sweep.csv` — 23 rows (Session 42), AND
  IT IS A READING PROMPT RATHER THAN A FILTER.** Every committed RCJ record
  whose only named US state is a state OTHER than the one it is filed under, with
  a **HAND-READ `verdict`** on each. **TEN are `MISFILED` — genuinely another
  state's — in FIVE states** (WY←UT 5, ND←AR 2, MO←MI, UT←OK, WA←FL), **and NOT
  ONE OF THEM IS TIER 3**, so the wrong-state defect has not reached a single
  award file here. The other thirteen are false positives with legible causes:
  a state name inside the recipient's own legal name (*Providence Health &
  Services–Washington*, a real ALASKA awardee, 3 rows, all Tier 3), a bare county
  in a county list (6), a street address, an ethnonym, a multi-state digest.
  **Nothing downstream consumes it** and no record is re-stated. Rebuild with
...
- **`data/reference/verification_queue_2.xlsx` — THE RETURNED WORKBOOK, AND IT
  IS READ-ONLY.** 399 rows, all answered, over 500 committed award rows, 382
  organisations, 31 questions and 21 states. Nothing in this repository writes
  to it and `test_03ap` pins its SHA-256 (`3479966e…`). It was moved from the
  repository root to `data/reference/` in session 49; the bytes are unchanged.
- **`data/reference/verification_queue_2_answers.csv` — 399 rows (Session 49).**
  Every answer with the `basis_type` derived from its own basis text:
  **`GENERAL_KNOWLEDGE` 306, `ORG_WEBSITE` 80, `STATE_SOURCE` 13**. **NOT ONE
  OF THE 111 HOSPITAL ANSWERS RESTS ON A STATE SOURCE**, which is the shape of
  the question rather than a weakness in the answers. It also holds the ONE row
  that came back `UNKNOWN` (NVHA's *"93 X 95 NV"*), which is deliberately NOT a
  §8 value and was written to no state file. Rebuild with
  `Rscript R/03ap_verification_queue_2.R --apply`.
- **`data/reference/verification_queue_2_changes.csv` — 487 rows (Session 49),
  AND IT IS THE AUDIT TRAIL FOR EVERY DOLLAR THIS PASS MOVED.** One row per
  (file, row index) that changed, with `old_type`/`new_type`,
  `old_flow`/`new_flow`, `bucket_before`/`bucket_after`, the amount, the
  `basis_type`, the verifier and `flow_note` — which says per row whether the
  §10.2 flow test was re-run, left alone, or HELD. 477 rows come from the
  answers and **10 from policy 2**, which reaches organisations no queue row
  covers.
- **`data/reference/unstated_form_typing_decisions.csv` — 169 rows (Session
  50).** ONE ROW PER ORGANISATION, hand-read, with the determined form, the
  `basis_type` and the source that settles it. This is the table a reader
  audits; `UF_TYPES` in `R/03aq_unstated_form_typing.R` is its source of
  record. **NOT ONE of the 169 rests on the STATE's own document**, which is
  the shape of the question rather than a weakness in the answers: a state's
  award document names a recipient and an amount and does not say what KIND of
  organisation it is.
- **`data/reference/unstated_form_typing_changes.csv` — 241 rows (Session 50),
  AND IT IS THE OVERLAY THE COMMITTED FILES CARRY.** One row per (file, row
  index) that moved, with `old_type`/`new_type`, `old_flow`/`new_flow`,
  `bucket_before`/`bucket_after`, the amount, the `basis_type` and a
  `flow_note` saying whether §10.2 was re-run. **AFTER ANY `--build` ON
  MISSISSIPPI OR SOUTH CAROLINA, RUN `Rscript R/03aq_unstated_form_typing.R
  --apply`** — or call `uf_overlay(built, "<file>.csv")` — or the next build
  wipes 241 determinations and nothing says so.
- **`data/reference/rural_cut_rows.csv` — 939 rows, and
  `rural_cut_by_state.csv` (Session 51), AND THEY ARE A REPORT, NOT A
  CODING.** Every `NAMED_HOSPITAL` row gets one evidence class for rural status:
  `STATE_SOURCE_RURAL`, `STATE_SOURCE_NOT_RURAL`, `SITE_NOT_RECIPIENT`,
  `FEDERAL_RECORD_CCN`, `GENERAL_KNOWLEDGE_ONLY` or `NOT_RECORDED`. Only the
  first, plus CMS-enrolled CAH/REH under the fourth, counts as rural: 151 rows
  / $157,997,116.60. Nothing is inferred from a county, a pool title or
  general knowledge. Rebuild with `Rscript R/03ar_rural_cut_report.R --build`;
  its guard fails when the partition moves, and then the cut must be re-stated.
- **`data/reference/classification_review_queue.csv` — 27 rows (Sessions 19,
  20, 22, 23, 24, 25, 26, 27, 28, 33, 38, 40, 42, 46, 49, 50, 51). SESSION 51
  RESOLVED `IA_COE_POOL_IS_TIER_2` (option (b)) AND SHRANK
  `UF_FORM_NOT_DETERMINABLE` TO MISSISSIPPI'S TWO ROWS / $3,250,000.** SESSION 50
  RESOLVED THREE MORE AND ADDED THREE** — `VQ_INKIND_AFTER_RETYPE` (option
  (a), settled on the four award paragraphs' own grammar: Salina Regional is
  the only one of the four in which the AWARDEE IS NOT THE SUBJECT of the
  sentence), `MS_FOUNDATION_PARENT_NOT_STATED` (option (b), +$3,785,455.49)
  and `MS_RECIPIENT_FORM_NOT_STATED` (typed directly). The three new ones are
  `UF_FORM_NOT_DETERMINABLE` ($3,999,320 over 5 rows in 2 states),
  `SC_ACADIA_PARENT_SCOPE` ($5,565,253) and `IA_COE_POOL_IS_TIER_2`
  ($50,000,000). **Session 49 resolved thirteen and added four.**
  ...
  **Session 38 added TWO North Carolina rows, and both are worth $0 in every
  option** — North Carolina prices nobody, so what they move is its
  named-hospital ROW COUNT, which for an unpriced state is the only hospital
  quantity there is (Nevada's rule). `NC_MIH_FORM_NOT_STATED` covers **Cape
  Fear Valley Mobile Integrated Health** and **Clay County**, the two names
  among the 39 that carry no EMS token while NCDHHS's own sentence calls all 39
  *"local EMS agencies"*; Cape Fear Valley is a health system on this
  pipeline's own knowledge and the archive says nothing about it beyond the
  name, so **nothing was promoted (§0.4)** and option (c) is the only one that
  would give North Carolina a named-hospital row at all.
  **`NC_HUB_LEAD_FORM_NOT_IN_VOCABULARY` is a condition this project had not
  recorded: THE SOURCE STATES A FORM §8 DOES NOT CARRY.** NCDHHS calls
  Trillium *"an NC Medicaid Tailored Plan and Managed Care Organization
  (MCO)"*, Vaya *"a public NC Medicaid Managed Care Organization (MCO)"* and
  Access East *"a comprehensive care management provider"* — which is NOT the
  unstated form eight other states raise, because the state HAS stated it and
  §8 has no code for the answer. The other two Hub Leads' stated forms §8 DOES
  carry, and both were typed from the source — Impact Health *"an independent
  501(c)3"* → `NONPROFIT_CBO`, UNC Health *"a public academic medical center"*
  → `UNIVERSITY_OR_AHC` (Oregon's OHSU precedent).
  **SESSION 39 ANSWERED TWO OF THE THREE AND THE ROW NOW HOLDS ONE.** §8 gained
  `MANAGED_CARE_ORGANIZATION` on session 10's `PHYSICIAN_PRACTICE` footing, so
  Trillium and Vaya — the two NCDHHS actually calls a Managed Care Organization
  — are typed from the state's own sentence at `MEDIUM`. **ACCESS EAST IS NOT
  AN MCO AND THE CODE WAS NOT WIDENED TO REACH IT (§0.4)**: *"a comprehensive
  care management provider"* is a different thing in North Carolina's own
  Medicaid vocabulary, so that row keeps §8's standing fallback with the
  state's own sentence in `recipient_type_source` and the question stays
  **OPEN**. Still **$0 either way** — an MCO is not a hospital type, so the
  retyping moved no dollar and no row count, which is what made the vocabulary
  addition safe to make.
  **Session 40 added TWO Arkansas rows, and between them they are the largest
  open money in this queue.** `AR_RECIPIENT_FORM_NOT_STATED` is the
  unstated-form question a **NINTH** time and **by far the largest in dollars —
  16 organisations / 21 award rows / $100,723,693.49, 67.5% of everything
  Arkansas has awarded** — against a named-hospital floor of 9 rows /
  $21,792,687.96. Like Oklahoma's and Michigan's it is **one-directional**
  (every one of the 21 is already `No`), so it states a genuine CEILING of
  **$122,516,381.45**, and **four of the six largest awards in the state are
  inside it**: Baxter Health $19,738,729.91, Mercy Health Fort Smith
  $19,056,249, Baptist Health $16,926,432, St. Bernards $14,551,090. Every one
  reads as a hospital to anyone who knows Arkansas and **not one is typed by
  the source** — DF&A publishes a recipient, an amount, and nothing else.
  **NOTHING WAS PROMOTED (§0.4)**, and a test prices the counterfactual at
  $89,106,021.91. Option (c) — typing them from the Governor's project
  descriptions — is **REFUSED** in the row rather than left open, because a
  description describes an ACTIVITY and §0.3a judges the RECIPIENT.
  **`AR_ARHP_CONSORTIUM_FLOW` is worth $18,833,521 and is a DIFFERENT
  QUESTION** — §10.2's, not §8's. Arkansas Rural Health Partnership is a
  hospital consortium on this pipeline's own knowledge and **not** on the
  document's, and §10.2's association row turns on what the source says the
  money DOES: administered to member hospitals (`PASS_THROUGH_DESIGNATED`,
  `Yes`) or spent on goods and services for them (`IN_KIND_BENEFIT`, `No`).
  The award list says **neither**, so both rows carry
  `FLOW_UNRESOLVED_HOSPITAL_AFFILIATED` and sit in **neither** bucket —
  Michigan's `MI_MHA_FLOW` footing. It is counted inside the ceiling above and
  must not be added twice.
  **Session 42 added TWO Wyoming rows, both one-directional and worth
  $2,452,302 upward between them.** `WY_TECH_FORM_NOT_STATED` covers Initiative
  3.1's four fallback rows ($1,535,722): its table publishes **no
  organisation-type column AND no EIN column**, so *"Powell Valley Health Care"*
  cannot be joined to *"Powell Valley Health Care Inc"* in Wyoming's OWN 1.1
  hospital table by any means §2 permits — worth **$752,302**, and **nothing was
  promoted (§0.4)**. It also records, rather than repairs, that *"Bighorn Valley
  Health Center, Inc. dba One Health"* is the IDENTICAL STRING in Initiatives
  3.1 and 4.1 and classifies differently in the two, because only 4.1's table
  states an eligible class — worth $0 either way.
  `WY_EMS_LEAD_AGENCY_FORM` is **$1,700,000**: *"Campbell County Health (CCH)
  Emergency Medical Servic"* is TRUNCATED by the producer at its cell edge and
  its EIN (83-0234097) is not in the 1.1 hospital table, so the EIN join that
  carries Wyoming's stated hospital form into 1.2 does not reach it — while
  *"Campbell County Hospital District dba Campbell C"*, a different string with
  no EIN published, IS typed a hospital on its name.
...
- **`data/reference/ak_year1_awardees.csv` — 244 rows (Session 46).** Alaska's
  rolling notice of intent to award, **a snapshot at 2026-09-21 and stale by
  construction** — DOH overwrites one url weekly, so read the fetch date before
  using any figure. Every row `amount_confirmed = No`; 39 carry
  `RECIPIENT_TYPE_VARIES_IN_SOURCE`. **THREE snapshots stay committed and they
  are not interchangeable**: 2026-08-28 is `AK_CMS_ANCHOR_FILE`, the file CMS
  described when it said 142 projects, and it does NOT roll; 2026-08-31 is
  `AK_PRIOR_FILE`, which rolls on every refresh and is what makes the growth a
  diff of two archived documents; 2026-09-21 is the current one. Session 46
  split the first two out of one constant — rolling the conflated version would
  have moved the 142-project reconciliation onto a file CMS never described. Rebuild with `Rscript R/03h_ak_year1_awardees.R --build`.
...
2. **The §9.11 findings ARE in the repo, and this blocker's own text was stale
   for thirty-six sessions.** It said *"no findings document was committed, so
   the eleven records cannot be re-read here"* — but **session 7 resolved
   exactly that** ("the §9.11 blocker's evidence landed as `DE Verify.xlsx`, 11
   hand-verified Delaware records"), and session 43 re-read all eleven from it
   without difficulty. `DE Verify.xlsx` sits at the repo root: 11 rows, 23
   columns, carrying `awardee`, `rhtp_award_yn`, `hospital_yn`, `aha_id`,
   `match_confidence`, `payment_status` and free-text `notes`.

   **What IS still open is narrower and worth stating as itself: the workbook
   is a loose xlsx at the repo root rather than a reference table, nothing
   reads it, and no code asserts anything about it.** So Stage 4's §9.3 design
   still has not been checked against it, and its seven mis-coded hospital rows
   (see §0.3a, re-counted session 43) sit uncorrected because no stage owns
   them. **Before Stage 4: ingest it to `data/reference/`, the way `R/03e` did
   for the owner's Florida workbook, and let Stage 5 own the re-coding.**
...
> **Session 35: 3,917 assertions across 39 files, all passing and 1
> self-skipping** (was 3,664 across 37). Two new files —
> `test_03ac_ct_year1_probe.R` (134) and `test_03ad_nm_year1_probe.R` (120).
> Both are weighted towards DRIVING each tripwire rather than merely calling
> it: every award phrase is fed to every watched surface and required to
> throw, the footer's non-strict demotion is driven in both directions, and
> the two new digest mechanisms are SYNTHESISED offline (Connecticut's
> per-node `?v=` stamp in an attribute; New Mexico's variable-length Complianz
> re-roll in a script body) so each proves the file digest moves and the
> content digest does not. Connecticut's set also pins the two-dates trap and
> the revision double-count; New Mexico's pins the $1 placeholder, the
> dropped Gallup row, and the RHCDF page's THREE navigation-only mentions.
> ONE TEST FAILED FOR THE RIGHT REASON WHILE BEING WRITTEN: a control test
> used `str_remove` where the OHS headline appears TWICE, so it passed by
> leaving the control intact — `str_remove_all` now, with the reason in a
> comment.
>
> **Session 34: 3,664 assertions across 37 files, all passing and 1
> self-skipping** (was 3,564 across 36). The new file is
> `test_03ab_ca_year1_probe.R` — 93 assertions, weighted towards the one thing
> that could go wrong: California's eleven candidates are named hospitals with
> real amounts on real executed awards, and NOTHING ABOUT THEM LOOKS WRONG, so
> the tests that carry the weight are the ones that fail if the SRHRP page ever
> stops saying what disqualifies it (its cigarette-tax funding source, its
> Alquist Act statute, and the ABSENCE of "RHTP" / "federal" from it). Plus the
> two controls on that same page, the three invisible-character findings, the
> CACHE-VARIANT and antispambot() digest mechanisms, and
> the 105-rows-102-hospitals count. The other 7 are the sweep's pins moving
> from 90 rows in 8 states to 101 in 9, with a new test requiring California's
> eleven to be caught by BOTH §6.2 filters.
>
...
**SESSION 72 LEFT THESE.** (1) `AHC_STRING_NAMES_NO_ENROLLED_ENTITY` is still open (UAB Montgomery, OHSU Casey Eye, MEDIC, the ORPRN/Perinatal strings, UMMS). UMMS's legal name matches no enrolment, so only option (c) (a source stating its form) could move it. (2) After any rebuild of AK or OR, run `R/03bj --apply` so the held rows' `cms_enrolment_record` returns.

**SESSION 71 LEFT THESE.** (Item 1 resolved in session 72 at option (a).) (1) Decide `ENROLLED_HOSPITAL_STATE_STATED_OTHER_FORM` (10 rows, $2,088,858: Alaska's tribal/state-agency typed rows and Oregon's hospital-owned RHCs), where a state source states a form CMS contradicts. (2) `AHC_STRING_NAMES_NO_ENROLLED_ENTITY`: UAB Montgomery, OHSU Casey Eye, U Michigan MEDIC, ORPRN/Oregon Perinatal, and UMMS (a system, not a university). (3) Only AR, NV, LA and WA builders chain `s71_overlay()`. After any other touched state's `--build` (AL AK OR WY MI GA NC IA OK), run `Rscript R/03bj_enrolled_hospital_operator.R --apply`. (4) A new CMS enrolment file or a new state file can add sweep hits; `eh_assert_read()` stops the build until each is hand-read into EH_APPLY or EH_HOLD.

**SESSION 70 LEFT THESE.** (1) Decide `AHC_ENROLLED_HOSPITAL_OPERATOR` (UAMS, $17,060,066). Archive hospital enrolment files for AL, OR, IA, MD, WA, UT and NC before sizing it repo-wide. OHSU, UNC and Emory must move with UAMS or none of them should. (2) Session 49's OTHER test (`verified_basis` > 10 chars) passed NARHC on a bare name, so re-read the other 34 OTHER rows for a stated form. (3) Never run a bare `R/03ap --apply` on Arkansas: it would re-write NARHC's OTHER. `R/03ai --build` re-applies `ar_resolve_narhc()` after the overlay.

**SESSION 69 LEFT THESE.** (1) `AR_R2_QUEUED_FORM`: UAMS, CARTI and NARHC, $7,068,862 in round 2. UAMS's answer also reaches round 1's $11,991,204, and NARHC's two rows now carry different types. (2) `AR_R2_RECIPIENT_FORM_NOT_STATED`: 21 orgs / $19,749,288.19. (3) The AR probe now expects TWO award-list links on the live home page and fails on a third. (4) Every Arkansas award is still an intent; DF&A agreements are due by the 2026-10-30 obligation deadline.

**SESSION 68 LEFT THESE.** (1) **EXTRACT ARKANSAS ROUND 2** from `data/evidence/recheck/2026-09-25/AR/` into `R/03ai`, on the round-1 template. Use the RUN model: the amount columns weld under the line model. Reconcile to $54,685,068.84 and both column totals. Read the Governor's RISE list for the $203,520 gap, and never force it closed. Code UAMS as a university (the OHSU precedent). ARHP goes to `AR_ARHP_CONSORTIUM_FLOW`, and St. Bernards Development Foundation to §10.2's foundation row. The AR probe trips until this is done; `ar_assert_award_index()` refuses the second list link by design. (2) After 2026-09-28 14:07Z, check `origin/main` for an AK line whose `origin` is `trig_015xk6QMPZNSY7GhC6muGdpN`. Only then delete `trig_018fYZk2Dd9sui3WqmDMefwS`. (3) Kentucky: FHKY is now a *"convening partner"*. If that holds when Hub Leads are named, it is Missouri's Hub Anchor question (is the convener a fiscal agent?), so read it off the award.

**SESSION 67 LEFT THESE.** (1) **READ ARKANSAS FIRST**: its 2026-09-24 20:22Z Routine run tripped on the home page naming "RISE AR and HEART Project Impact". RISE AR and HEART are the two unawarded initiatives (~$59.6M), so this may be their roster. Re-fetch, read, and extract or add to `known` with a justifying sentence (§2.3). (2) KY reported CHANGED on `funding` and `rch` with no tripwire; read the diff. (3) Delete Alaska's disabled old Routine (`trig_018fYZk2Dd9sui3WqmDMefwS`) once the new AK runner publishes Mon 2026-09-28 14:00Z. (4) The 21-day recency check on newsroom-only CMS rows in `test_00` stays deliberately. WV's second release did reach `BOTH`, so a row stuck newsroom-only is a medicaid.gov finding, not growth.

**SESSION 66 LEFT THESE.** (1) **PROVE TWO NEW ROUTINES PUBLISH, THEN DELETE THE 31 DISABLED ORIGINALS** (old ids in `config/routines.csv`'s `old_trigger_id`). First candidates: KY (new, Mon/Thu 19:30Z) and AR (new, Thu/Sun 20:20Z). Look for lines on `origin/main` whose `origin` is the NEW id. (2) **ALASKA HAS TWO ENABLED ROUTINES on one runner** (old `trig_018fYZk2Dd9sui3WqmDMefwS`, new `trig_015xk6QMPZNSY7GhC6muGdpN`). Disabling the old one was refused by the permission layer. **The owner must disable it before Mon 2026-09-28 14:00Z**, or that runner stops on its exactly-one check. (3) **The CMS test fix must be on `main` before Mon 2026-09-28 13:00Z**, when the new CMS Routine (`trig_011pUVRNkSnjCNvZfQ89kPbM`) runs the suite. (4) The CMS 09-24 miss is explained in `config/probe_gaps_explained.csv`.

**SESSION 65 LEFT THESE.** (1) The CMS Routine's two 13:10Z suite failures came from the data it fetched (main was green). Read the 2026-09-28 13:00Z run's failures first, then explain or fix the 09-24 miss. (2) WA's prompt lacks the 5xx retry and only its own conversation can edit it; recreate it (new id, new `routines.csv` row) or accept the risk. (3) The other 32 runner prompts lack the retry too. (4) Session 64's items (1), (3) and (4) are done.

**SESSION 64 LEFT THESE.** (1) Put the NE (`R/03r`) and NV (`R/03u`) probes on Routines; LA's probe does not watch the webinar deck that carried its award. (2) Five new queue rows: `NE_5_3_FORM_NOT_STATED`, `NE_WINNEBAGO_TRIBE_HOSPITAL_SCOPE`, `NV_RECIPIENT_FORM_NOT_STATED_0924`, `LA_RECIPIENT_FORM_NOT_STATED`, `LA_RCCB_HOSPITAL_SETTINGS_UNNAMED`. (3) Run Stage 2 so the record table carries the seven new registry flags. (4) Confirm WA's re-fired line reached `main`, or `probe_coverage.R --check` stays red. (5) VT/VA/FL/AK/NC's single missing rows and SD's 27SC091800 ($150,000, CFDA 93.798) are still unextracted. (6) `R/03r` and `R/03u` now re-apply session 49's overlay inside `--build`; `R/03v` and the others still do not.

**SESSION 63 LEFT THESE.** (1) Re-extract NV (155 named on NVHA's page), NE Initiative 5.3, MI's 6 new rows, LA's Rural Clinician Credit Bank and SD's Rural Strong contracts; add VT/VA/FL/AK/NC's missing rows. (2) Put `R/03bi --probe` on a Routine; expect NE's and NV's probes to trip on their award indexes. (3) Register CA's Distressed Hospital Small Grant Program and TX HHS0016568/HHS0016121/HHS0015358 in `non_rhtp_state_programs.csv`. (4) Re-point spec §0.2's GA Dual Track example by patch -- Dual Track is state money. (5) `R/03s`, `R/03t` and `R/03aj` `--build` drop session 49's overlay; re-apply `R/03ap` after any rebuild.

**SESSION 61 LEFT THESE.** (1) **Extract INDIANA'S GROW regional recipients** from `data/evidence/recheck/2026-09-24/IN/` (186 rows, no per-org amounts; region figure in `round_amount`, never divided); pull CMS IN enrolment files first, and give `R/03s` a `--probe` on the regional-grants page. (2) **Merge this branch before 2026-09-24 16:50Z** or the NEWSROOM Routine's first firing gates out and the coverage check names it. (3) Stage 2 on the 09-24 pull, then `R/02c`, then each failing disposition (session-sized). (4) `NJ_VIRTUA_PARENT_BRIDGE` and `NJ_RECIPIENT_FORM_NOT_STATED`. (5) Nebraska 5.3 intent ($5.46M), Louisiana 'RHTP Updates September 2026', Hawaii HPCA $17M -- session 60's leads, still unread.

**SESSION 60 LEFT THESE.** (1) **Extract NEW JERSEY** from `data/evidence/recheck/2026-09-24/NJ/njrht_funding_allocations_2026-07-31.pdf` (103 rows; AHS/AtlantiCare/Inspira print SITES in brackets — §0.3a's awardee-field corollary; NJHA's HRET row is §10.2's association row). (2) **Indiana's Regional Grants**: read the two Governor releases and the GROW page; `R/03s` has no `--probe`. (3) Decide the coverage proposal in the session-60 doc §1 (RCJ pull + `R/01b` on a weekly Routine gated on `/api/stats`; schedule DE ID OH SD TX; one newsroom-sweep probe for the 18 no-release states). (4) Re-run `R/02c` then Stage 2 on the 09-24 pull, expecting the disposition builders to fail and be re-read one by one. (5) `WA_WSHA_FLOW`. (6) `rhtp_footer_parse()` misses HCA's deck variant (no 'with' before '100 percent').

**SESSION 59 LEFT THESE.** (1) **MERGE `claude/gifted-franklin-y69ll9` BEFORE 2026-09-24 09:10Z** — CO and WA fire then, and every new Routine's prompt gates on its probe being on `main`; if the merge slips, `probe_coverage.R --check` will correctly name the unlogged firings. (2) Virginia's two first-tier lists and Washington's priced first-tier sub-recipients are extractable today and were not extracted. (3) The session-55 recheck copy of Tennessee's release carries TDH's anonymous Coveo search token; scrubbing it re-dates that manifest line and is the owner's call. (4) The shared classifier misses "Town of", "... Schools", "Public Library" and "County Mayor"; widening it needs the all-states byte-identical rebuild. (5) MA, MN and NJ still have no CMS release, no RCJ signal since 08-27 and no probe — Tennessee's exact blind spot.

**SESSION 54 LEFT THESE.** (1) West Virginia has no probe; its releases promise
more awards. (2) Vermont's list is "partial and ongoing" and `R/03at --probe`
should go on a Routine (it has none). (3) Connecticut's DSS pages still name
nobody; `R/03ac` watches them. (4) The CMS trigger-list Routine
(`trig_01EozMStALcrUp75s32qFnJ3`) was never re-bound to a pushing session.
(5) The QUEUED eight are AZ CO MT ND RI UT VA WA.

**FIRST: DECIDE HOW ROUTINES PUBLISH (session 52).** They cannot today: no
repository source, no outcome branch. Options: re-bind all 16 to one persistent
"probe runner" session created with `source_url` + `outcome_branch`
(`create_session`, then `create_trigger(persistent_session_id=...)`, then disable
the old triggers), and choose whether unattended runs push to `main` (the one
log file only) or to a `probe-log` branch. New York's prompt already carries
the push step; the other 14 were left as they were. After re-binding, set
`logging_since` in `config/routines.csv` to the switch-over time. **The suite
will go red the first time a registered firing leaves no line and anyone
then logs anything — that is the check doing its job.**

**AFTER ANY PARTIAL REBUILD, RE-APPLY SESSION 49's OVERLAY PER FILE, NEVER
WITH A BARE `--apply`.** A bare `--apply` re-derives its plan from the
already-overlaid files and shrank `verification_queue_2_changes.csv` from 487
rows to 31 in session 52 (restored from git). Use `vq_apply(vq_changes()
filtered to the file)`.

**SOUTH CAROLINA: ASK THE AGENCY ONE QUESTION** — *"Which of the 228 rows on
the Year 1 Award List does your ~60% count as hospital awards?"* — and read
`sc_benchmark_bounds.csv` before adjusting anything. Self Regional's four
site-named rows are typed inconsistently ($2,735,000); recorded, not fixed.

**TWO ROSTERS ARE PUBLISHED AND UNEXTRACTED, AND THEY ARE THE FIRST THING TO DO
(session 51).** Both are archived under `data/evidence/recheck/2026-09-22/`.
- **NEW YORK**: the RCHI awardees list, 56 lead-applicant rows, $76,190,022.00
  (the allocation to the dollar). It has been public since **2026-09-04**. The
  Governor says "90 awards to 56 organizations", so the table is one row per
  LEAD APPLICANT and not per award action. `R/03ag` must be REWRITTEN as an
  award extractor, and §7's third eligible class decides each coding.
- **KANSAS**: Emerging Technology Award Winners, 14 awards, $16,006,648. The
  PDF paints text with the `'` operator and **`R/utils_pdf_text.R` returns
  nothing for it**, so the reader needs that operator first, with the
  byte-identical rebuild check on all 41 committed PDFs.

**THE ROUTINES' VERDICTS ARE NOT REACHING `main`.** New York's roster sat for
18 days while its Routine reported SUCCEEDED four times, and
`logs/probe_results.csv` on `main` holds no New York line at all. Fix that
before trusting any Routine's silence.

**SEVEN OF EIGHT NAME-TRIPWIRE HALTS WERE PAGE FURNITURE** (WI, ME, CA, KY, NC,
DE, ID; the session doc names each). Each needs its page's `known` list
updated, with the sentence that justifies it, or the next firing halts again
before its later checks run. **Do not widen the suffix list.**

**MISSOURI AND CONNECTICUT HAVE AWARDED HOSPITALS AND NAMED NONE.** CMS: "20
rural hospital projects" (~$35M, MO, 2026-09-22) and "four rural hospital
systems" ($50M, CT, 2026-09-17). Watch DSS and DSS-CT for the lists.

**THERE ARE NOW TWO OVERLAYS AND A REBUILD WIPES EITHER ONE SILENTLY.** Session
49's is `R/03ap_verification_queue_2.R --apply` (22 files); session 50's is
`R/03aq_unstated_form_typing.R --apply` (Mississippi and South Carolina). They
do not overlap — no state is in both — but **after ANY `--build` on MS or SC,
run `--apply` on `03aq`**, and after any build on one of session 49's states,
run `--apply` on `03ap`. Iowa's Centers of Excellence pool row is built by
`ia_award_rows()` and survives a rebuild on its own; Iowa's session-49 overlay
does not.

**THREE QUESTIONS OPENED IN SESSION 50 AND THE SECOND IS THE ONE WORTH READING
FIRST.** `IA_COE_POOL_IS_TIER_2` ($50,000,000) — **RESOLVED in session 51, option (b)** — asked whether a **Tier 2** figure
belongs in a partition whose every other dollar is Tier 3 — it was recorded
because it was asked for, and the row's own label is what keeps it legible.
`SC_ACADIA_PARENT_SCOPE` ($5,565,253) is a hospital company's award over a mixed
estate and is 4.8% of South Carolina's new figure. `UF_FORM_NOT_DETERMINABLE`
($3,999,320 over 5 rows) is the residual of the typing pass, and **$3,000,000 of
it is one Mississippi row** — Delta Health Transformation Council, Inc.

**SOUTH CAROLINA'S DOLLAR SHARE NOW OVERSHOOTS THE AGENCY BENCHMARK AND THAT IS
A QUESTION FOR THE OWNER, NOT FOR CODE.** 69.2% against ~60% (65.9% with Acadia
subtracted), while the award count lands on ~50% almost exactly. If the
benchmark counts only RURAL hospitals, or excludes psychiatric hospitals, the
comparison needs re-stating rather than the classification re-doing. **Nothing
was adjusted toward it and a test fails if a later session quietly does.**

**AND THE UNSTATED-FORM QUESTION IS NOW SMALL EVERYWHERE IT WAS LARGE.** Kansas,
Maryland, Nebraska, Oklahoma, Nevada, Michigan, Missouri, Iowa, North Carolina,
Arkansas and Wyoming were answered by session 49's queue; Mississippi and South
Carolina by session 50. **What is left is five rows in four organisations**, and
the CCN match (blocker 5) is still the only thing that can raise any of this
above `MEDIUM`.

**THE VERIFICATION IS AN OVERLAY, NOT A COLUMN EDIT, AND A REBUILD WILL WIPE IT
IF YOU FORGET.** Every state extractor still rebuilds its CSV from its archived
source. `vq_overlay()` (`R/03ap_verification_queue_2.R`) is what re-applies the
305 verified rows on top, and it is idempotent. **AFTER ANY `--build` ON A
TOUCHED STATE, RUN `Rscript R/03ap_verification_queue_2.R --apply`** — or better,
call `vq_overlay(built, "<file>.csv")` inside whatever you are doing, which is
what the ten rebuild-equality tests now do. The twenty-two touched files are
listed by `--apply`'s own output.

**AND IF YOU ADD A STATE FILE, THE OVERLAY DOES NOT COVER IT.** The changes CSV
is keyed on `(file, row index)`, so a state whose row ORDER changes needs the
plan rebuilt (`--report` first, then `--apply`), not patched. `vq_plan()` keys
on the CLASSIFIER's answer — name + recipient_type + determination_confidence —
so a re-run against already-verified files matches nothing and writes nothing.

**FOUR QUESTIONS OPENED THIS SESSION AND THE FIRST IS THE ONLY ONE WORTH REAL
MONEY.** `VQ_INKIND_AFTER_RETYPE` ($10,591,281 against $932,310 held) asks
whether the three rows that moved IN_KIND_BENEFIT -> DIRECT should follow Salina
Regional or Salina should follow them; all four share one generated basis
sentence and a reviewer with the KDHE and MDH award paragraphs settles it in ten
minutes. `MS_FOUNDATION_PARENT_NOT_STATED` is $3,785,455,
`KS_CITIZENS_FOUNDATION_TWO_SPELLINGS` $146,476, `NV_UNIDENTIFIED_RECIPIENT` $0.

**`MS_RECIPIENT_FORM_NOT_STATED` IS NOW THE LARGEST OPEN FORM QUESTION IN THE
REPOSITORY** — 96 rows / $56,022,017.62, 53.8% of everything Mississippi has
awarded — because Mississippi is the one extracted state that was NOT in the
workbook. It is the obvious candidate for a third verification pass, and South
Carolina's 150 rows / $92,759,791.23 (flagged in its own files only, session 47)
is the other.

**AND THE CCN MATCH (BLOCKER 5) IS NOW THE ONLY THING THAT CAN RAISE ANY OF
THIS ABOVE `MEDIUM`.** 306 of the 399 answers are `GENERAL_KNOWLEDGE` and sit at
`LOW` by design; §7 reserves `HIGH` for a CCN match and nothing in this pass has
one. The AHA Annual Survey / CMS Provider of Services extracts would lift
several hundred rows at once.

**THE NAME TRIPWIRE IS NEW AND THE FIRST FIRING WILL LOOK LIKE A DEFECT. IT IS
NOT.** `rhtp_assert_no_new_organisations()` throws when a watched page names an
organisation the committed archive does not, so a Routine will HALT rather than
print CHANGED. Read the names it prints, re-fetch, and then either extract what
you found or add the names to that page's `known` list **with the sentence that
justifies each one**. **Do NOT widen `RHTP_ORG_SUFFIX_TOKENS` and do NOT pad a
`known` list to make it quiet** — that is §6.1's `SWEEP_VERDICTS` rule applied
to a tripwire: widening an exclusion list until nothing is left suppresses the
real ones along with the false.

**AND IF IT FIRES ON SOMETHING THAT IS NOT A NAME, THE FIX IS PROBABLY A
BREAK TOKEN, NOT A WIDER EXCLUSION.** Louisiana's three false positives were
a month welded onto a programme name by the reduction flattening a table cell;
`RHTP_ORG_NAME_BREAK_TOKENS` fixed all three at once. A new one of that shape
belongs there, measured against two archived snapshots the way Louisiana's was.

**EIGHTEEN PROBES CARRY IT AND TWO DELIBERATELY DO NOT.** Alaska's rolling
xlsx roster is diffed by the extraction itself; South Dakota's portal already
has its own organisation-name tripwire. A test reads the source of every file
in `R/` and requires any file offering `--probe` to run it, so **a new state
file will fail the suite until it is wired** — which is the intent.

**MISSISSIPPI'S SECOND TRANCHE IS DATED AND IT IS THE FIRST THING TO CHECK.**
The Governor's 2026-09-14 release says the **Workforce Expansion Initiative**
and **Psychiatric Emergency Services (EmPATH)** *"are currently being reviewed
and will be announced in the next 30 to 45 days"* — **2026-10-14 to
2026-10-29** — and that two further opportunities (care-gap innovation, EHR
modernization) launch in **October**. $101,792,073 of the allotment is in no
public roster. `ms_assert_remaining_tranches()` watches for exactly that and
deliberately does NOT re-fire on the 167 already recorded; Routine
`trig_01RmgWna5oXyoMCXYrVqGVvE` runs Tuesdays and Fridays 09:40 UTC.

**WHEN IT LANDS, EXTEND `ms_parse_release()` TO THE NEW SECTION — DO NOT WRITE A
SECOND FILE.** `ms_year1_awardees.csv` is 167 rows of a **partial year**, and
the three pool counts (97 / 43 / 27) are asserted, so a fourth section fails the
build until it is wired in. **And re-read both odd-shape rows before touching
the parse**: a missing space cost $2,500,000 and the total caught it; an
organisation whose legal name contains `" - "` cost a NAME and a COUNTY and **no
total anywhere noticed**.

**NEW MEXICO IS NO LONGER A CLEAN NEGATIVE AND IS THE NEXT LIKELY EXTRACTION.**
HCA has **selected and named six Healthy Horizons Regional Hubs — three of them
New Mexico hospitals** — with *"more than $74 million in regional funding"* and
**no per-hub amount**. It was deliberately NOT extracted (reported first,
Arkansas's and Wyoming's footing). `nm_assert_hubs_selected_not_awarded()` fails
the day a per-hub figure appears, at which point `R/03ad` must be **rewritten as
an award extractor** — `nm_year1_status.csv` has no `amount` column by design
and `nm_year1_awardees.csv` is asserted absent. **Read the pool rule first**:
$74m/6 = $12.3m is nobody's published figure (§6.2), and **six regions are FIVE
organisations** because UNM holds Regions 2 and 3.

**LOUISIANA'S SIX REMAINING WINDOWS CLOSE 2026-09-30, NINE DAYS OUT.** One has
already passed (Rural Clinician Credit Bank, Mid-September). LDH slipped every
window by 30 to 46 days rather than awarding, so expect either an announcement
or a second slip — and the control now re-dates itself either way rather than
halting. Routine `trig_01BqyRZ4d8S8EvKdtCN9nmX5`, Wednesdays and Saturdays
12:50 UTC.

**READ `logs/probe_results.csv` BEFORE OPENING ANY FIRED SESSION.** Every probe
now appends its verdict there, so "has anything moved?" is a grep rather than a
transcript hunt. A `TRIPWIRE` row is the signal; an `ERROR` row is a statement
about our access and never about the state (§0.4).

**THE COMPLETENESS RE-CHECK IS STALE AND SAYS SO.** Its archive is dated
2026-08-29 and Alaska's extraction is now 2026-09-21, so its AK arm reports the
staleness instead of alleging a withdrawal. Re-run
`Rscript R/03q_state_completeness_recheck.R --fetch --force` with `RECHECK_DATE`
bumped, and extend it beyond the seven states it covers — it found something in
**two of seven** and has never covered KS, SD, TX, MD, MS or WY.

**ALASKA WILL BE STALE AGAIN NEXT WEEK, AND IT HAS RESUMED PLANNING AWARDS.**
244 actions / $239,186,195 as of 2026-09-21; Routine
`trig_01U4RxGWMH8yg37UKupHqTki` runs Mondays 14:00 UTC. **When you refresh it,
roll `AK_PRIOR_FILE` and leave `AK_CMS_ANCHOR_FILE` alone** — they were one
constant until session 46 and rolling the conflated version would move the
142-project reconciliation onto a file CMS never described.

**MISSISSIPPI IS THE FIRST THING TO CHECK AND IT IS THE MOST IMMINENT STATE IN
THIS REPOSITORY (session 44).** Its own funding page says *"The selection
process is complete"* for three programmes and that *"Governor Tate Reeves
will formally announce details regarding all executed sub-awards in the coming
weeks"*, and third-party reporting of 2026-08-06 put the first awards *"by the
end of the month"* — **a date that has passed**. Routine
`trig_01RmgWna5oXyoMCXYrVqGVvE` runs `R/03ak --probe` **TUESDAYS AND FRIDAYS
09:40 UTC**; it ran live and reports UNCHANGED. **Check the Routine is running
against `main`**: its prompt refuses to act if `ms_probe` is absent from
`R/03ak`, so until this branch merges it will report that and stop, which is
intended.

**WHEN MISSISSIPPI ANNOUNCES, WRITE A NEW EXTRACTOR RATHER THAN PATCHING THE
PROBE** — `ms_year1_status.csv` has no `amount` column by design and
`ms_year1_awardees.csv` is asserted absent. **AND PARSE THE CMS SHARE, NEVER
THE HEADLINE**: Mississippi's footer is the only non-100%-federal one here, its
headline is the allotment PLUS an $82,960 match, and the tier check ACCEPTS
that headline as a Tier 2 pool. `rhtp_footer_parse()` and
`rhtp_assert_footer_text_tier()` do it correctly; **do not widen
`RHTP_FOOTER_ALLOTMENT_MARGIN`**. Remember §0.3 too: 700+ applications and
$676M sought against ~$82M available means an applicant list is not an award
list.

**IDAHO AND OHIO SHOULD GO ON ROUTINES, AND NEITHER IS SCHEDULED YET.** Both
have `--probe` implemented and both said more is coming: Idaho has **eleven
opportunities open or just posted** with vendor conferences into September
2026, and Ohio's own release says *"Additional contracts will be awarded in
the coming months"* against **$192,030,262 in no public roster**. Delaware's
probe exists for the narrower case — an amount appearing on four awards that
today have none, which would force `de_year1_awardees.csv` to be REWRITTEN
rather than patched.

**AND WHEN ANY OF THE THREE PUBLISHES MORE, READ THE RECIPIENT COLUMN FIRST
(§0.3a's new second half).** Delaware is the standing proof that §8's name
rule can miss every recipient a state names: it returns `NONPROFIT_CBO`/`LOW`
for Beebe Healthcare, Nemours Children's Health and TidalHealth alike, and
left alone it would report a state with four hospital awards as having none.

**TEN STATES REMAIN QUEUED — AZ CO MT ND RI UT VA VT WA WV, $1,916,786,628 and
24 candidates between them.** Session 43's §2 is still their disposition and
the order to work them in. **West Virginia's four candidates still need
reading before anyone trusts them** — file names as document titles, one of
them the Breast and Cervical Cancer Screening Program, and whether any is RHTP
is not established.

**BLOCKER 2 IS NOW THE CHEAPEST LARGE WIN IN THE REPOSITORY.** Delaware's
extraction used `DE Verify.xlsx` as evidence and did NOT ingest it. The next
step is to bring it into `data/reference/` the way `R/03e` did for the owner's
Florida workbook, so Stage 5 can own the **seven** mis-coded hospital rows —
and rows 8 and 9 in particular, whose Continuous Glucose Monitoring initiative
has published no award notice this environment can reach, which is why they
are named in `de_year1_status.csv` and deliberately NOT in Delaware's award
file (§0.4).

**WYOMING IS EXTRACTED (session 42), SO IT IS NO LONGER A HUNT — IT IS A
WATCH.** 77 award actions, $173,859,752 named, 31 named-hospital rows worth
$72,661,324. Every one of session 41's four warnings was met: the RUN model
reads it and an assertion prices what the line model loses ($5,156,000); the
1.1 class is Illinois's and its form is typed FROM THE SOURCE; every row is an
intent with the obligation deadline in its future; and the reconciliation
closes to the dollar at three levels. **What is left is the calendar.** The
Advisory Committee's **next meeting is 4 or 5 NOVEMBER 2026, with the CMS
federal site visit**, and Wyoming's obligation deadline is END OF OCTOBER 2026
— so between now and then the things that can change are an executed-contract
notice in the Drive folder, an award announcement on the programme page, or a
new call on `wyrhtp.submittable.com` (which today reads *"There are presently
no open calls for submissions"*, and an open call would make this file a
PARTIAL view of Year 1). `R/03aj --probe` watches all four pages and runs the
award tripwires against the LIVE bytes.

**IT IS ON A ROUTINE AS OF SESSION 43**: `trig_016y7C12cyxy7GmGNJDxoAbW`,
**TUESDAYS AND FRIDAYS 21:10 UTC**, first firing 2026-09-04 — twice-weekly
because the obligation deadline falls BEFORE the next committee meeting, and
21:10 because the thirteen Routines already running occupy a continuous band
from 10:50 to 20:20 UTC. It ran live first and reports UNCHANGED on all four
pages. **Check the Routine is running against `main`**: its prompt refuses to
act if `wy_probe` is absent from `R/03aj`, so until this branch merges it will
report that and stop, which is intended.

> **AND THE SENTENCE THAT STOOD HERE WAS WRONG, WHICH IS §2.1's HAZARD IN THIS
> FILE'S OWN "NEXT SESSION" SECTION.** It said *"NO ROTATING-TOKEN STUDY HAS
> BEEN DONE on `health.wyo.gov`"* and that the content digest was inherited
> rather than measured. **Session 42 measured it twice and both measurements
> are already written down** — this file's own session-42 block calls it *"THE
> ELEVENTH ROTATING-DIGEST MECHANISM"* on two fetches three seconds apart
> (199,946 bytes each, different hashes), and `wy_probe()`'s docstring says the
> content digest *"is MEASURED here rather than inherited"* and names the
> second mechanism, the Gravity Forms honeypot label the first live probe
> reported as a changed page. Corrected in session 43. Session 35 caught the
> same shape in `R/03ab`'s header; this one was in the instructions a future
> session reads first.

**AND WHEN WYOMING EXECUTES, `R/03aj` MUST BE REWRITTEN RATHER THAN PATCHED.**
`wy_assert_intent_not_award()` is designed to fail the day the minutes' or
WDH's deadline sentences go, because all 75 rows are intents and
`amount_confirmed = No` is only honest while that is true. The
$30,877,990 approved at POOL level naming nobody at all — the fiscal-agent
sweep, three competitive RFPs, 4.3 and administration — is where the next
roster comes from, as is the Wyoming Innovation Partnership's $38,618,260,
which is in the award file as an `Unclear` pass-through and names no
subrecipient, and `wy_assert_controls()` refuses a SEVENTH
recipient-level table for exactly that reason.

**THE SUPERSEDED SESSION-41 ITEM FOLLOWS, because its evidence notes still
hold.**

**WYOMING WAS THE FIRST ITEM, AND IT WAS AN EXTRACTION, NOT A HUNT (session 41).**
The roster is committed under `data/evidence/WY/` — the Advisory Committee's
*"Award Approvals - 8.11.26"* (SHA-256 in the manifest) with the minutes
beside it. Build `R/03aj_wy_year1_awardees.R` on Arkansas's template:
**read it with `rhtp_pdf_run_table()`, never `rhtp_pdf_lines()`** — two 1.1
rows and four 1.2 rows carry the recipient name in a separate run, and the
line model publishes 16 hospitals where there are 18 and orphans $5,156,000
in silence; assert that the line model still merges. Reconcile 1.1 to
**$48,200,174**, 1.2 to $23,260,000, GME to $17,712,410.00 and the summary to
$205,004,742, all to the dollar, and pin BOTH figures for the University of
Utah (table $9,255,398.00 sums; minutes $9,225,398 does not). Every row is a
committee approval with an end-of-October obligation deadline —
`NOTICE_OF_INTENT_TO_AWARD` + `amount_confirmed = No` — and three 1.1 and two
1.2 awards are CONDITIONAL, which is a flag, not a different row. The 1.1
class is HOSPITALS ONLY; 1.2 is EMS regionalization LED BY hospitals, so
§0.3a judges the recipient. Then put the Drive folder on a watch: the next
committee meeting is **2026-11-04/05, with CMS's site visit**, and the
$17,612,195 fiscal-agent pool and three RFPs name nobody yet. **And record
the new §0.1 mechanism in the disposition file: RCJ files Utah's documents
under Wyoming.**

**INDIANA WAS CHECKED LIVE IN SESSION 42 AND HAS NOT AWARDED.**
`in_assert_regional_not_awarded()` was run against the live page: all three
pre-award predicates hold and the reduced content is byte-identical to the
2026-08-31 archive (the FILE digest moves on LEADING WHITESPACE ALONE — a tenth
digest mechanism and the mildest yet). The 2026-09-03 CMS release NAMES NOBODY:
the $120M *"will support Indiana's GROW regional grants"* across *"eight
regions"*, a Tier 2 announcement of the pool. **BUT THE CLOCK HAS STARTED** —
Indiana's own timeline gives *"July — September 2026 Application review,
scoring, and award determinations"* and **"Sept. 1, 2026 Grant agreement period
begins, and funding distributed to individual entities"**. Re-check it every
session until it awards; `R/03s` has no `--probe` and should get one.

**THE WRONG-STATE SWEEP IS A ONE-OFF MEASUREMENT AND MUST BE RE-RUN, NOT
ASSUMED.** `R/02c_state_attribution_sweep.R` found ten misfiled records in five
states and NONE of them Tier 3 — but that is a property of the corpus as pulled
on **2026-08-27**, not of the aggregator. **Re-run it after the next national
pull**, before extracting any further state: `sweep_assert()` fails if a
misfiled record ever becomes Tier 3, and that failure puts every state file in
question. Its verdicts are HAND-READ; a new flag stops the build until a human
opens the record, which is the design and not a defect.

**HAWAII NEEDS A WATCH ON SHPDA'S PAGE, NOT AN EXTRACTOR.** Its one
non-state award (Hawaii Primary Care Association, 8/28) is FQHC money with an
amount only HANDS carries, and HANDS is 403 to this environment on three
agents. The hospital money is RVBI's *"Statewide Hospital Infrastructure …"*
item, at RFI stage since 8/19; SHPDA annotates each item in a parenthesis
(*"(RFI released on 8/19)"*, *"(Notice of Award on 8/28 to …)"*), so a
content digest of that page plus a tripwire on *"Notice of Award"* inside
the hospital item's parenthesis is the whole probe.

**MASSACHUSETTS: RE-TEST THE ESTATE BEFORE DOING ANYTHING ELSE.** Every
mass.gov path is 403 on four agents; the state's own timeline puts contracts
in fall 2026 through COMMBUYS, which is unreadable. If `web.archive.org` ever
answers, the 2026-08-06 snapshot the availability API reports is the first
thing to read.

**THE SIX WORKED-BUT-UNCODED STATES NOW READ `INVESTIGATED_NO_PROBE` (session
43); WYOMING IS THE SEVENTH AND IS `EXTRACTED`.** HI, MA, MN, NJ, SC and TN
have been worked and are no longer `NOT_EXTRACTED`, which said nobody had
looked — but the new code is **deliberately weaker** than
`INVESTIGATED_NO_LIST` and **the finding still goes stale by construction**,
because none of the six has a probe and only Hawaii has an evidence archive at
all. **THE INTENDED NEXT ACTION IS UNCHANGED: WRITE THE PROBES.** Take **SC
and MN first** — SC has AWARDED and notified by email (712 applications, no
roster), and MN is the largest §0.3 trap in the project (94 pre-identified
eligible entities, 70% of the award to hospitals, formula-based and
non-competitive, with neither the 94 nor any award published). §4 of
`docs/session43_wyoming_watch_low_candidate_queue.md` names what each of the
six probes would watch. **Do not hand-edit the tables** — both are rebuilt
from `R/03k`'s constants.

**THE FOURTEEN-STATE LOW-CANDIDATE QUEUE WAS READ END TO END IN SESSION 43 AND
IS NO LONGER A HUNT — IT IS THREE EXTRACTIONS AND ELEVEN WATCHES.** MT, MS,
OH, CO, WV, ND, UT, VT, VA, ID, WA, AZ, DE, RI, $2,668,093,442 between them.
Full disposition: `docs/session43_wyoming_watch_low_candidate_queue.md` §2.
**NOTHING WAS EXTRACTED — the task was to report first, Arkansas's and
Wyoming's footing** — so what follows is the order to work them in.

1. **MISSISSIPPI FIRST, AND IT IS THE MOST IMMINENT STATE IN THIS
   REPOSITORY.** Selection is COMPLETE on three programmes and the Governor
   has PROMISED a named announcement of *"all executed sub-awards"*. Its first
   awards were due *"by the end of the month"* in an 2026-08-06 report, so
   that date has PASSED. **Read the CMS SHARE out of its footer, never the
   headline** — Mississippi is the first non-100%-federal footer here and the
   headline is ACCEPTED as a Tier 2 pool while being the allotment plus an
   $82,960 match. `mississippirhtp.com` is a dedicated RHTP domain with five
   initiative pages, so a probe is cheap.
2. **DELAWARE IS A FOUR-ROW EXTRACTION WITH `amount` EMPTY ON EVERY ROW** —
   Beebe ×2, Nemours, TidalHealth — and it is **§0.3a's worked example as live
   data**, so it codes `DIRECT`/`Yes` and NOT `NON_HOSPITAL`. It would be the
   first Delaware award file and it needs its own tripwire, because DHSS may
   yet price these.
3. **IDAHO AND OHIO NAME ONE AWARDEE EACH** (Comagine Health, no amount; Ohio
   University, $10M). Idaho has ELEVEN open opportunities and is the likeliest
   of the fourteen to grow.
4. **MONTANA AND COLORADO ARE NEGATIVES WITH DATES INSIDE SEPTEMBER 2026** and
   want watches, not investigations.
5. **WEST VIRGINIA, ARIZONA, WASHINGTON AND UTAH NEED THEIR CANDIDATE SETS
   READ BEFORE ANYTHING ELSE** — see §2.4 of the session doc. West Virginia's
   is the one that could mislead: four state grant documents titled by
   FILE NAME, one of them the Breast and Cervical Cancer Screening Program,
   and whether ANY of them is RHTP is **not established**.
6. **ND, VT, RI and VA are clean negatives**; RI's three candidates are opioid
   settlement money the session-20 sweep already caught.

Work them by allotment, not by candidate count — Iowa is the standing proof
that the count says nothing, and Delaware proves it again from the other end:
five candidates, four of them real awards nobody had looked at.

**ARKANSAS IS EXTRACTED AND ON A TWICE-WEEKLY WATCH (session 40), SO IT IS NO
LONGER A MANUAL ITEM.** 31 organisations, 37 priced award actions,
$149,177,618.45, 9 named-hospital rows / $21,792,687.96. Every one of session
39's four warnings was met: the RUN model reads the columns and an assertion
keeps the line model's merge visible; the `Total:` row is the reconciliation
target and a test drives the mistake of reading it as a recipient; the 16
unstated-form organisations are queued at $100,723,693.49 with **nothing
promoted**; and RISE AR and HEART are watched rather than assumed. `R/03ai
--probe` runs as Routine `trig_01Sw1CDPqYQKXFq4WEVRHH2a`, **THURSDAYS AND
SUNDAYS 20:20 UTC**; it ran live and reports UNCHANGED. **Check the Routine is
running against `main`**: its prompt refuses to act if `ar_probe` is absent
from `R/03ai`, so until this branch merges it will report that and stop, which
is intended.

**WHAT ARKANSAS WILL DO NEXT, AND IT IS THE HALF OF THE YEAR THAT IS MISSING.**
RISE AR and HEART have not awarded; both application windows have CLOSED
(2026-07-24, 2026-08-07), **neither publishes an award date**, and ~$59.6M of
the $208,779,396 allotment is unawarded against *"the $209 million the state
expects to award by this fall"* and CMS's own obligation deadline of
**2026-10-30**, which both NOFOs print themselves. **When either awards,
`R/03ai` must be REWRITTEN rather than patched** — `ar_assert_award_index()`
refuses a SECOND award-list link on the home page for exactly that reason, and
the file's 37 rows are two initiatives of four. **Read the eligible class off
the AWARD, not the NOFO**: it is *"rural hospitals, clinics, EMS providers,
behavioral health providers, CINs"* — hospitals AMONG OTHERS, New Hampshire's
`Unclear` and not Illinois's `Yes`.

**AND DO NOT READ THE INITIATIVE PAGES FOR AWARD STATUS.** All four carry the
identical *"What is next? Upcoming announcements ... via Notices of Funding
Opportunities (NOFOs)"* sentence, including THRIVE and PACT, which have
awarded $149M between them. The signal is the **home page's award-list link**.

**SOUTH CAROLINA IS THE SECOND, AND IT NEEDS A WATCH RATHER THAN AN
EXTRACTOR.** It **has awarded** — bulletin MB# 26-026, 2026-07-31, *"Notices of
Award Determination for the 712 applications"* — and told the recipients **by
email**, publishing no roster, no amounts and not even a count of awards. That
is a new shape here (South Dakota at least published counts and totals), and it
means `sc_year1_status.csv` with **no `amount` column** (Texas's device) is the
honest artifact today, not an award file. **Read the §0.1 trap before writing
anything**: SCDHHS's RHTP page also carries two REAL named rosters — the $48.2M
Rural & Medically Underserved Area Grant (awarded 2024-02-02) and the ~$35M
Behavioral Health Crisis Stabilization Grant **to hospitals** (awarded
2023-06-23) — and **§6.2's date test disposes of both on its own**, 18-30 months
before the NOA. The §7 route is **SCRA**, which SCDHHS *"has contracted ... to
manage and administer the Tech Catalyst Fund"* and which has not awarded.

**MINNESOTA IS THE §0.3 TRAP TO WRITE DOWN BEFORE IT LANDS.** MDH allocates
**70% of its award to hospitals as a formula-based, NON-COMPETITIVE grant**,
against **94 named-in-advance eligible entities** — ~$135.2M, the most
hospital-weighted allocation in this repository. A formula allocation to a
...
*"awarded"*. Its direct-allocation deadline passed 2026-05-26; one competitive
RFP closes 2026-09-14.

**TENNESSEE AND NEW JERSEY ARE NEGATIVES AND NEITHER NEEDS ANYTHING SOON.**
Tennessee is at solicitation stage with a clean positive control and its award
channel — the Caspio RHTP Partner Portal — is **UNREADABLE/UNKNOWN (§0.4)**.
New Jersey's own SORH page still says the state *"applied for"* the funding and
its full FY2026-27 grants directory contains *"rural"* zero times.

**EIGHT ZERO-SIGNAL STATES REMAIN** — HI, MA, WY and the five behind them. Three
of the thirteen checked so far had published rosters (FL, NC, **AR**), so work
the rest in full rather than opportunistically, and use the channels this pass
proved, in this order: **`/api/v1/activity` first** (it supplied the route in
all five), then the **governor's newsroom**, then the agency's **grants
directory or bulletin channel** — South Carolina's award notice is a Medicaid
bulletin, not a press release, and is invisible from the programme page.


**NORTH CAROLINA IS EXTRACTED AND ON A WEEKLY WATCH (session 38), SO IT IS NO
LONGER A MANUAL ITEM.** 44 named recipients, `amount` empty on all 44, and 0
rows and $0 in every bucket. `R/03ah --probe` runs as Routine
`trig_01SepuwJWXSkqpNQ9WgJkxMb`, **Saturdays 10:50 UTC**; it ran live and
reports UNCHANGED. **Check the Routine is running against `main`**: its prompt
refuses to act if `nc_probe` is absent from `R/03ah`, so until this branch
merges it will report that and stop, which is intended.

**WHAT NORTH CAROLINA WILL DO NEXT, AND IT IS THE SECOND TIER.** Each Hub Lead
runs its own regional funding opportunities and **that is where the hospital
money is**; Trillium's Region 2 page names nobody today and
`nc_assert_positive_control()` is designed to FAIL the day one does. Two more
things are overdue: two opportunities closed with no roster and **both
application dates have passed** (NC Minority Diabetes Prevention 2026-07-17,
Expanding School Health Centers to Rural Areas 2026-08-12), and the
**$20M-a-year Rural Health Innovation Fund *"will launch this fall"***. And
`nc_assert_row_count_is_the_finding()` fails the day a per-recipient amount
appears, at which point `nc_year1_awardees.csv` must be **REWRITTEN, not
patched** — its empty `amount` column is only honest while NCDHHS publishes
none.

**TWO QUESTIONS ARE OPEN IN THE REVIEW QUEUE AND BOTH ARE WORTH $0.**
`NC_MIH_FORM_NOT_STATED` (Cape Fear Valley, Clay County) moves North Carolina's
named-hospital ROW COUNT from 0 to 1 under one of its three options and nothing
else; `NC_HUB_LEAD_FORM_NOT_IN_VOCABULARY` asks what §8 code three Hub Leads
take when the state states a form §8 does not carry. **Neither is answered
here, and neither should be answered in the extractor.**

**KENTUCKY AND NEW YORK ARE BOTH ON TWICE-WEEKLY ROUTINES NOW (session 38), SO
NEITHER IS A MANUAL ITEM EITHER.** New York is `trig_01G2ZijwemE2YgbAuiBirKs2`,
**SUNDAYS AND WEDNESDAYS 11:10 UTC** — the tightest clock in the project, since
its RCHI contracts were due to BEGIN 2026-09-01 and **all contracts must be
EXECUTED by 2026-10-30**, against 91 applications DOH says are still under
review. Kentucky is `trig_01FqfAJvnMzz811goNzNZYqi`, **MONDAYS AND THURSDAYS
19:30 UTC**, because it published TWO award-notification dates itself
(2026-07-10, 2026-08-26) and both have passed. Both prompts refuse to act if
their probe is absent from `main`. **When either awards, its `R/03af` or
`R/03ag` must be REWRITTEN as an award extractor rather than patched** — both
status tables have no `amount` column by design and both award files are
asserted absent. **And read New York's eligible class off the AWARD, never off
the rule** (§10.2's third worked case, added this session).

**NINE OF THE TWELVE NO-SIGNAL STATES ARE STILL UNOPENED, AND THEY ARE WHERE
THE REMAINING MONEY IS.** AR, HI, MA, MN, NJ, SC, TN, WY — plus NC, now worked
but not extracted. **North Carolina is the proof that the group is worth
working**: no CMS release, no RCJ candidate, and 44 named recipients already
published. Use the questions this file has accumulated rather than a candidate
count, because there is no count to use.

**LOUISIANA IS THE STATE MOST LIKELY TO MOVE NEXT, AND IT IS ALREADY OVERDUE
SEVEN TIMES OVER (session 36).** Every one of its seven Budget Year 1
solicitations has an announcement window LDH published itself, and **all seven
closed before session 36 ran**. Routine `trig_01BqyRZ4d8S8EvKdtCN9nmX5` runs
`R/03ae --probe` **Wednesdays and Saturdays 12:50 UTC** (first fire
2026-09-05); it ran live and reports UNCHANGED. **Check the Routine is running
against `main`**: its prompt refuses to act if `la_probe` is absent from
`R/03ae`, so until this branch merges it will report that and stop, which is
intended. **When Louisiana awards, `R/03ae` must be REWRITTEN as an award
extractor rather than patched** — `la_year1_status.csv` has no `amount` column
by design and `la_year1_awardees.csv` is asserted absent. **The pool to watch
is Capital Improvement**: $41.60M projected, 160 applications, awards of
$100,000–$10,000,000, and a class of hospitals AMONG OTHERS, which is
New Hampshire's `Unclear` and not Illinois's `Yes`. **And do not read
`rhtla.net/api/facilities` as a roster** — 3,576 facilities, 305 named
hospitals, no money in it at all.
...
**MAINE IS ON A SCHEDULE NOW (session 34), SO IT NO LONGER NEEDS A MANUAL FIRST
ITEM.** `R/03aa --probe` runs as Routine `trig_01ALmis3jAgiLXVb6ZDLCj8Y`,
**Tuesdays and Fridays 16:20 UTC** (first fire 2026-09-04) — Wisconsin's
cadence rather than Alaska's, because Maine is stale by appointment and one
appointment has already passed. **Check the Routine is running against `main`**:
its prompt refuses to act if `me_probe` is absent from `R/03aa`, so until this
branch merges it will report that and stop, which is intended.

**CALIFORNIA IS THE OTHER STATE ON THE SHORTEST CLOCK, AND IT IS NOT YET ON A
SCHEDULE (session 34).** All four CalRHT opportunities are closed with award
notification due **August / September 2026** and grant agreements **September /
October 2026**; WDRR's window closed **2026-08-31**. `R/03ab --probe` exists,
runs live and reports UNCHANGED, and it should go on Wisconsin's and Maine's
twice-weekly footing at the start of the next session. **When California
awards, `R/03ab` must be REWRITTEN as an award extractor rather than patched**
— `ca_year1_status.csv` has no `amount` column by design and
`ca_year1_awardees.csv` is asserted absent. **And read the eligible class
before coding any pass-through**: every CalRHT pool names hospitals AMONG
OTHERS, which is New Hampshire's `Unclear` and not Illinois's `Yes`.
...
5. **THE RCJ QUEUE IS NEARLY EXHAUSTED AS A DISCOVERY SIGNAL, AND THE NEXT
   PHASE IS STATES WITH NO SIGNAL AT ALL (session 36).** 25 states are worked
   (1,332 candidates between them); **14 remain open holding 34 candidates
   TOTAL, and the largest is FIVE ROWS** — DE 5/5, AZ 4/2, CO 4/4, WV 4/4,
   MS 3/3, RI 3/3, then eight states at 2 or 1. Working that tail down is
   still worth doing and it is no longer where the money is.

   **TWELVE STATES CARRY NO RCJ SIGNAL AT ALL AND HOLD $2.36 BILLION OF
   ALLOTMENT**: NC ($213.0M), KY ($212.9M), NY ($212.1M), FL, AR, TN, WY, SC,
   MN, HI, MA, NJ. Eleven are `NOT_EXTRACTED` with no CMS release either —
   `trigger_source = NEITHER` on **both** discovery layers.

   **FLORIDA IS THE TWELFTH AND IT IS THE EXISTENCE PROOF.** No CMS release,
   no RCJ candidate, and **81 extracted awards worth $188,201,256** already in
   this repository, reconciled EXACTLY against the Governor's own awardee PDF
   (session 21). A state invisible to both discovery layers had published a
   complete Year 1 roster the whole time. **So `NEITHER` has never meant the
   state awarded nothing, and there are now eleven more of them to read.**
   Start with the largest allotments — NC, KY, NY — and use the questions this
   file has accumulated rather than a candidate count, since there is no count
   to use.

   **CA, CT, KY, LA, NM, NY, TX and WI** are `INVESTIGATED_NO_LIST`; **AR**,
   IA, KS, MD, ME, MI, MO, NC, NE, NH, IN, OK and NV are `EXTRACTED`.

   **AND ARKANSAS IS THE THIRD PROOF, AND THE LARGEST (session 40).** No CMS
   release, no RCJ candidate, and **31 organisations / 37 priced award actions
   / $149,177,618.45** already published on a dedicated RHTP domain — 71.5% of
   its allotment — plus a 50-project roster from the Governor that reconciles
   to the cent. Three of the thirteen no-signal states checked so far had a
   published roster (FL, NC, AR), so the group is worth working in FULL rather
   than opportunistically.

   **AND NEW MEXICO ADDS THE NINETEENTH QUESTION, WHICH IS THE ONE THAT ASKS
   WHY A CANDIDATE SET IS CHEAP: IS THE $1 PLACEHOLDER HIDING SOMETHING?**
   Missouri and Maine established that RCJ prices the wrong KIND of action at
   $1. New Mexico shows what that costs a survey: its whole
   `rcj_federal_amount_sum` is **$7**, so it ranks as the emptiest candidate
   set in the file — and behind those seven rows sit a state fund, a
   wrong-section title, and TWO NAMED NEW MEXICO HOSPITALS. **A $1 row is not
   a cheap row; it is an unpriced row, and the reason it is unpriced may be
   that the aggregator has the wrong document.**

   **AND CONNECTICUT ADDS THE TWENTIETH, WHICH IS THE CHEAPEST TRIPWIRE IN
   THIS LIST: HAS THE STATE PUBLISHED ITS OWN AWARD DATE, AND HAS IT PASSED?**
   Connecticut's OPM page prints NOFO #26OHS001's full timeline down to
   *"Grant Awards Announced ... August 17, 2026"*. That single line dates the
   negative, sizes the wait, and tells the next session exactly what it is
   looking for — and it had already gone by when this ran. Wisconsin's council
   deck did the same thing (session 30) and Maine's DOE page did (session 33);
   Connecticut is the first where the date is **in the past and the roster
   still absent**.

   **AND CALIFORNIA ADDS THE EIGHTEENTH QUESTION, WHICH IS THE ONE THAT WOULD
   HAVE COST THIS PROJECT ITS MOST ATTACKABLE NUMBER: WHAT DOES THE AGGREGATOR'S
   OWN DESCRIPTION SAY THE MONEY BUYS?** Every previous §0.1 check asked about
   the recipient, the tier, the programme label or the date. California's eleven
   candidates pass all of those on sight — named rural hospitals, real amounts,
   real executed awards, the right agency — and RCJ's own description field
   says *"MTCAP, MTCAR, NPC 5 Evaluation of campus, and an SPC-4D Evaluation of
   the Hospital Building"*. Those are **seismic engineering deliverables**, and
   they were sitting in the committed pull the whole time. **Read the
   description before the name.**
...
`MEDIUM`, with **over 340 hospital recipients** waiting on a CCN match — Georgia
adds 21 and Alaska 6 in session 22, and the CCN match is also what resolves
Kansas's 23 rows / $40,182,073, Maryland's 24 / $36,558,089 and — the largest
of them all — **Arkansas's 16 organisations / $100,723,693.49** of unstated
