...
**SESSION 71 LEFT THESE.** (1) Decide `ENROLLED_HOSPITAL_STATE_STATED_OTHER_FORM` (10 rows, $2,088,858: Alaska's tribal/state-agency typed rows and Oregon's hospital-owned RHCs), where a state source states a form CMS contradicts. (2) `AHC_STRING_NAMES_NO_ENROLLED_ENTITY`: UAB Montgomery, OHSU Casey Eye, U Michigan MEDIC, ORPRN/Oregon Perinatal, and UMMS (a system, not a university). (3) Only AR, NV, LA and WA builders chain `s71_overlay()`. After any other touched state's `--build` (AL AK OR WY MI GA NC IA OK), run `Rscript R/03bj_enrolled_hospital_operator.R --apply`. (4) A new CMS enrolment file or a new state file can add sweep hits; `eh_assert_read()` stops the build until each is hand-read into EH_APPLY or EH_HOLD.
