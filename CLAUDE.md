...
- **`data/reference/me_year1_status.csv` — 7 rows (Session 33; a row added by
  the 2026-09-04 watch), AND IT HAS NO `amount` COLUMN.** What each Maine RHTP
  channel publishes. **The seventh row is `PLANNED_RECIPIENT_TBD`**: Hospital
  Efficiency & Financial Management Funds (Capital), **$29,000,000** plus
  **$35,350,000** for oversight and planning, recipient *"Provider TBD via
  competitive procurement"* — from Maine's own **Revised Y1 Budget Narrative**
  (dated 2026-03-25, published onto the DHHS programme page after 2026-09-02).
  Its scope names hospitals as the parties **paid**, so this is where Maine's
  hospital money is planned to go, and **no hospital is named**. **Those figures
  are NOT reconciled against the RHEF's announced $30,000,000 and must not be**
  — different lines, different periods, and a plan is not an award (§0.3); a
  test pins the refusal. Four rows are pre-award
...
```
Rscript tests/run_tests.R                        # 5,194 assertions, zero quota
